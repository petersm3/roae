...
The constraint solver (`solve.py` + `solve.c`) finds that 5 rules extracted from King Wen narrow 10^89 possible orderings to **at least 31.6 million** (partial enumeration on 64 cores — true count is higher). Only Position 1 is universally locked. Earlier claims that 23 of 32 positions were locked and that two adjacency constraints gave uniqueness were based on a 438-solution partial sample. A larger enumeration shows 4 boundary constraints (not 2) are needed to uniquely determine King Wen. The rules are confirmatory (extracted from King Wen, then shown to be highly constraining) rather than predictive (derived independently). See [SOLVE.md](SOLVE.md) for details.
