...
The constraint solver (`solve.py`) finds that 6 rules extracted from King Wen empirically lock **23 of 32 pair positions**. The remaining 9 positions have thousands of valid orderings, narrowed to 1 by two specific adjacency constraints. The rules are confirmatory (extracted from King Wen, then shown to be highly constraining) rather than predictive (derived independently). See [SOLVE.md](SOLVE.md) for details.
