...
- The constraint solver (`solve.c`) finds that 5 rules narrow 10^89 possible orderings to hundreds of millions. Canonical counts (2026-04-18): **706,422,987** at the d3 10T partition (sha `f7b8c4fb…`), **286,357,503** at the d2 10T partition (sha `a09280fb…`). Both are partial enumerations at different partition strategies; under true exhaustive enumeration they would converge. Only Position 1 is universally locked. Four boundary constraints uniquely determine King Wen at both scales, with boundaries `{25, 27}` partition-stable (mandatory at both d2 and d3); the other 2 boundaries in the 4-set are partition-dependent. See [SOLVE.md](SOLVE.md) for the full analysis.
