...
- **The C6/C7 boundary adjacencies** (from [`SPECIFICATION.md`](SPECIFICATION.md) §C6, §C7) are NOT required in `solutions.bin` — the file contains all C1–C5 solutions, and C6/C7 are additional constraints used to narrow from ~742M to King Wen specifically. Your verifier should NOT reject a record that fails C6 or C7; the file intentionally contains many such records.
