...
> **⚠️ The d3 560T canonical (`9a968fa2…`) is SUSPECT — opened 2026-06-21.** A proven solver eviction-resume defect (CASE-D write-ordering) + the campaign's 5 Spot evictions on the pre-fix solver mean the 560T run **may be incomplete**. **This document's headline result depends on 560T-specific data** (the greedy minimum dropping back to 4 at 560T); if re-validation revises the 560T record set, the "4 at 560T" and the non-monotone 4→5→4 claim **may change**. The 10T and 100T points are unaffected. Resolution: a from-scratch re-run will reproduce `9a968fa2` (→ **CANONICAL-verified**) or **supersede** it. See [CANONICAL_HASHES.md](CANONICAL_HASHES.md) §"d3 560T".
