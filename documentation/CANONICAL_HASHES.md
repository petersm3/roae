...
**Campaign details:** D128als_v7 Spot westus3 enum, 4 TB Premium SSD for shards, 171.5 h wall time across 5 weekday Spot evictions (all in a tight 07:12-07:49 PT window — see [HISTORY.md](HISTORY.md) "June 1-8, 2026" entry). 158,364/158,364 cells scanned (100%), 65,281 cells produced solutions (41.2% yield). Merge: D16als_v7 Standard, external chunked-sort on Premium scratch, 18 h 42 m wall. 43.88 B pre-merge shard records (per-sub-branch canonical) → 10.525 B unique canonical (4.17× cross-sub-branch rediscovery ratio — NOT an orientation-dedup ratio) ⚠ **[LABEL CORRECTED 2026-08-28 — this clause previously called the pre-merge total a count of raw records and its ratio a deduplication ratio. `solve.c` hashes and dedups on pair identity with the orient bit masked, and CLEARS each thread's table after every sub-branch (cited by symbol rather than line number, re-verified against this tree 2026-09-07, because `solve.c` line numbers drift: `analyze_solution()` hashes and compares `canonical[]` — the record with the orient bit cleared, matched as `existing[ci] & 0xFC` — and `flush_sub_solutions()` / `flush_sub_solutions_d3()` each end by `memset`-ing `ts->sol_table` and zeroing `ts->solution_count`), so a shard record is a per-sub-branch CANONICAL key rather than a raw oriented leaf. The record total is therefore a LOWER BOUND on raw leaves visited, and the ratio measures cross-sub-branch rediscovery under the depth-3 partition, not orientation multiplicity. Both values are unchanged, and the old-vs-new run comparison above is unaffected because it compares two totals of the same kind. The reasoning is set out at [CAMPAIGN_METHODOLOGY.md](CAMPAIGN_METHODOLOGY.md) §7 ("Worked example — the 560 T canonical campaign"), whose pre-merge shard-record row carries the same marker.]**
...
- C-side: `solve --verify solutions.bin` — checks every record satisfies C1–C5, plus sorted-order and dedup, per [SPECIFICATION.md](SPECIFICATION.md). ⚠ **King Wen's presence is printed, not enforced, by `--verify`** (measured 2026-09-02: the King Wen record deleted from an artifact and the header count patched → `King Wen found: No` … `VERIFY=PASS`, rc 0). Read that line by eye, or pass **`--expect-kw`** to fold it into the verdict: under that flag King Wen's absence sets `fail_kw`, the run reports `VERIFY=FAIL` with rc 1, and `KW_REQUIRED` records which contract was in force. *(Corrected 2026-09-02 — this line previously listed "KW-present" among the checks.)* ⚠ **[CORRECTED 2026-09-07 — this bullet went on to call folding it into the verdict "a prepared `solve.c` change held behind the solve.c change gate". It shipped on 2026-09-04 and is in the tracked binary's source:** `--expect-kw` is parsed in both of `main`'s argv branches that accept it — the `--verify` branch and the `--validate` branch, each matching `strcmp(argv[ai], "--expect-kw")` — gates inside the `--verify` handler's `if (g_expect_kw && !kw_found_v)` guard, and sets `fail_kw`, the last term summed into `total_fail`, mirroring `verify.py`'s flag of the same name. (Cited by symbol rather than line number, re-verified against this tree 2026-09-07, because `solve.c` line numbers drift; the two parse lines are byte-identical, so they are identified by their enclosing argv branch and not by an address.) The C-side and Python-side bullets therefore described the same flag as unshipped and shipped, one line apart. **The reported-not-enforced DEFAULT is unchanged and remains correct** — only the flag's availability was misstated. Found in the same 2026-09-07 doc-vs-code sweep that corrected this page's 560 T `--verify` witness row, which cites this bullet.]**
...
**Size convention (applies to every entry above):** the **File size** field is the **logical (decompressed) size** of the canonical stream *including* the 32-byte header — `gzip -dc solutions.bin | wc -c`, the same stream the registered sha256 is taken over — and the record count is `(size − 32) / 32`. A merge/`--analyze` log line that reports "records × 32" (record-bytes only) is 32 bytes short of that logical size — that fence-post is the source of the 2026-06-14 false-corruption alarm and the 2026-07-04 100T count re-correction. ⚠ **[CORRECTED 2026-09-25 (Q-758, Codex V3A-008#3) — this read "the **File size** field is always the on-disk size".** Since #169 `solutions.bin` is gzip-framed by default. Its on-disk size is the compressed container, which `ls -l` reports and which matches no registered figure. The registry's sizes and shas are both of the decompressed stream, as the re-derive recipe above (`gzip -dc solutions.bin | sha256sum`) and [TR-3](../reports/TR3_REPRODUCIBLE_ENUMERATION.md) ("compare it against `gzip -dc solutions.bin | wc -c`, not `ls -l`") already say. Measured 2026-09-25 on the worker, re-running the `--selftest` child's own enumeration with the directory kept: magic `1f8b`, on-disk 512,472 B, `gzip -dc | wc -c` = **4,344,992 B** = 32 + 135,780 × 32, and `gzip -dc | sha256sum` = `403f7202…`. The container's own sha256 is a different digest, and it is not the canonical one. A reader checking an on-disk size against this registry would have declared a good artifact corrupt. No registered size or sha changes.]**
