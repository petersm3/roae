...
**v3 (= v1 sha-equivalent at every tested scale)** is the canonical-producing lineage on `main` HEAD post-2026-05-25 reset. The "Active canonicals" above are all v1/v3 sha-equivalent rows. v2's canonicals are recorded below in **Historical (frozen lineages)** — they remain valid as the historical v2 record but are no longer produced by `main`.
