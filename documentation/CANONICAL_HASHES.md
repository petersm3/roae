...
**v2 lineage CLOSED (2026-05-24, operator directive):** No further v2 runs are planned at any scale. The v2 11.2T `2cc966e4…` and v2 100T `cc4a5377…` canonicals stand as the historical v2 record (not deleted, just frozen). The v2 prune-stack source code remains in `main` but is superseded for future runs by the v3 lineage. **v3 is the canonical-producing lineage going forward** (sha-preserving on v1 prunes + LTO/PGO/bitset speed wins; Phase 11/12 validation in flight 2026-05-24). v1 remains the sha anchor that v3 reproduces byte-identically.
...
**Records-per-dollar at 560T-class budgets:** v3 is sha-preserving on v1's prune predicates AND ~9.2% faster per node (LTO + PGO + bitset; Phase 11 cross-build gates this empirically). v2's prune-stack overhead makes it ~3× v1's per-node cost. Net at any fixed dollar budget: v3 processes ~3× more nodes than v2 and finds ~2-5× more records — and v3's record set at the same dollars approximately ⊇ v2's record set at v2's smaller node budget. v2 wins on records-per-node-budget; v3 wins on records-per-dollar. Phase 11/12 v3 validation gates the 560T solver decision; if v3 reproduces v1's 11.2T and 100T canonicals byte-identically, **v3 becomes the unambiguous 560T solver choice on records-per-dollar**. v2 is retained as the prune-efficiency-bench lineage at 11.2T + 100T (its empirical record). See `petersm3/x:roae/V1_V2_V3_RECORDS_PER_DOLLAR_ANALYSIS_2026_05_24.md` for the full derivation.
