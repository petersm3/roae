...
| Boundary-minimum is non-monotone with scale (4 → 5 → 4) | earlier ROAE publication (2026-06-11) | **CORRECTED** (self, 2026-07-04) | A survivor-counting artifact: the "4 at 560T" stopped at 1 remaining non-KW survivor instead of 0. The trajectory is monotone 4 → 5 → 5, with the identical 5-set {1, 4, 21, 25, 27} at 100T and 560T; the lone 4-boundary survivor is rec#330177707 (KW with positions 2–3 pair-swapped). *Reproduction inputs: the 10T/100T analyze logs ship under `runs/`; the 560T leg reads `analyze_v3_560T.log` from the archived 560T canonical, which is not in this repository* | [BOUNDARY_MINIMUM.md](BOUNDARY_MINIMUM.md) |
