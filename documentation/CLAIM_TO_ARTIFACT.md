...
| 8 | The conflict has **four** minimal two-rule cores | TR-2 | `core_{gender_ccn8,parity_ccn4,rhythm_ccn4}_unsat.drat.gz` — `core_gender_ccn4_unsat.drat.gz` is **not** in `reports/certificates/` | per-pair CNF + `kissat`, then `drat-trim` | 3 of 4 CERTIFIED publicly; the fourth ({gender, S25–28}, found 2026-08-28) is verified, certificate shipping pending — see CORRECTIONS |
