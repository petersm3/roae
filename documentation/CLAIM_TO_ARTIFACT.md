...
| 12 | 22 DRAT certificates verify | `reports/certificates/` | the archived `.drat` set | `bash reports/certificates/verify_all.sh` | CERTIFIED — **22/22 `PASS cert`, zero FAIL, executed 2026-09-02 on the shipped tree** (drat-trim `2e3b2dc0`). Supersedes the 21/21 replay of 2026-08-28, which predated the 22nd file. ⚠ `verify_all.sh` pipes drat-trim into `grep -q`, so the log records only the PASS line, not the checker's own `s VERIFIED` output — queued to be teed |
