...
| 5 | Exactly 15 parity-class alternations | TR-6 | `lean/KingWen.lean` `alternations_15_general` | `lean lean/KingWen.lean` | KERNEL — the independent leg; see row 6 |
...
| 7 | The four literature rules are jointly unsatisfiable | TR-2 | `grander_strict_unsat.drat.gz` | `python3 sat.py --emit-cnf grander-strict f.cnf && kissat f.cnf` | CERTIFIED |
| 8 | The conflict has **four** minimal two-rule cores | TR-2 | `core_{gender_ccn4,gender_ccn8,parity_ccn4,rhythm_ccn4}_unsat.drat.gz` | per-pair CNF + `kissat`, then `drat-trim` | CERTIFIED — the fourth was found 2026-08-28; see CORRECTIONS |
...
| 10 | Nine pre-registered Davis composites: five null, one notable, three data-like | TR-10 | `reports/evidence/dav_tier1.out` | `./solve --dav-verify` | ESTIMATE — masses now carry SEs; two verdicts corrected 2026-08-28 |
| 11 | KW's complement-distance sum C3 = 776 | TR-5, Lean | `lean/KingWen.lean` `kw_c3_exactly_776` | `lean lean/KingWen.lean` | KERNEL |
| 12 | 21 DRAT certificates verify | `reports/certificates/` | the archived `.drat` set | `bash reports/certificates/verify_all.sh` | CERTIFIED — 21/21 `s VERIFIED`, executed 2026-08-28 |
| 13 | 13 Lean modules are kernel-clean, no `native_decide` | `lean/README.md` | the 13 `.lean` sources | `lean lean/<Module>.lean` per module; `#print axioms` | KERNEL — 13/13 executed 2026-08-28; axioms ⊆ `[propext, Classical.choice, Quot.sound]` |
| 14 | n=9 canonical count = 26,112 | TR-11 | CPOG certificate | `python3 sat.py --certify-count f1c5 --f1-pairs 9 --expect 26112` | CERTIFIED — reproduced on a third host, byte-identical |
| 15 | KW satisfies C1/C2/C4/C5 | TR-11, Lean | `lean/KingWen.lean` `kw_valid` | `lean lean/KingWen.lean` | KERNEL |
