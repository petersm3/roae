...
| 16 | The 31-rule literature registry | `LITERATURE_RULES_POPULATION_TESTS.md` | `solve.py` `REGISTRY_KW_EXPECTED` | `python3 solve.py --registry-verify` | MIXED — **26 of 31** reproduce a source-stated KW value; **5** are KW-derived ⚠ *(was 27/4 until 2026-09-07: `d7` was counted as source-stated, but its eight slots are hard-coded from King Wen's own twelve xiaoxi positions, so KW's 8/8 is guaranteed by construction — LITERATURE_RULES_POPULATION_TESTS.md classifies it DATA-LIKE, and shifting the window by 1–5 drops KW to 4, 0, 1, 2, 1)* — KW-measured anchors |
