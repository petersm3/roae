...
## 2026-09-04 — three lines of the 2026-08-23 withdrawal entry are restored, because a reword on a branch had removed them

**What happened.** The `≈3×10³⁷` withdrawal entry was first written on branch `v4-query-program`
on 2026-08-23 (commit `b2b8d0da`). On 2026-08-24 the same branch reworded its *"How it was found"*
paragraph to name the reviewer explicitly (commit `4deac6a0`, *"Name Codex explicitly where its
review informed a published change"*), and that reword **replaced** three lines instead of appending
to them. The branch was not yet running this repository's documentation gates on that day, so the
append-only rule was not enforced at the time. The 2026-09-04 merge of `origin/main` into that branch
brings `b2b8d0da` into a history the gates walk, and GATE 10b correctly reports the three lines as
lost.

**What is restored.** The three lines are reproduced below **verbatim, each on its own line**, so
that every committed version of this ledger is again a subset of the working copy. They are the
opening of the original paragraph; its continuation was never removed and is unchanged. The later,
explicitly attributed wording stands beside them — under append-only, **both stay**.
...
**No figure, claim or attribution moves.** The reviewer named on 2026-08-24 (an OpenAI Codex review,
...
## 2026-09-04 — a published sentence said a named scholar mislabelled a count; he had labelled it correctly in the sentence we did not quote

**`reports/TR11_EXACT_COUNTING_BY_SYMMETRY_QUOTIENT.md` (v1.21) and `documentation/CITATIONS.md`
`#huangshisheng1997` said that Huang Shisheng 黄石声 (1997) "mislabels" 8!×8! = 1,625,702,400 as the
count of *arbitrary* arrangements of the 64 hexagrams, quoting his 「六十四卦如果任意排列」 and adding
that arbitrary arrangement of 64 objects is 64!.** The quotation stopped one sentence short. His text
(《殷都学刊》1997(2), p. 21) continues: 「只有当上下卦都定好次序，才是唯一的64种排法」 — only once the
orders of both the upper and the lower trigrams are fixed is there a single arrangement. That sentence
defines the space being counted: arrangements generated by the two trigram orderings, of which there
are exactly 8!·8!. His label is correct for the space he describes; "mislabels" was our reading of half
a passage, and it is withdrawn.

**What the corrected text keeps.** His first sentence, read alone, would suggest unrestricted
arrangement, and his later 「从六十四卦十六亿多种排法中，唯独选出」 uses the figure as rhetorical
motivation — both are now described as what they are. Two adjacent imprecisions moved with it: the
figure is reported by Huang as the argument of 沈宜甲 and 董光璧 (whose primaries this project has not
obtained), and the name "matrix-form" is Chen Zhuangwei (2007)'s gloss, not Huang's.

**What did not move.** No count, theorem or canonical value; the Luo Jianjin (2015) credit for posing
the enumeration question; the "first quantitative answers" sentence; the scope clause that closed-form
counts of simply-constrained spaces predate this suite. The v1.21 changelog row stands as the record of
what was written.

**Why it is being corrected under a publication freeze.** The freeze declared 2026-08-16 bars
assertions of novelty until named prior-art readings are complete. This edit does the opposite — it
cedes ground and withdraws a claim about another author's competence — and the operator lifted the
freeze for it explicitly on 2026-09-04. Leaving a false characterisation of a living scholar standing
because a rule aimed at over-claiming happened to catch it would have been the worse reading of that
rule.

Found when the operator asked whether the prior-art chain was sound; adjudicated from the first-hand
