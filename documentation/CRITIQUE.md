...
>
> **Note (2026-06-08): 560T canonical landed.** A new d3 560T canonical (sha `9a968fa2…`, 10,525,271,997 orderings) became the deepest published enumeration on 2026-06-08, 3.07× the 100T scale. The findings below were originally computed on the 100T canonical (`915abf30…`); the 560T `--analyze` pass (3 h 47 m on D128 with the rewrite commits 8ac5e8f/fe58e71/bf8d8a5/c0ec4c3) completed 2026-06-11. **Findings whose validity is robust under the 100T → 560T extension** (because the 100T solution set is a strict subset of the 560T set): KW's membership in the canonical, KW vs C3-ceiling, mandatory boundaries {25, 27}, partition-stability claims — all reaffirmed at 560T. **Findings that shifted at 560T**: boundary-minimum (see below), top pairwise-MI ranks, working-4-subset count. Each is flagged inline where mentioned.
>
> **Update (2026-06-11): full 560T `--analyze` complete — corrects 2026-06-09 partial reading.** `verify.py --jobs 64` PASS on all 10,525,271,997 records. The 2026-06-09 partial note read "Boundary-minimum is still ≥ 5" from §[8] = 0 working 4-subsets; that conflated two distinct minima. The complete analyze shows: **§[6] greedy-ordered minimum = 4**, set **{4, 27, 25, 21}** (boundary 4 alone eliminates 99.999% of non-KW; 27 → 481 survivors; 25 → 14; 21 → 1). **§[7] no 3-tuple works** (best 3-set leaves 15 survivors — 4-boundary minimum is proven at 560T). **§[8] = 0 unordered 4-subsets** (down from 4 at 742M and 8 at 11.2T) — at 560T no *unordered* 4-tuple of boundaries jointly reduces survivors to ≤ 1; the greedy ordered minimum works only because each boundary's effect compounds on the prior. **This makes "4-set uniquely identifies KW" a scale-bounded framing; the durable claim is "4 boundaries suffice via greedy-ordered application," reaffirmed at 560T.** §[9]: `{25, 27}` is one of the most-INFORMATIONALLY-INDEPENDENT boundary pairs (ratio 0.007), quantifying why those two keep appearing in every minimum set. §[10] top pairwise MI: pos 12 ↔ 13 = 1.3417 bits (cascade-region 11–20 dominates the top-10). §[18] boundary 4 alone yields 45.14 bits of conditional-entropy information gain — over half the 77.81-bit total. §[28] edit-distance mode is at **30** (2.79 B records = 26.5% of all canonicals). Full findings: see [SOLVE-SUMMARY.md](SOLVE-SUMMARY.md) §"560T canonical results" + [HISTORY.md](HISTORY.md) "June 10-11, 2026" entry.
...
- The complement distance analysis shows King Wen places complements significantly closer together than random (0th percentile), suggesting deliberate organization around opposition. This is a genuine finding *at this scope* (random pair-permutations as the reference). The constraint solver (`solve.py`) further shows that among all orderings satisfying Rules 1-6, King Wen's complement distance is at the **3.9th percentile** — it sits in the lower 4% of complement distances within that population. **Scope note**: this percentile claim is against Rules 1-6 (C1+C2+C5 in the new naming), *before* C3 ≤ 776 is applied. Once C3 is added — using KW's exact value of 776 as the ceiling — KW sits at the C3 maximum (~340M of 3.43B canonical orderings tie with KW at 776; minimum is 424). The "actively minimizes" framing is appropriate against the Rules 1-6 reference but NOT against the C1+C2+C3 canonical; see [SOLVE-SUMMARY.md §Rule 3](SOLVE-SUMMARY.md) for the resolved framing and §Open question 11 below for the C3-ceiling finding.
...
- **Boundary constraint claims must be scoped to the partition depth AND to "ordered" vs "unordered" framing.** The claim "2 adjacency constraints suffice" (early sample) was undersampling. The claim "4 boundaries minimum, specifically `{25,27} ∪ one-of-{2,3} ∪ one-of-{21,22}`" was d2-specific. At d3 10T, there are **8 working unordered 4-subsets**. At d3 100T no unordered 4-subset suffices; greedy-optimal was a 5-set `{1, 4, 21, 25, 27}`. **At d3 560T (current deepest, 2026-06-11): greedy-ordered minimum drops back to 4, set `{4, 27, 25, 21}` (applied in that order); unordered working-4-subset count = 0** (down from 8 at 11.2T and 4 at 742M). So the durable structural claim is the *ordered* version: 4 boundaries suffice via greedy-ordered elimination, recovered at the deepest scale. The *unordered* "exact 4-set uniquely identifies KW" framing is scale-bounded — true at 10T, false at 100T, true again at 560T (only via greedy-ordering), zero at the unordered level. Boundaries {25, 27} are present in the greedy-ordered minimum set at every partition tested (the partition-stable claim is robust).
...
- ~~No formal proof that 4 boundaries are minimum across *all* valid orderings.~~ **UPDATED 2026-06-11 with full 560T `--analyze`:** the boundary-minimum is partition + scale-dependent and **NOT monotone in scale**. Greedy-ordered minimum trajectory: **4** (d2 10T, d3 10T) → **5** (d3 100T, set {1,4,21,25,27}) → **4** (d3 560T, set {4,27,25,21}). Unordered-4-subset count trajectory: **8** (11.2T) → **4** (742M) → **0** (560T). The 100T-era expectation that the minimum would "further raise at 1000T+ enumeration" did not hold; at 3.07× the scale (100T → 560T) the *greedy-ordered* minimum dropped back to 4. The durable structural claim is "4 boundaries suffice via greedy-ordered application," reaffirmed at the current deepest scale. The minimum-unordered-set count is genuinely a function of partition depth, not a universal invariant. Boundaries {25, 27} remain partition-stable across all canonicals tested (present in the greedy minimum at every scale).
...
8. **Minimum boundary-adjacency set exhaustive minimum.** Currently: greedy-ordered 4-boundary minimum proven for d2 10T, d3 10T, and d3 560T canonical datasets; greedy-ordered minimum was 5 at d3 100T. {25, 27} mandatory in every greedy minimum set. §[7] at 560T proves NO 3-subset works (best 3-set leaves 15 survivors). Deeper enumeration at 1120T+ could in principle reveal a 3-subset that works at higher depth. If a 3-subset ever works at exhaustive enumeration, the "4-minimum" claim is falsified.
