...
>
> **Update (2026-06-09): partial 560T re-evaluation findings.** From the in-progress `solve --analyze` on the 560T canonical: `verify.py --jobs 64` (Python second-language re-verify) **PASS** on all 10,525,271,997 records. **Boundary-minimum is still ≥ 5** at 560T — section [8] of the analyze confirmed 0 working 4-subsets (same as 100T). Whether the minimum is exactly 5 or has grown to 6+ requires a later section that explicitly enumerates 5-subsets; that result is pending. New finding from §[9] (not produced by 100T-era public outputs): boundary-pair correlation structure shows `{25, 27}` is one of the most-INFORMATIONALLY-INDEPENDENT pairs (ratio 0.007), quantifying why those two boundaries keep appearing in every minimum set across all partitions tested.
...
- **Boundary constraint claims must be scoped to the partition depth.** The claim "2 adjacency constraints suffice" (early sample) was undersampling. The claim "4 boundaries minimum, specifically `{25,27} ∪ one-of-{2,3} ∪ one-of-{21,22}`" was d2-specific. At d3 10T, there are **8 working 4-subsets** with different interchangeable boundaries (from the 1-6 range). **At d3 100T (the current canonical), no 4-subset suffices: the minimum climbs to 5 boundaries, with greedy-optimal set `{1, 4, 21, 25, 27}`.** **What is stable across d2 10T, d3 10T, and d3 100T**: boundaries {25, 27} are mandatory in every working minimum set. **What is NOT stable**: both the required count (4 at 10T, 5 at 100T) and the rest of the minimum set structure. This means the headline "mandatory boundaries {25, 27}" finding IS robust; everything else is partition-scoped and continues to grow at deeper enumeration.
