...
>
> **Update (2026-06-11, corrected 2026-07-04): full 560T `--analyze` complete.** `verify.py --jobs 64` PASS on all 10,525,271,997 records. The complete analyze shows: **§[6] greedy-ordered minimum = 5**, set **{4, 27, 25, 21, 1}** (boundary 4 alone eliminates 99.999% of non-KW; 27 → 481 survivors; 25 → 14; 21 → 1; boundary 1 eliminates the last impostor, rec#330177707 — KW with the position-2/3 pair blocks swapped) — identical in membership and greedy order to 100T. **§[7] no 3-tuple works** (best 3-set leaves 15 survivors — no 3-subset isolates KW at 560T; minimum ≥ 4). **§[8] = 0 working 4-subsets** (down from 4 at 742M and 8 at 11.2T) *(742M caveat: that "4" was computed under the pre-format-v1 "survivors ≤ 4" convention, not the canonical-era "≤ 1" — directionally comparable, not convention-identical; see [BOUNDARY_MINIMUM.md](BOUNDARY_MINIMUM.md) §Result table.)* — at 560T no 4-tuple of boundaries jointly reduces survivors to ≤ 1, consistent with the §[6] minimum of 5. **This makes "4-set uniquely identifies KW" a scale-bounded framing; at canonical depth the minimum is 5.** *(Correction 2026-07-04: the 2026-06-11 version of this update reported "§[6] greedy-ordered minimum = 4" — a survivor-counting error that stopped at 1 remaining non-KW survivor instead of 0; the 2026-06-09 partial reading "boundary-minimum is still ≥ 5" had in fact been right. See [BOUNDARY_MINIMUM.md](BOUNDARY_MINIMUM.md).)* §[9]: `{25, 27}` is one of the most-INFORMATIONALLY-INDEPENDENT boundary pairs (ratio 0.007), quantifying why those two keep appearing in every minimum set. §[10] top pairwise MI: pos 12 ↔ 13 = 1.3417 bits (cascade-region 11–20 dominates the top-10). §[18] boundary 4 alone yields 45.14 bits of conditional-entropy information gain against the 77.81-bit sum of per-position marginals. ⚠ **`77.81` is a SUM OF PER-POSITION MARGINALS — `sum_p H(pair at p)`, as `solve.c:45449` states — NOT a total or joint entropy.** The joint entropy over these records is bounded by log₂(10,525,271,997) = **33.29 bits**, so 77.81 exceeds its own ceiling by 2.34× and boundary 4's 45.14 exceeds it by 1.36×. The numbers are correct; the earlier wording called it a *total*, and called boundary 4's share more than half of one — both describing a quantity this is not. Corrected 2026-09-07. §[28] edit-distance mode is at **30** (2.79 B records = 26.5% of all canonicals). Full findings: see [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) §"560T canonical results" + [HISTORY.md](HISTORY.md) "June 10-11, 2026" entry.
...
- **Structured-permutation null models (comprehensively addressed 2026-04-19).** Seven null-model families are now tested via `solve.c --null-debruijn-exact`, `--null-gray`, `--null-latin`, `--null-lex`, `--null-historical`, `--null-random`, `--null-pair-constrained`, plus a sampled counterpart in `solve.py --null-debruijn`:
  ...
  **What this establishes:**
  ...
  - **C1 is astronomically KW-specific.** Zero of **2,760,021,104** permutations sampled across six unconditional families satisfy C1, consistent with the theoretical rate of ~10^-44. *(Total corrected 2026-09-02, prose batch P43: this read 1.86 billion, the roster total from when the random-permutation row was 10⁸. Sum of the six unconditional rows of the table above: 134,217,728 + 256 + 100,000 + 1,625,702,400 + 720 + 10⁹; the historical point-tests and the pair-constrained row are excluded. The 2026-08-30 aggregate correction landed at [SOLVE.md](SOLVE.md) and [GUIDE.md](GUIDE.md) and reached neither this page nor [PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) nor [SOLVE_SUMMARY.md](SOLVE_SUMMARY.md).)* For de Bruijn and Gray code families the 0% result is not just empirical — it is provable (see §C1 impossibility below).
  ...
  - **C3 concentration varies by family.** Random: 0.003%. de Bruijn: 0.18% (~65× random). Latin-square: 6.67% (~2,350× random). Pair-constrained (C1): 6.42% (exact: 6.4211367496%, `verify.py --check-null-g --unpinned`). Gray: 0% *of this sampler's 10⁵ draws*, the smallest C3 observed among them being 832 > 776. ⚠ **[SCOPE CORRECTED 2026-09-02 (prose lane, `RP-84738f4b`) — this sentence went on to assert, as an empirical universal over the Gray-code family, that King Wen's complement distance is unbeaten by any Gray code. That does not follow from the measurement and is withdrawn: 832 is a *sample* minimum over 10⁵ draws from a non-uniform sampler (the :60 marker above cites `solve.c`'s own "Non-uniform sampler" print), so it is an **upper** bound on the family minimum, and the draws cover ~10⁻¹⁷ of the ~10²² 6-bit Gray codes. What survives is exactly the scoped rate statement the :60 marker already gives: 0 of 10⁵ sampled Gray codes reach C3 ≤ 776, rule-of-three ≤ 3×10⁻⁵ on *that sampler's* rate. Whether any 6-bit Gray code has C3 < 776 is **open**; see CORRECTIONS.md 2026-09-02.]** The pair-constrained rate being similar to Latin-square is suggestive — both impose strong structural symmetry on complement placement.
  ...
  **Costas arrays at order 64 — closed, negatively, without enumeration.** Order-64 Costas arrays exist by a standard construction, and none of them can satisfy C1. *Existence:* the Golomb construction G₂ over GF(67) with primitive elements α = 7 and β = 61 gives an order-65 Costas array. Because α + β = 68 ≡ 1 (mod 67) it has a dot at the corner (1, 1), and deleting that row and column (the G₃ variant) leaves an order-64 Costas array. *Exclusion by C1:* read as a hexagram sequence under the binary value convention every family on this page uses, a C1 sequence puts 1 next to 32, 13 next to 44, 19 next to 50 and 31 next to 62. Each is a reversal pair whose values differ by exactly 31, so the sequence has four displacement vectors drawn from the two-element set {(1, +31), (1, −31)}. Two of them coincide, and the Costas distinct-difference property fails. No enumeration and no database is needed. Executed 2026-09-25: the G₃ array is a permutation of 0..63 with all C(64, 2) = 2,016 displacement vectors distinct, it fails C1, and King Wen (which satisfies C1) carries the vectors (1, −31), (1, +31), (1, −31), (1, +31) at those four pairs. ⚠ **[CORRECTED 2026-09-25 (Q-758 / Q-700, Codex V3A-015#5 and V3A-056#3) — this paragraph was headed "Remaining gap" and read: "Costas arrays at order 64 (uncertain existence via standard Welch/Lempel–Golomb constructions; full 64! enumeration is infeasible at ~10^89 candidates). Costas at order 64 is the last open 'structured permutation' family within reasonable scope; testing it would require either obtaining a published database of order-64 Costas arrays or implementing sporadic constructions. Deferred." Existence was never uncertain: G₃ at q = 67 is a standard variant of the Lempel–Golomb construction, which the sentence itself named. The test it deferred needed two lines of arithmetic, not a database or a sporadic construction. The 64! figure was accurate but irrelevant.]**
...
- ~~No formal proof that 4 boundaries are minimum across *all* valid orderings.~~ **UPDATED 2026-06-11; corrected 2026-07-04:** the boundary-minimum is **monotone non-decreasing in scale**. Greedy-ordered minimum trajectory: **4** (d2 10T, d3 10T) → **5** (d3 100T, set {1,4,21,25,27}) → **5** (d3 560T, identical set {1,4,21,25,27}). Working-4-subset count trajectory: **8** (11.2T) → **4** (742M) → **0** (100T, 560T). *(742M caveat: that "4" was computed under the pre-format-v1 "survivors ≤ 4" convention, not the canonical-era "≤ 1" — directionally comparable, not convention-identical; see [BOUNDARY_MINIMUM.md](BOUNDARY_MINIMUM.md) §Result table.)* *(The 2026-06-11 version of this entry claimed the minimum "dropped back to 4" at 560T — a survivor-counting error; see [BOUNDARY_MINIMUM.md](BOUNDARY_MINIMUM.md).)* The 100T-era expectation that the minimum would "further raise at 1000T+ enumeration" is so far neither confirmed nor refuted: one increment (10T → 100T), then stability (100T → 560T). The minimum-set count is genuinely a function of enumeration depth, not a universal invariant. Boundaries {25, 27} remain partition-stable across all canonicals tested (present in the greedy minimum at every scale).
...
1. ~~**Costas arrays at order 64.**~~ **RESOLVED 2026-09-25 — the family exists and C1 excludes all of it.** The Golomb G₃ construction at q = 67 gives order-64 Costas arrays. No C1 sequence is a Costas array, because the four reversal pairs 1↔32, 13↔44, 19↔50 and 31↔62 each contribute a displacement vector (1, ±31), and two of the four must coincide. Proof and execution record: §"Missing analyses" above, the paragraph headed "Costas arrays at order 64". ⚠ **[CORRECTED 2026-09-25 (Q-758, Codex V3A-015#5) — this item read: "Standard Welch/Lempel–Golomb constructions give adjacent orders 62 and 66; a direct order-64 family is not known to the author", and it proposed a literature survey, a C1/C2/C3 test of each array, and sporadic or computer-search constructions. The Golomb family covers order 64 directly (q − 3 with q = 67 prime), and the C1 test settles every array at once, so none of the three steps is needed.]**
...
**Results (2026-07-04, tier-1, 2×10⁹ probes — reported in full as pre-committed):**
*(**"tier-1" is the scoring-axis class** — principled rules stated ahead of the data, as against
the "tier-2" data-like rules and the tier-2 escalation band below. It is a property of the *axes*,
not of the run's scale: this is a 2×10⁹-probe estimator run, **not** the campaign "Tier 1" 11.2T
canonical of [LARGE_SCALE_CAMPAIGNS.md](LARGE_SCALE_CAMPAIGNS.md) and not the "Tier 1"
determinism-hardening level of [CANONICAL_HASHES.md](CANONICAL_HASHES.md). The `*_tier1.out`
evidence filenames throughout this section carry this sense.)* all 13 functionals
