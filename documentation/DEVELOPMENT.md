...
>    (`#include <zlib.h>` at `solve.c:330`) without a manual step. Fixed
...
(in `scripts/pre_push_gate.sh`) decides per-sha whether the pushed range touches
`roae.py` or `example/`, and if so runs `doc_gates.sh generated` in the pushed sha's
own worktree (the `case " $GENSHAS " in` block of the per-sha worktree loop, the one
that calls `bash scripts/doc_gates.sh generated`; the per-ref range loop has a
same-shaped block that only records the decision). *(Line citations 125-132 /
219-228 into that file removed 2026-09-24, Q-710: that file moves, so it is cited by name.)* It is
**required on four paths, not merely "when there is a
...
| `PRECOMMIT_STAMP_SELFTEST=` | `PASS` · `FAIL` — emitted by `scripts/pre_commit_stamp_gate.sh --selftest` only, never on the commit path. The selftest drives the stamp comparison over fixtures, so it can prove the comparison is correct but cannot prove it is REACHED; that second half is what `roae-private/scripts/q673_commit_stamp_redtest.sh` exists for. |
| `TR12_STAMP_REFUSED=` | `INPUT-CHANGED-MID-RUN` · `NEWER-STAMP-PRESENT` · `ERROR-CANNOT-MEASURE` — emitted by `scripts/tr12_repro_gate.sh --stamp` when it declines to write a stamp. Before this existed, `--stamp` recomputed the fingerprint and wrote it with no comparison and no refusal, so a tree that changed mid-run was stamped as though it had reproduced. `INPUT-CHANGED-MID-RUN` **names the file** that moved; `ERROR-CANNOT-MEASURE` is emitted when the closure is empty or unreadable, never silently treated as zero. |
...
| `DOC_GATE_EMITTED_SURFACE_DROPPED` | emitted lines rejected as shell-fragment generation rather than verdicts (`echo 'WORK=$(mktemp -d); RAW=...'`), plus (Q-795) any emitting line that ends in the comment `# not-a-verdict` — the declared marker for a harness assignment such as `echo "Q7WIT_DIR=$d"  # not-a-verdict`. The marker is the author's claim, not a proof, so it belongs only on lines no consumer greps as a verdict |
...
| `CITATION_LINE_GATE` | `citation_line_gate.sh` | `PASS` \| `FAIL` \| `ERROR`. For every `solve.c:N` citation in `SOLVE_C_CLI.md`, mines the citing line — or, when that yields nothing, the nearest preceding heading — for distinctive identifiers, and requires at least one to occur inside the cited span. A **ratchet**, not an absolute: `FAIL` when the stale count rises above the pinned budget **or** a new stale citation key appears; `ERROR` when the document yields no citation at all or none that is checkable. Both directions are red-tested on a fixture pair differing by one digit of one citation. **`--all-files`** (Q-786, 2026-09-25) widens the population from `SOLVE_C_CLI.md` to every tracked text file, because one solve.c insertion (batch 8, +1,193 lines) had silently staled 56 citations in 21 other files. Two legs. **Leg A (shift)**: from `git diff -U0 BASE -- solve.c` (`--base REF`, else `$CITGATE_BASE`, else `HEAD`) it builds the old→new line map and `FAIL`s every `solve.c:N` / continuation `:N` whose citing line the change left byte-identical while N moved, printing `file:line` and `map(N)`; content-free, so it also covers citations no anchor can check, and vacuous on a clean committed tree against `HEAD`. **At pre-push the base is the pushed range's** (Q-792, 2026-09-25): `pre_push_gate.sh` runs `doc_gates.sh all` in a worktree of the pushed sha, where `HEAD` is the pushed sha, so it sets `CITGATE_BASE` to the remote sha being pushed over, or, for a new branch or an unfetched remote tip, to the nearest merge-base with an `origin/*` ref. When neither exists it prints that leg A is vacuous for that sha and does not block. **Leg B (anchor)**: the check above over every tracked `*.md` / `*.py` / `*.sh`, anchors mined from the citing line's paragraph (plus the section heading in markdown) and required within `[N-2, M+2]`; stale citations are pinned per file by key, each with a reason, and the pin is exact both ways (a pinned citation that stops being stale is a `FAIL` until the pin is removed). Shared exclusions, counted and printed by class: `CORRECTIONS.md`, `HISTORY.md`, `CORRECTIONS_INVENTORY.tsv`, `reports/evidence/`; Codex finding ids; revision pins (`at <sha>`, `<sha>:solve.c`, `git show`); `@`-glued citations; dated revision rows. **`--all-files --all-targets`** (Q-791, 2026-09-25) applies both legs to `<file>:N` citations into **every** tracked file, not only `solve.c`: targets resolve against `git ls-files`; markdown-link (`[FILE.md]` `(FILE.md):N`) and backticked-name forms count; a bare `(:N)` (or `at :N` / `see :N`) is read as the file named earlier on its line, else the last `<file>:N` in its paragraph, else the citing file itself (self-reference); revision pins are per citation, and `<file>:N@<sha>` is the form for a citation that records a past tree; Codex batch locators are excluded; its pins are keyed by citing file, target and anchor. `doc_gates.sh citation-lines` runs the document mode, `--all-files`, `--all-files --all-targets` and `--selftest`, each judged on its own token. **Leg A2, `.tsv`, the `.sh` rarity rule and content-hashed pins** (the batch 7–10 pre-publication review, S1/S3, and Q-793, 2026-09-25). A citation repinned and then re-staled inside one push range was invisible: leg A reads only byte-identical citing lines, leg B read no `.tsv`, a common `.sh` word could land in the ±2 window by chance, and pins were keyed by name. **Leg A2** takes every citation on a line the range *edited*, pairs it with the same target's citations on that line's counterpart at BASE (the most similar cited line of its diff hunk), and `FAIL`s it as `REPIN` unless the new number is `map(old)`, or the old cited lines moved there verbatim, or it lands under leg B, or a content-hashed pin covers it. **Leg B reads `*.tsv`**, mining the note cell that carries the citation. **For `.sh` targets**, an anchor that occurs on more than 3 lines of the script must be on the cited span itself; only a rare anchor keeps the ±2 slack. **Every pin carries a 12-hex sha256 of its cited lines** (`file TAB [target TAB] key TAB hash TAB reason`). A pinned citation whose cited content changes for any reason stops matching its pin, and the gate prints `PINNED CONTENT CHANGED`. Key `-` is an *attested* pin: an unanchored citation that a person checked by content. It is exact like the others. `--base` also accepts a tree (`git write-tree`). Measured on the batch 1–10 tree against `5c296837`: leg A2 examined 417 edited citations and flagged 70. 43 of the 70 were already pinned leg-B rows. 2 were real: historical `build_pgo.sh` lines without a revision pin, now fixed. 25 were correct repins with no anchor; these are the leg's false positives, each checked by content and attested. The rarity rule cut chance landings of deliberately shifted `.sh` citations (±7, ±50, ±200 lines) from 26 of 180 to 15 of 180. It found one real stale citation and costs two pinned false positives. `CITGATE_SH_TIGHT=0` and `CITGATE_PROBE_SHIFT=k` reproduce that measurement. Leg A2 cannot see a citation that was already wrong at BASE and still maps correctly (the review's S2); only an anchor can |
...
| `DOC_GATES_SELFTEST` | `doc_gates.sh --selftest` | `PASS` \| `FAIL`, whole-line, printed once at the end of the mutation-testing run (never by `all` or by any named gate). `PASS` means every fire-proof and every stays-clean control in the `--selftest` region reported `[ok]`; a single `[FAIL]` — including a scratch fixture whose setup broke, which is reported as a failed leg (only a fixture whose stated premise does not hold, or no tmpdir, prints `[SKIP]`) — prints `FAIL`. The run refuses a dirty tree, a concurrent `--selftest` (lock) and a nested one (depth guard) with rc 2 and **no token**, so the absence of the line means the self-test did not run, never that it passed. Added 2026-09-24 (Q-703 batch); registered here because it is a `grep -qx` contract, not because a hook reads it yet — wiring it into pre-push is Q-720 |
| `GATE25_POPULATION_FROM_DOCS` | `doc_gates.sh repro-reach` (GATE 25, also inside `all`) | the integer count of tracked markdown files GATE 25 scanned for reproduction commands — the whole `$DOCS` population (`git ls-files '*.md'`, 110 at 5c296837 + batch 2), not the hand-written glob list that never read `viz/` before Q-703 (2026-09-24). Printed on every run before the verdict lines; a count below the number of tracked `*.md` files means the population shrank, which the `--selftest` control pins at `=2` on a two-file fixture |
...
| `TR12_REPRO_GOLDEN_STATE` | `tr12_repro.sh` | `DIFFED` \| `MINTED-UNVERIFIED`, whole-line into `VERDICTS.txt`. `DIFFED` means every reproduced row was compared against an expected block that already existed; `MINTED-UNVERIFIED` means at least one was **written by this run** under `--mint-missing` and therefore diffed against nothing. It is the machine-readable companion to the **value** of `TR12_REPRO` itself, which since 2026-09-13 is `PASS` when nothing was minted and `PASS:MINTED-<n>` when `<n>` rows were — so a consumer greping `-qx 'TR12_REPRO=PASS'` fails closed on a minted run instead of reading it as a reproduction. Same cure, one level up, as `TR12_N31_GOLDEN`'s `OK` \| `MINTED-UNVERIFIED` split. `FAIL` is never qualified |
...
| `N3_MEASURED_NULLS_GATE` | Q1c and Q10a's KW-orbit-rank leg were published as **hand-written absence claims** — `SKIP:merged-into-Q4AC`, and the sentence "(iv) KW-orbit-rank: DROPPED" inside a golden. Both were true **at n=31 (and not at n=9)** and neither was checked (⚠ *scoped 2026-09-24, Q-717: the n=31 receipt measures both EMPTY, but at n=9 the KW-orbit-rank leg is NONVACUOUS — `scripts/tr12_expected/n9/c_q10a_kwrank.txt` has `q10a_rank3 13056`, not 0*), so from outside "we computed it and the answer is nothing" was indistinguishable from "we never ran it". Each is now produced by a function that can FAIL, and this gate extracts both verbatim and runs them against certificates the live battery will never see: a rank-0 anchor (real `--kc-o3-unrank 0` output), two engine sources disagreeing, a bracket contradicting its own rank, a certificate that has grown a class-rank field. 11 pinned verdicts — 2 EMPTY, 2 refusals, 1 COMPUTABLE, 6 ERROR — and 9 mutants, every one killed. n=9 can only ever exercise the *refusal* direction, which is exactly why the EMPTY direction needs a red-test rather than a run |
...
- **Independent verifier**: `verify.py` (repo root) is a pure-Python implementation
  ...
  **C1 (pair structure), C2 (no 5-line transitions), C3 (complement
  ...
  refuting the claim. (The adjudication cited HISTORY.md lines 1590-1593 for this;
...
- **Enumeration and merge have very different resource profiles — run them
  on separate VMs.** Enumeration is core-bound; its RAM floor is the per-thread
  hash tables, which the solver prints at start (the archived 10T run:
  `Hash table: 16777216 slots (2^24) x 32 bytes = 512 MB per thread` and
  `Initial hash memory: 32768 MB (64 threads x 512 MB, auto-resize at 75%)`).
  The in-memory merge is RAM-bound: it allocates every **pre-dedup** shard record
  at once (`malloc(total_records × 32)`), ≈443 GB for the 13,832,832,979 input
  records of d3 100T, and auto mode switches to the external merge
  (`SOLVE_MERGE_MODE=external`) when that exceeds 80% of RAM.
  ⚠ **[CORRECTED 2026-09-24 (Q-762, CX-82) — this read "64 pthreads, ~10 GB RAM
  flat" for enumeration and sized the merge as `malloc(unique_records × 32)`, with
  "≥128 GB RAM" at 100T. The solver's own log gives 32,768 MB of initial hash memory
  at 64 threads, and the merge buffer is sized from all input records, not the
  deduplicated count.]**
  Splitting the phases keeps enumeration on a 128-core D-als-v7 Spot VM
  (`D128als_v7`) and runs the merge on a right-sized, on-demand D-als-v7 VM
  (`D16als_v7`–`D64als_v7`) with `SOLVE_MERGE_MODE=external`, sized by RAM and
  I/O rather than core count, because the merge uses one or two cores.
  ⚠ **[CORRECTED 2026-09-24 (Q-779) — this read "keeps enumeration on a lean
  F-series SKU and only pays for a memory-dense E/M-series VM during the brief
  merge". This project does not use F-series VMs; it retired them on 2026-04-19
  for the D-als-v7 family (`CLAUDE.md`). It does not size merges on E/M-series
  memory-dense VMs either: the external merge streams in chunks, so a D-als-v7
  VM is enough.]** See
...
- **Sub_*.bin integrity check on eviction resume.** After spot eviction
  and redeploy, the monitor checks every existing sub_*.bin with `gzip -t`
  and checks that its **decompressed** length (`gzip -dc sub_X.bin | wc -c`)
  is a multiple of 32; a `SOLVE_COMPRESS=0` raw shard is checked on its file
  size. Files that fail (eviction killed the flush mid-write before fsync) are
  removed so the solver re-runs those sub-branches from checkpoint rather than
  merging corrupt data. ⚠ **[CORRECTED 2026-09-24 (Q-762, CX-82) — this check
  read "`size % 32 == 0`" on the file itself. Shards have been gzip-framed by
  default since #169, and `solve.c` retired exactly that byte check when it
  moved to compressed shards, so the old rule deleted valid shards.]**
...
`--sub-branch` targets exactly one. It bypasses checkpoint.txt loading, but
that does **not** make it a fresh run: the parallel path calls `sub_ckpt_load`
unconditionally, so any `sub_ckpt_*` state left in the working directory is
restored. Run each experiment in a fresh directory. ⚠ **[CORRECTED 2026-09-24
(Q-762, CX-82) — this said the checkpoint.txt bypass guarantees a fresh run with no
accidental resume from stale state; `sub_ckpt_load` makes that false.]**
...
(`perf_bench.sh:158-159`) rather than joining `claude-vnet`, so no private
path from the orchestrator exists at all, and then creates the VM with
`--public-ip-sku Standard --nsg-rule SSH` (`:163`) and connects with
`StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null` (`:172-173`).
...
teardown that closes the window is `az group delete` (`:141-145`), called
explicitly at four sites (`:176`, `:261`, `:320`, `:540`) with **no `trap`** — so an
...
Queued, not scoped. ⚠ *Corrected 2026-09-24 (Q-779): this also read "possibly
M-series VM for analysis step". Memory-dense E/M-series VMs are outside this
project's VM guidance (D-als-v7 family, [DEPLOYMENT.md §Two-phase deployment](DEPLOYMENT.md#two-phase-deployment-enumeration-vm-vs-merge-vm)),
so an analysis step that needs more RAM than a D-als-v7 VM has would have to be
redesigned to stream, not moved to a larger SKU.*
...
5. **~~Null-model comparison against structured permutations~~ — DONE
   ...
   the seven families. They need no sampling: order-64 Costas arrays exist
   (Golomb G₃ at q = 67), and C1 excludes every one of them analytically
   ([CRITIQUE.md](CRITIQUE.md) §"Missing analyses"). ⚠ **[CORRECTED
   2026-09-25 (Q-758) — this read "Costas arrays specifically were not among
...
6. **~~Partition-stability re-check on 100T data.~~ — DONE (measured at
   ...
   SOLVE.md:389 and :603, SOLVE_SUMMARY.md:221, LEADERBOARD.md:186-189 — and
