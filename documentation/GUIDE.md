...
Only Position 1 is universally locked. The number of boundary constraints needed to uniquely identify KW **is partition + scale-dependent and non-monotone with scale**: greedy-ordered minimum is 4 at d2/d3 10T, **5 at d3 100T** (greedy-optimal `{1, 4, 21, 25, 27}`), and **4 again at d3 560T** (greedy-optimal `{4, 27, 25, 21}` applied in order — the 2026-06-11 analyze). The *unordered* working-4-set count is also scale-bounded (8 at 11.2T, 4 at 742M, 0 at 560T) — at the deepest scale no unordered 4-tuple of boundaries jointly identifies KW, but the *greedy-ordered* application still works at 4 boundaries. Durable claim: "**4 boundaries suffice via greedy-ordered application**," reaffirmed at the deepest scale. Boundaries **{25, 27} remain in every greedy minimum at all four partitions tested** — the single most stable structural finding. See [SOLVE-SUMMARY.md](SOLVE-SUMMARY.md) §"560T canonical results" and [SOLVE.md](SOLVE.md) §Boundary analysis for the full story.
