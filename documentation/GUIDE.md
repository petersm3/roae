...
| XOR algebraic regularity (7 products) | Theorem (universal **within C1**) | N/A — for any **reverse/inverse** pairing every product lies in the 7-set (containment is universal); that all seven are *attained* is a fact about C1's pairing, not about every such pairing — the complement-only matching yields {111111} alone, and 729 of the 4,096 reverse/complement perfect matchings attain all seven *(corrected 2026-09-21, Q-630: the previous cell read "true for any reverse/inverse pairing", which is right for containment and wrong for attainment; see [SOLVE.md](SOLVE.md) §Theorem 2)* *(corrected 2026-08-30: the unqualified "any pairing" is false — the perfect matching (0,1), (2,3), …, (62,63) uses every hexagram once and yields the single product 000001, which is not among the 7. That matching is not a reverse/inverse pairing, so it is outside the theorem's scope rather than a counterexample to it; see [SPECIFICATION.md](SPECIFICATION.md) §Theorem (XOR universality) and [SOLVE.md](SOLVE.md) §Theorem 2)* |
...
The constraint solver (`solve.c`) goes further: 5 rules narrow 10^89 possibilities to an estimated ≈1.33×10³⁸ valid orderings (the counts below are *budget slices* of that space, not the space itself). Most of that funnel is elementary rather than surprising: roughly **44 of the ~51 orders of magnitude come from C1 alone** (the pair-structure skeleton, 64! → 32!·2³²), and only about 7 from the constrained measurement (C2–C5). Canonical counts:
