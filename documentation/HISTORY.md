...
**Phantom 11.2T anchor-drift incident (2026-05-31 ~09:00 → ~13:30 UTC).** Stage 2's sha-gate exit code was rc=22 ("sha mismatch"), even though the produced sha matched the canonical. The cause was a typo in the hardcoded `ANCHOR_11_2T_SHA` value in the dress rehearsal scripts: `0c0fe37cdf3d92ba953b3c41a5e84d54c1f88b22e7d1e0e3e9a52deb8a3ef6c5` — which, as discovered ~4 hours later via empirical sha256 of two independent archived `solutions.bin` files on solver-data, corresponds to nothing real. It originated as the example sha in a `solve --validate-canonical <sha>` usage example in a roae-private session doc on 2026-05-28; the full sha after the `0c0fe37c` prefix was typed without verifying against `CANONICAL_HASHES.md`. The wrong value then copy-pasted into three dress rehearsal scripts on 2026-05-30 and 2026-05-31. The first 8 hex chars (`0c0fe37c`) matched the real anchor's abbreviated header convention used throughout the project, so the wrong full sha looked right on cursory review. No sha-equality gate had ever fired against the typo'd value until the dress rehearsal Stage 2 gate, because all earlier validation work (Tier 1 1T sha-gate, #100 11.2T sha-check, #114 100T sha-gate) used independently sourced sha values from `solutions.sha256` sidecar files or `CANONICAL_HASHES.md` directly. The incident cost ~6 hours of investigation work (briefly declared a 560T launch blocker, drafted a 5-phase investigation plan), and was resolved when an empirical sha256 of `solver-data:/t62_dress_11p2T/solutions.bin` (an unrelated archive from the 2026-05-28 t62 dress rehearsal on 560T hardware) produced `0c0fe37cf449cbc6e275...`, identical to the dress rehearsal v2 sha and to `CANONICAL_HASHES.md`. **Lesson:** never hardcode a 64-character sha from another doc's usage example; source canonical shas from the artifact's `.sha256` sidecar, a fresh `sha256sum`, or `CANONICAL_HASHES.md`. A `roae-private/PHANTOM_DRIFT_RESOLUTION_2026_05_31.md` writeup records the full lifecycle.
