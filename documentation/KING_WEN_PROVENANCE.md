...
| **序卦傳** | classical commentary | Commentarial rationales *about* the received order — it narrates the received sequence transition by transition, so it is not a competing order. |
| **雜卦傳** | classical commentary, but **not** merely commentary on the order | ⚠ **[RE-SCOPED 2026-09-03.** This row previously read, jointly with 序卦傳, *"not competing orders"*, and for the 雜卦傳 that is wrong: it presents the 64 in an arrangement that **differs from the received sequence**, and it is not generated by any stated rule — unlike every other entry in this table, which is dismissed precisely because a rule generates it. It is reported to largely respect the same pairing structure (C1), which would make it an **attested, non-algorithmic alternative 64-ordering** — i.e. a candidate **corpus control**, which is exactly what the standing single-sequence objection asks for. Identified by Codex, endorsed by Fable on review; the C1-respecting characterisation is theirs and is **not independently re-derived here**. 🔴 Using it as a control is NOT done and is not claimed: it requires fixing its order to a specific recension first, which is unstarted work.]** |
