...
1. **"The absence of 5 is extraordinary / unique to King Wen"** — McKenna treated the no-5-line-transition property as a smoking gun of deep intentional design. Two separate ROAE findings challenge the "extraordinary / unique" framing:
   ...
   (b) **Not unique among historical orderings**: `solve.c --null-historical` tests 4 documented hexagram orderings. **Two of four satisfy C2 exactly** — King Wen and Jing Fang's 8 Palaces arrangement (c. 77-37 BCE). The Mawangdui silk-text ordering (168 BCE) comes close but fails: it has exactly one 5-line transition, at its Kan→Zhen octet seam; Fu Xi (Leibniz's natural-binary, not a traditional divinatory ordering) has two. *(Corrected 2026-07-05: earlier text claimed 3 of 4 and a "shared classical Chinese design principle" — computed on an erroneous Mawangdui array, now withdrawn; see CITATIONS.md errata.)* McKenna's framing of C2 as a KW fingerprint still overreaches (Jing Fang shares it, and within reverse/inverse pairs 5-line transitions are impossible), but the pan-tradition inference is no longer supported. See [CITATIONS.md](CITATIONS.md) for the McKenna and Cook 2006 literature credits.
