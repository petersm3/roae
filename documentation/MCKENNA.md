...
- The complement distance is genuinely unusual — complements are placed significantly closer than random (0th percentile), suggesting deliberate organization around opposition. `solve.c --null-gray-random` (10⁵ samples) additionally shows that **no random 6-bit Gray code achieves KW's C3 ≤ 776**: the minimum observed across 10⁵ Gray codes is 832, strictly greater than KW's 776. Even a structured permutation family explicitly optimized for adjacency (Hamming-1 transitions) cannot beat KW on complement placement.
