...
McKenna's observation was empirical; ROAE supplies the proof and confirms the result at full canonical scale: **100.000000% of all 10,525,271,997 records in the d3 560T canonical (`9a968fa2…`, current deepest) have odd wrap-around** — exactly as the theorem requires, and necessarily so, since `solve --verify` enforces C4 + C5 on every record and the parity follows from them. Which odd value the wrap-around takes (d=1 vs d=3 — the only options once C2 forbids 5) is budget-dependent, but the *parity* is invariant.
