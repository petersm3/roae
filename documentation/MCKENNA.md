...
- The constraint solver (`solve.c`) finds that 5 rules narrow 10^89 possible orderings to billions. Canonical counts:
  ...
  All are partial enumerations at different partition strategies + node budgets; under true exhaustive enumeration they would converge. Only Position 1 is universally locked. The **number of boundary constraints needed to uniquely identify KW grows with partition depth: 4 at 10T (both d2 and d3), 5 at 100T d3** (greedy-optimal {1, 4, 21, 25, 27}); the 560 T re-evaluation may grow it further toward 6. Boundaries `{25, 27}` are **partition-stable across all partitions tested to date** (mandatory at d2 10T, d3 10T, AND d3 100T); the 560 T finding is queued. Other boundaries in the minimum set are partition-dependent. See [SOLVE.md](SOLVE.md) §Boundary analysis for the full analysis.
