...
McKenna's observation was empirical; ROAE supplies the proof and confirms the result at full canonical scale: **100.000000% of all 10,525,271,997 records in the d3 560T canonical (`9a968fa2…`, current deepest) have odd wrap-around** — exactly as the theorem requires, and necessarily so, since `solve --verify` enforces C4 + C5 on every record and the parity follows from them. Which odd value the wrap-around takes (d ∈ {1, 3, 5} — parity forces odd; note C2 constrains the 63 *linear* transitions, not the wrap: C1–C5-valid orderings with a 5-line wrap exist, SAT-decided 2026-07-03, see CIRCULAR_KING_WEN.md) is budget-dependent: at the 560T canonical `solve --verify-wrap-parity` measures **91.83% d=3 / 8.17% d=1 / zero d=5** (9,665,706,017 vs 859,565,980 records), with the d=1 minority growing with node budget. The *parity* itself is invariant.
...
1. **"The absence of 5 is extraordinary / unique to King Wen"** — McKenna treated the no-5-line-transition property as a smoking gun of deep intentional design. Two separate ROAE findings challenge the "extraordinary / unique" framing:
   ...
   (b) **Not unique among historical orderings**: `solve.c --null-historical` tests 4 documented hexagram orderings. **Three of four satisfy C2 exactly** — King Wen, the Mawangdui silk-text ordering (168 BCE), and Jing Fang's 8 Palaces arrangement (c. 77-37 BCE). Only Fu Xi (Leibniz's natural-binary, not a traditional divinatory ordering) has 5-line transitions. This strongly suggests **C2 was a shared classical Chinese design principle** rather than a King-Wen-unique phenomenon. McKenna's framing of C2 as a KW fingerprint overreaches; his framing of C2 as evidence of *some* intentional design in Chinese I Ching scholarship holds up. See [CITATIONS.md](CITATIONS.md) for the McKenna and Cook 2006 literature credits and the Jing Fang / Mawangdui comparative finding.
...
- **~~The sequence is unique among historical orderings~~** [**SUPERSEDED 2026-04-19 by more detailed testing**] — Earlier ROAE output (`--sequences` in roae.py) compared King Wen against Fu Xi and Mawangdui orderings. `solve.c --null-historical` now tests **four** historical orderings (Fu Xi, King Wen, Mawangdui, Jing Fang's 8 Palaces). The corrected finding: **3 of 4 ancient orderings (KW, Mawangdui, Jing Fang) satisfy C2 exactly** — the no-5-line-transition property is NOT specific to King Wen but is shared across multiple classical traditions. C1 (pair structure) and C3 (complement distance ≤ 776) remain King-Wen-specific within the tested historical set: Mawangdui and Jing Fang have their own pair geometries and complement-distance profiles, but not KW's specific ones.
...
- The no-5 property is real but largely explained by the pair structure: `solve.c --null-pair-constrained` (10⁹ Monte Carlo samples) gives C2|C1 = **4.29%** — a 23.5× multiplier over the unconditional 0.18% rate. Within reverse/inverse pairs, 5-line transitions are mathematically impossible (distances are always even or 6), so all C2 work happens at the 31 between-pair boundaries. In addition, as of 2026-04-19 analysis, C2 is **not unique to King Wen** — Mawangdui and Jing Fang 8 Palaces also satisfy it.
...
- The constraint solver (`solve.c`) finds that 5 rules narrow 10^89 possible orderings to billions. Canonical counts:
  ...
  All are partial enumerations at different partition strategies + node budgets; under true exhaustive enumeration they would converge. Only Position 1 is universally locked. The **number of boundary constraints needed to uniquely identify KW is partition + scale-dependent and non-monotone with scale**: greedy-ordered minimum is 4 at d2/d3 10T, **5 at d3 100T** (greedy-optimal `{1, 4, 21, 25, 27}`), and **4 again at d3 560T** (greedy-optimal `{4, 27, 25, 21}` applied in order — the 2026-06-11 analyze). The *unordered* working-4-set count is scale-bounded (8 → 0 across 11.2T → 560T). Durable claim: "4 boundaries suffice via greedy-ordered application." Boundaries `{25, 27}` are **partition-stable across all four partitions tested to date** (in every greedy minimum at d2 10T, d3 10T, d3 100T, AND d3 560T). Other boundaries in the minimum set are partition + scale-dependent. See [SOLVE-SUMMARY.md](SOLVE-SUMMARY.md) §"560T canonical results" + [SOLVE.md](SOLVE.md) §Boundary analysis for the full analysis.
