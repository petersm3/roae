...
- **Complements are unusually close** — `--complements` shows King Wen places complementary hexagrams significantly closer together than random (0th percentile): a strong structural regularity in how opposites are placed.
...
- The complement distance is genuinely unusual — complements are placed significantly closer than random (0th percentile), a strong regularity in how opposites are placed. `solve.c --null-gray-random` (10⁵ samples) additionally shows that **no random 6-bit Gray code achieves KW's C3 ≤ 776**: the minimum observed across 10⁵ Gray codes is 832, strictly greater than KW's 776. Even a structured permutation family explicitly optimized for adjacency (Hamming-1 transitions) cannot beat KW on complement placement.
...
- The sequence, arranged by people in antiquity (traditionally attributed to [~1000 BCE](https://en.wikipedia.org/wiki/King_Wen_of_Zhou), though modern scholarship is divided on the exact origin and dating), carries mathematical structure that is vanishingly unlikely by chance — zero random permutations satisfy its combined constraints (95% upper bound: less than 1 in 3,333). Whether that structure reflects explicit intentional design, as McKenna believed, or accumulated refinement over generations is outside what these measurements can decide.
