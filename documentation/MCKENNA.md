...
- **Complements are unusually close — scope stated** — `--complements` shows King Wen places complementary hexagrams closer together than random (0th percentile **against unconstrained random permutations**; 3.9th against the pair-constrained null). Within the fully constrained C1+C2+C3 population the picture inverts: KW sits at the C3 **maximum** (see [SOLVE.md](SOLVE.md)'s C3-ceiling note).
...
- The complement distance is genuinely unusual **against unconstrained random permutations** (0th percentile; 3.9th against the pair-constrained null — and within the C1+C2+C3 population KW sits at the C3 maximum, see [SOLVE.md](SOLVE.md)). `solve.c --null-gray-random` (10⁵ samples) additionally shows that **no random 6-bit Gray code achieves KW's C3 ≤ 776**: the minimum observed across 10⁵ Gray codes is 832, strictly greater than KW's 776. Even a structured permutation family explicitly optimized for adjacency (Hamming-1 transitions) cannot beat KW on complement placement.
