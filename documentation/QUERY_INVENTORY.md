...
| **Q4a/c** | the C3 census over SUPER: histogram of cd\* + μ = P(C3 = 776) with ⚠ **one Wilson interval on the acceptance mass, not per-bin CIs — narrowed 2026-09-06 (QSET-2 finding 5); this is the unfixed half of the 2026-09-05 correction to the same row, which addressed only its space label** — **ESTIMATE with CI**; the exact version is not shipped — no instrument in this program counts
