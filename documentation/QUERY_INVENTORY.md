...
`PENDING:<flag>`. The aggregate `QUERY_PROGRAM=PASS` is emitted when **no row FAILed**; `SKIP`
*and* `PENDING` rows alike do not block it. ⚠ **Corrected 2026-09-13 (V3A-044#1).** This read
*"emitted only if every non-`SKIP` token in scope is `PASS`"*, which says a `PENDING` — a
non-`SKIP` token — withholds the aggregate. **It does not.** What a `PENDING` withholds is
`TR12_REPRO_COMPLETE`: `row_skip` counts `PENDING` into `NSKIP`, and the driver emits
`TR12_REPRO_COMPLETE=YES` only at `NSKIP` = 0, so a single `PENDING` already forces
`TR12_REPRO_COMPLETE=NO`. The driver was right; this sentence was wrong. The n=9 rehearsal of this same driver emits `QUERY_DRYRUN=PASS`
...
| `PENDING:--kc-coset-census` | (ℤ/2)⁶ coset labelling of the transversal — subgroup set and coset-id map still undefined, so the producer cannot be specified yet. ⚠ *Corrected 2026-09-24 (Q-707 sibling): this cell read "aggregated over the scan tables"; TR-12 §Q10 (2026-09-20, narrowed at CX-58) shows the per-mask reading cannot be aggregated out of the scan-pass tables at all, and the step-difference reading would be a sum of persisted raw-kernel cells instead.* | Q10(b) only | Σ over cosets == layer mass at n=9; exhaustive n≤13 orbit counts |
...
| `PENDING:kissat` | external SAT solver, **not on PATH** (`which kissat` empty; `d4`, `cpog-gen`, `drat-trim` also absent) | Q4(b) | n/a — an install, not a gate ⚠ *(2026-09-25, CX-93: `TR12_Q7_WITNESSES` no longer carries this value — the Q7 witnesses are pinned and verified solver-free; the only battery row that still needs the solver is the opt-in `TR12_Q7_RESOLVE`, pinned `SKIP:opt-in`, never `PENDING:kissat`)* |
...
| **`TR12_V3_FIG`** | ✅ **THE JOIN EXISTS as of 2026-09-23** — `solve.py --v3-spectrum GRID OUT` reads the already-produced rank grid (`a1_v3` → `v3_rel_grid.tsv`), evaluates the frozen `--compute-stats` battery per walk, and emits `spectrum/v3_spectrum.tsv`; `fig_tr12_kc_spectrum` renders from it with no ladder and no VM. The token **stays pinned** at `PENDING:viz-v3-spectrum` (`scripts/tr12_expected/n9/_EXPECTED_SKIPS.txt`), and it will **not** clear on its own: `scripts/tr12_repro.sh` records `TR12_V3_FIG=PASS` only when `c_viz` rendered `fig_tr12_kc_spectrum`, and the battery **never invokes `--v3-spectrum`** — row `a1_v3` produces the rank grid and nothing joins it — so inside the battery `v3_spectrum.tsv` is always absent and the row always skips (`viz/report_figures.py::tr12_figures` docstring says the same). The shipped table and figure are the committed `tr12/v3_spectrum.tsv` and `reports/figures/fig_tr12_kc_spectrum.{png,svg}`, produced outside the battery. Wiring the join into the battery is a separate open item. ⚠ *Corrected 2026-09-24: this cell read "`scripts/tr12_repro.sh` already records `TR12_V3_FIG=PASS` when the figure is present, so it clears on the next full-31 run **by measurement**" — false, because nothing in the battery produces the figure's input.* The n=31 receipt keeps `PENDING` because that is what was true when that run executed | **the join shipped; the battery leg did not** |
| **`TR12_Q7_WITNESSES`** | `kissat` is not on PATH, and installing it is **not sufficient**: which satisfying sequence a solver returns is **build-dependent**, so a golden of solver-chosen bytes would pin one build's arbitrary choice. The reproducible contract is a **property** of the returned witness, not its bytes | no — needs a contract decision first ⚠ **[corrected 2026-09-25, CX-93: decision taken — the bytes are PINNED (`reports/evidence/q7_witnesses/`, kissat 4.0.4, once, README with commands and sha256s) and the row verifies the pinned sequences' PROPERTIES solver-free, so it PASSes with or without `kissat`. The live re-solve is the opt-in row `TR12_Q7_RESOLVE` (`--q7-resolve`), which asserts SAT status and re-verifies, never byte-equality.]** |
...
| **`TR12_XA_CD`** | **doubly blocked**: the R-1 throughput anchors **and** a W0-D t-unit → `SOLVE_NODE_LIMIT` mapping certificate. The latter **cannot be produced at HEAD** — the production enumerator has no reduced-n mode (`solve.c:1708-1710`, `init_pairs()` — the pair table is hardcoded to 32 pairs off `KW[]`, which is *why* there is no reduced-n mode; note this is true of the production DFS only, as the f1c5 counting DP does have `--f1-pairs N` at `solve.c:14783`) — and even after W0-D the verdict is **one-sided**, in the direction TR-12 §3 now gives: the production DFS (`backtrack` / `backtrack_iterative`) applies C3 only as a filter at the full-walk leaf and prunes on one combined `kw_dist` budget weaker than the t-ladder's boundary cap, so t-units bound production nodes from **below** (INFEASIBLE sound, EXHAUSTIBLE not; `reports/evidence/w0d_lower_bound/`). ⚠ *(Corrected 2026-09-25, Q-787: this read "since the production DFS prunes with C3 while t counts SUPER prefixes, so t-units bound nodes from above". The C3 premise is false of the code and the direction was reversed.)* | no |
...
|---|---|---|---|---|---|---|
| **Q1** | `rank_O3^SUPER(KW)` exact + the r−1/r/r+1 neighbour bracket + the H3b certificate (`rank(unrank(r))==r`, `unrank(rank(KW))` byte-identical to KW, `#provenance`). | `$SOLVE --kc-o3-cert "$FDIR" "$GDIR" KW --kc-cert-out $OUT/q1_rank_kw.json --kc-ooc --kc-cache-mb 196608 > $OUT/q1_rank_kw.txt` | f+g | POINT | `$OUT/q1_rank_kw.txt` (text) + `q1_rank_kw.json` (JSON) | `TR12_Q1=PASS` ⚠ **RULED 2026-09-04 (§9.1): DOC + GATE.** `rank3(KW)=0` is a labeling theorem (KW-derived `pl`/`pa`), executed at n=9 in miniature; the run must print `rank3=0, class_first_rank3=0, orient_idx=0` → **`TR12_Q1_LABELING=PASS`**; no rank/rarity statement about KW under O3 may be published; **Q1b (REL) is the reported coordinate**; O3′ stays PROPOSED. |
...
| **Q7** | per historical arrangement: IN/OUT + first-violated constraint under the pinned order C1→C2→C3→C4→C5 + full violation list; rank if IN. | `$SOLVE --check-arrangement KW --cert-out $OUT/q7_kw.json` · for each of `_r7_mawangdui _r7_fuxi _r7_jingfang`: `A=$(python3 -c "import solve;print(','.join(map(str,solve.<fn>())))"); $SOLVE --check-arrangement "$A" --cert-out $OUT/q7_<name>.json` · SAT witnesses via `python3 sat.py --witness moore-strict` / `grand-strict` then the same call; **ranks** for IN cases via the §0.4(2) adapter into `--kc-o3-cert`. ⚠ *(corrected 2026-09-25, CX-93: as built, the battery does NOT run `sat.py --witness` for the row — it reads the PINNED witnesses `reports/evidence/q7_witnesses/<target>.txt` (produced once with kissat 4.0.4; README there) and runs `$SOLVE --check-arrangement "$(sed -n 's/^SEQ=//p' <file>)" --cert-out $OUT/q7_<target>.json`, plus KW-distinctness and the rule re-score through `solve.py`/`verify.py`, in row `a0_q7_witnesses`; the live `--witness` command is the opt-in row `a0_q7_resolve` (`--q7-resolve`). Ranks: `a2_q7_ranks` calls `--kc-o3-rank`, not `--kc-o3-cert`, and at n = 31 prints `witness_serial` for each pinned witness.)* | **—** (verdicts); f+g (ranks only) | **NOLADDER — runnable today** | `tr12/q7_<name>.json` (JSON) + `q7_summary.md` (MD) | `TR12_Q7=PASS` |
...
| **Q9** | the reportable negatives, as **certified restatements with scopes** — no compute. TR-5 free action / 23 record-twins; TR-6 15 alternations + 30 switches; TR-7 odd wrap distance, 16 circular alternations, 32 switches; **plus §11's equivariance ceiling P(KW-record) ≤ 1/24** and the 8 forced literature rules promoted to theorem class. | none (writing). Cite **by theorem name, not by line** — 🔎 **PUBLIC ANCHOR:** `git grep -n "twenty_four_dvd_solution_count" <sha> -- lean/Automorphism.lean`, `git grep -n "within_pair_even_nonzero\|wrap_parity_general" <sha> -- lean/KingWen.lean`, `git grep -n "c3_slot_decomposition" <sha> -- lean/C3Decomposition.lean`. All four are committed and carry `#print axioms`. ⚠ `lean/C1RuleConstants.lean` — **the §3.3 branch finding is now STALE; see the correction there.** | — | **DOC (no compute)** | `$OUT/q9_negatives.md` (MD) | `TR12_Q9=PASS` |

### §2 — the visualization program

| id | must produce | exact command line(s) | ladders | class | artifact | verdict token |
|---|---|---|---|---|---|---|
| **V1** | positional-marginal field: `P(pair j at slot k)` = 32×31 heat matrix, KW's placements overlaid. **Source: `atlas.layers[k].marginal_raw`** (RAW frame — requires `--kc-raw`). | scan (row Q6) then `python3 solve.py --atlas-queries $OUT/atlas.json --atlas-out $OUT --atlas-select v1` then `viz/report_figures.py` *(both landed — §1 rows `PENDING:atlas-consumer` / `PENDING:viz/viz_kc_*.md`; these cells named the plan until 2026-09-24)* | f+g | DERIVED (post-scan) | `tr12/scan/v1_field.tsv` + `reports/figures/fig_tr12_kc_field.{png,svg}` + `viz/viz_kc_field.md` *(figure name corrected 2026-09-24: this read `fig_tr12_v1.*`, a file that never existed)* | `TR12_V1=PASS` |
| **V2** | mass river: layer-k mass split across k=1..31, KW's path drawn as a line. **Source: `atlas.layers[k].by_class{d1,d2,d3,d4,d6}`.** | same scan → `--atlas-queries … --atlas-select v2` → `viz/report_figures.py` | f+g | DERIVED | `tr12/scan/v2_river.tsv` + `tr12/scan/v2_branches.tsv` + `reports/figures/fig_tr12_kc_river.{png,svg}` + `viz/viz_kc_river.md` *(figure name corrected 2026-09-24: this read `fig_tr12_v2.*`, which never existed)* | `TR12_V2=PASS:REDUCED-NO-BRANCH-CLASS-RIVER` **(distance-class form only — §3.1)** *(token corrected 2026-09-24: this read bare `TR12_V2=PASS`; the emitter has written the qualified value since 2026-09-04)* |
| **V3** | rank spectrum: walk-decomposable functional values of `unrank(r)` on a systematic grid `r = i·⌊N/K⌋`, K=10³. | `for i in $(seq 0 999); do R=$(python3 -c "print($i*(1097051278789181790036112071176579186688//1000))"); $SOLVE --kc-unrank "$FDIR" $R --kc-ooc --kc-cache-mb 196608; done > $OUT/v3_rel_grid.txt` — **then the join:** `python3 solve.py --v3-spectrum $OUT/v3_rel_grid.tsv $OUT/spectrum/v3_spectrum.tsv` (`--v3-spectrum-order REL`, the default; landed 2026-09-23 — see `SOLVE_PY_CLI.md`). ⚠ *Updated 2026-09-24: this cell read "the join that produces `spectrum/v3_spectrum.tsv`, which does not exist; `solve.py` has no batch walk-functional evaluator at HEAD". The evaluator now exists; what is still missing is a **battery** row that runs it — `scripts/tr12_repro.sh` stops at the grid (row `a1_v3`).* This cell named no command until 2026-09-12 (V3A-044#4) | f (REL axis); f+g for the O3 axis | **POINT-BATCH — K=10³ MEASURED 31.4 min**, K=10⁴ ≈ +2.5–3 h. Keep it ONE process (cold phase is per-invocation). | `$OUT/v3_rel_grid.tsv` + `spectrum/v3_spectrum.tsv` (committed as `tr12/v3_spectrum.tsv`, REL, K=1000) + `reports/figures/fig_tr12_kc_spectrum.{png,svg}` + `viz/viz_kc_spectrum.md` *(corrected 2026-09-24: this read "`PENDING:viz-v3-spectrum` (the figure `fig_tr12_v3.*` is not produced)"; the figure exists under the `kc_` name, rendered outside the battery)* | `TR12_V3_TSV=PASS`, `TR12_V3=SKIP:leg-TR12_V3_FIG` — still the battery's value, because the battery does not run the join (§2.0 `TR12_V3_FIG`); `--v3-spectrum` itself emits `V3_SPECTRUM=PASS\|FAIL` |
| **V4** | KW's neighbourhood shells: `g(KW-prefix_k)` vs k, log-scale = Q3 as a figure. **Source: the `g=` column of Q3's trace — needs NO scan.** | `viz/report_figures.py` (landed) over `$OUT/q3_profile_kw.tsv` | (rides Q3) | DERIVED, no compute, local | `reports/figures/fig_tr12_kc_shells.{png,svg}` + `viz/viz_kc_shells.md` *(figure name corrected 2026-09-24: this read `fig_tr12_v4.*`, which never existed)* | `TR12_V4=PASS` (aggregate of `TR12_V4_TSV` and `TR12_VIZ`) |
| **V5** | transition grammar: `P(next-choice class \| layer k)` heat map, KW's actual choices marked, where the choice class is the **(d, w) cross-tab**: `d = popcount(a XOR b)` (the boundary distance class) × `w = popcount(entry XOR exit)` ∈ {2, 4, 6}, the within-pair Hamming distance of the newly placed pair — the new-pair category **as pinned by operator ruling 2026-09-23** (`solve.py::atlas_emit_v5`; `viz/viz_kc_grammar.md`). **Source: `atlas.layers[k].kernel`** (keys `m<a>_<b>`; needs `--kc-raw`), gated at emit time to marginalise back to `by_class[d]` at every `(k, d)` and to the layer flow, with `--atlas-fault v5-cross-swap` as the fault that only a `(d, w)` recount can see. | same scan → `--atlas-queries … --atlas-select v5` → `viz/report_figures.py` | f+g | DERIVED | `tr12/scan/v5_grammar.tsv` (`k d w mass p_cond kw_d kw_w`, 31 × 5 × 3 = 465 rows at full-31) + `reports/figures/fig_tr12_kc_grammar.{png,svg}` + `viz/viz_kc_grammar.md` | `TR12_V5=PASS` for the `(d, w)` cross-tab; `PASS:REDUCED-NO-CROSSTAB` (every `w = -1`) only when the atlas carries no kernel ⚠ *Updated 2026-09-24: this row read "Source: `atlas.layers[k].by_class`", named a figure `fig_tr12_v5.*` that never existed, and marked V5 "distance-class form only — §3.1". The `w` axis landed 2026-09-23; see §3.1's V5 row.* |
...
| id | must produce | exact command line(s) | ladders | class | artifact | verdict token |
|---|---|---|---|---|---|---|
| **LS-w0** | TR-8's pair-only null (a C1-only-space count; CHEAP, near-closed-form) — the **only** LS item inside the funded waves. | `$SOLVE --null-pair-constrained 1000000000 > $OUT/ls_pair_null.txt` | — | **NOLADDER — NOW-able** | `$OUT/ls_pair_null.txt` (text); **since 2026-09-08 the artifact carries two labels that must travel with the number (F-5 §5 / D15): `threshold_label` = KW-anchored (the tail is taken at KW's own `rc4_violations` = 2) and `correction_family` = the 91-observable suite ledger TR-8 reads it against (Bonferroni bar 0.05/91)** | `TR12_LS_W0=PASS` |
| **LS-forced8** | the eight forced-1.0 rows (mmt4, p1c4, s1, s6, r3, r4, r5, c2). §11 rules these **PROVEN C1 constants** ⇒ theorem class ⇒ they move into **Q9**, not into a DP run. | none (citation). ⚠ artifact **absent from this branch** — §3.3. | — | DOC (no compute) | folded into `$OUT/q9_negatives.md` | `TR12_LS_FORCED8=PASS` |
...
| id | must produce | exact command line(s) | ladders | class | artifact | verdict token |
|---|---|---|---|---|---|---|
| **EW-1** | the surprise-localization ledger: the exact per-choice surprise spectrum (31 bars) decomposing `log₂ N ≈ 129.689` bits = `Σᵢ −log₂ p_i`, **plus the pre-fixed interpretation contract** (concentration ⇒ a **LEAD** on where an undiscovered constraint might be sought, not a localization *(reworded 2026-09-13, V3B-03#23: this read "where an undiscovered constraint **must live**". `Σᵢ bitsᵢ = log₂ N` is a chain-rule identity, and nothing in it places a missing constraint's bits at the steps where `bitsᵢ` is large — any sequence-global functional, this project's own C3 sum included, spreads them across every step. "Must live" was an over-claim; concentration generates a candidate to investigate, it does not prove locality. The battery no longer prints the clause, so no verdict token is affected.)*; near-uniform typicality ⇒ boundable evidence that no further simple positional constraint exists — **both outcomes are findings**), plus the units bridge to TR-9's 296-bit ledger stated explicitly, plus the Q8 gallery comparison band. | `awk` the `bits=` column out of `$OUT/q3_profile_kw.tsv`; ~~band from `$OUT/gallery/q8_super.tsv`~~ 🔴 **PHANTOM (2026-09-04, §9.4): `q8_super.tsv` has no `bits` column.** Band = `for w in $(cut -f3 $OUT/gallery/q8_super.tsv); do $SOLVE --kc-profile "$FDIR" "$GDIR" "$w" --kc-tsv …; done` (1,000 profiles; statistic `top1_share = max bits / Σ bits`). **Rides Q3 — needs f+g, NOT the scan.** Gate: `sum_bits == log2N` on the `#o3-trace-summary` line. ⚠ **RULED 2026-09-04 (§9.4): calibrated null ADOPTED (audit P3) — KW's `top1_share` vs the gallery's 1st/99th percentiles, two-sided, once; the "both outcomes are findings" contract is replaced by the three pre-stated outcomes; n=9 rehearsal verdict `typicality-bound`.** | (rides Q3 + Q8) | DERIVED, no compute | a private frontier note + `$OUT/ew1_spectrum.tsv` | `TR12_EW1=PASS` + `TR12_EW1_NULL=<verdict>` |
...
`solve.c:29375` gates `t_done` on `fkc->total.l0 <= (1ull << 27)` with the high limbs zero — that
...
Measured against a **freshly generated** n=9 atlas (`--kc-build` / `--kc-g-build` at `--f1-pairs 9`,
then `--kc-scan f g atlas.json --kc-raw`): **42,537 B**, schema `"version": 2`. `layers[k]` carries
**13** keys — `k`, `flow`, `by_class{d1,d2,d3,d4,d6}`, `marginal_quotient{q…}`,
`marginal_raw{pair…}`, and the eight logging tables added by `eb6ba6a4`: `counts`, `extrema`,
`hist`, `digits`, `outdeg`, `kwrank`, `rid_mass`, `kernel`.

⚠ *Re-measured 2026-09-12. This paragraph read "Measured against `fixtures/kc_n9/atlas_n9.json`
(5,684 B) … and nothing else", and **every row below inherited that premise**. It was stale: the
logging build added eight per-layer objects, and the cited fixture is **not in the tree** (§8.10) —
regenerate rather than cite it. Two rows were corrected in opposite directions on the strength of
it the same day, one of them wrongly, so treat a verdict here as provisional until it has been
checked against a real atlas.*

🔴 **`--kc-raw` is load-bearing twice.** `kernel` is emitted only under `want_raw` (`solve.c:28000`),
the same flag `marginal_raw` needs. An n=31 scan run **without** `--kc-raw` loses V1 *and* V5's
derivability together, and that is the one case in this table that genuinely cannot be recovered
without paying the wall again. Everything else below is either served, derivable consumer-side, or
blocked on something that is not the scan.
...
| **Q6** per-layer **per-(state,choice)** mass; argmax/argmin-nonzero *choices* | per-layer per-**distance-class** mass only | ⚠ **reduced form.** Ships as per-layer distance-class mass + per-slot pair marginals. The per-choice argmax/argmin needs a new emitter. §9 already rules the argmin "loneliest corridor" **figure fodder, not a headline claim** — so the reduced form does not cost a headline. **Do not silently publish the reduced table as the spec's table.** ⚠ *A correction of mine was WITHDRAWN here on 2026-09-12, same day it landed. Acting on Codex KCP5 #4 I deleted "The per-choice argmax/argmin needs a new emitter", on the grounds that `extrema` (`solve.c:27908`) records argmax/argmin witnesses. **That was wrong and the sentence is restored.** `verify.py:1849` `q6_extremes_oracle` is the operational DEFINITION of reading (B) — state = (placed set, LAST ENDPOINT), choice = the UNORDERED pair — and it names the exact trap I fell into: "key on the ORIENTED pair → 3072/896 … 1728/224", which is what `extrema` produces, and "k=0 agreement proves nothing". Measured at n=9: oracle vs atlas disagree at EVERY layer (k=0 4736/3968 vs 3072/896; k=8 1952/992 vs 1728/224). `extrema` keys on the ORIENTED choice and on a state refined by the C5 residual `rid`, so it is a finer partition and no arithmetic recovers the coarse extremum. The per-(state,choice) table is also not a cheap re-emit: its cell space at n=31 is of order **10¹²–10¹³**, against the kernel's 126,976. *(An earlier revision of this note stated "~2×10¹²" flatly; that figure has no source in the tree and is restated here as the order-of-magnitude band an independent estimate supports — the conclusion is insensitive to a factor of 100, but the precision was not earned.)* Four places in this tree already said so — `scripts/tr12_repro.sh:3097`, `scripts/tr12_expected/n9/c_q6.txt:2`, `solve.py:12945`, `solve.py:14846` — and my edit contradicted all of them.* |
| **V2** layer mass split **by top-level branch class** | `branch_atlas[]` is per-branch **totals**, not per-layer-per-branch | ⚠ **reduced form.** Ships in the spec's parenthetical alternative — split by distance class of the k-th transition. ⚠ *Cost restated 2026-09-12, because this row was understating it in the expensive direction: the full form is **not** a re-scan, it is an **f-ladder REBUILD** — the DP state is `(canonical-mask, last, rid)` with no branch tag, so no persisted table recovers the split, and obtaining it means 56 ladders or a 56-wide channel, order 10² TB. That is worse than re-running the paid pass, not cheaper, and it is the only row here on that side of the line. Measured exception: at **layer 0 only** the split is exact — grouping `branch_atlas[]` by `entry` reproduces the layer-0 kernel column exactly, 12/12 cells summing to N.* |
| **V5** choice classes = distance class **× new-pair category** | ~~distance class only~~ `layers[k].kernel` (`--kc-raw`) → the `(d, w)` cross-tab, `w = popcount(entry XOR exit)` | ✅ **served in full since 2026-09-23** (the verdict below is kept for its history). The operator pinned the new-pair category as `w = popcount(entry XOR exit)` ∈ {2, 4, 6}, and `solve.py::atlas_emit_v5` now derives the cross-tab from the kernel key with no scan-side change, refusing unless it marginalises to `by_class[d]` at every `(k, d)` and to the layer flow; `TR12_V5=PASS`. It degrades to the reduced `w = -1` table (`PASS:REDUCED-NO-CROSSTAB`) only for an atlas with no kernel — i.e. a scan run without `--kc-raw`, which is why `--kc-raw` stays load-bearing. ⚠ *Superseded 2026-09-24 — this cell read as follows:* ~~⚠ **reduced form.** The second dimension is not shipped as its own table.~~ ⚠ *Corrected 2026-09-12 (Codex KCP5 #4, Opus disposition): this cell read "The second dimension is absent." It is not absent from the DATA — with `--kc-raw` the atlas persists every nonzero raw kernel cell `m<a>_<b>` (`solve.c:28002`), and both `d = popcount(a ^ b)` and the new pair's identity through entry `b` are recoverable from the key, so the joint is derivable from saved data without a re-scan. ⚠ *Narrowed 2026-09-12, because the first wording overstated it: "derivable" means **over data that already ships**, NOT with the shipped consumer. `solve.py` never reads `layers[].kernel` (zero hits, as for all eight logging tables) and the V5 emitter hardcodes the `w` column to `-1` (`solve.py:12292@12bef2b7`), so this is new consumer code — an afternoon, but not zero. The `partner()` rule it needs is total on all 64 hexagrams by construction via `db1_partner` (`solve.c:6725`) and `build_pairs()` (`solve.py:68`); note `kc->partner[64]` is NOT the right source — it is `-1`-filled outside the n-pair subset.* What is genuinely missing is the CATEGORY DEFINITION (`viz/viz_kc_grammar.md:61-62`), which is a consumer-side decision, not a scan-side gap. The distinction matters because the n=31 pass is paid once: "absent from the schema" would mean re-scanning to get it, and that is not the situation.* |
| **KW's path percentile per layer** (Q6) | ~~derivable: `mass_below` from Q3's `--kc-trace` ÷ `layers[k].flow`~~ | ~~✅ served, but **only by joining Q3's output to the atlas**~~ 🔴 **WRONG (Codex A09 f3; Q-48 executed; RULED 2026-09-04 §9.3):** `mass_below` is the O3 rank-block contribution, not a percentile numerator. Served instead by `kw_p` / `kw_class_pct` from the atlas alone (no join). ⚠ *Added 2026-09-12: the atlas now also carries `kwrank` — per class, the exact `lt`/`eq`/`gt` bins of every nonzero `w` against the reference walk's own `kw_w`, each with count, Σ orbit and Σ w·orbit (`SOLVE_C_CLI.md:2229`). That is a finer, mass-weighted percentile available from the atlas alone, and **no consumer reads it** (zero hits in `solve.py`, as for all eight new tables). 🔴 **Trap:** `kwrank`'s reference walk is King Wen only at n = 31; at every other n it is the O3 midpoint `unrank_O3(⌊N/2⌋)`, stamped in `kw_src` (an n=9 atlas reads `"O3-MIDPOINT"`). A "KW percentile" rehearsed at n=9 or n=13 from `kwrank` is **not KW**.* |

*(2026-09-24: V5 has left this list — its axis was pinned by the operator and landed 2026-09-23; the question below now concerns Q6 and V2.)*
...
⚠ *(2026-09-25, CX-93: this section no longer blocks Q7. The two SAT witnesses were produced once
on a worker with kissat 4.0.4 and are pinned in `reports/evidence/q7_witnesses/` with the CNF and
model sha256s; the battery verifies the pinned sequences with no solver (`a0_q7_witnesses`), and its
live re-solve is opt-in (`--q7-resolve`). A SAT witness needs no DRAT — the model is its own
certificate, re-checked by `sat.py --decode`'s `model_check` and by the row's property checks — so
the `--witness` path's missing proof flag is irrelevant to Q7; it still matters for the UNSAT legs
above.)*
...
> a missing sidecar, a missing field, a layer count other than n+1, a layer INDEX set other than
> exactly `00..n`, or a non-zero tool status. The
...
>
> ⚠ **Corrected 2026-09-25 (Q-782, found by Opus RR).** Until this date the check was on layer
> COUNT, not layer IDENTITY: it never asked which indices the n+1 digested layers were, and
> `--f1c5-layer-sha DIR` skipped an unreadable layer with exit 0. Measured at n=9: `g_layer_09`
> removed and a stray `g_layer_10` (a copy of 08 with its sidecar) added gave
> `LADDER_SHA_CHECK=OK`, and so did `g_layer_05` at mode 000 plus that stray. The row now requires
> each `<prefix>_layer_kk.bin`, k = 0..n, to be present, readable, digested and matched, and fails
> on any other `*_layer_NN.bin` in the directory, naming each (`MISSING`, `UNREADABLE`,
> `NOT-MATCHED`, `STRAY`); the DIR form exits 2 on an unreadable layer. A healthy ladder's
> transcript is byte-identical, so the n=9 goldens did not move. **No shipped verdict is
> affected:** the n=31 receipts carry these rows as `SKIP:cost-gated` / `SKIP:banked-pre-scan`,
> so this row never ran at n=31, and the n=9 goldens were taken on intact ladders. Red tests:
> `tests.py` `TestQ782LadderShaRowChecksLayerIdentity`.
>
...
> `runs/`. At n=9 the committed golden pins the values. That comparison is the separate
> `TR12_{F,G,T}IDENT` row; at n≥31 it runs whether or not `--with-laddersha` is passed.
> ⚠ *(corrected 2026-09-25, Q-683 taken under Q-782: with `--with-laddersha` the identity row used
> to be SKIPPED as `SKIP:full-pass-ran`, leaving the registry comparison unmade; it now runs
> beside the full pass. At n<31 it still stands down, since the shipped registry is n=31's.)*
...
| §2 | V1–V5 | V1, V2, V3, V4, V5 | all; V2 in reduced form (§3.1) — V5 in full `(d, w)` form since 2026-09-23 *(this read "V2/V5 in reduced form")*; V3's join exists but is not run by the battery (§2.0 `TR12_V3_FIG`) |
...
| 8.3 | `--kc-extremal` LANDED as a flag; Q5's wave-3 SKIP unchanged | usage block solve.c:37002 (re-anchored 2026-09-12, Q-540: the previous `:16701ff` was wrong at every sha checked -- :16701 is `kc_g_heartbeat_sec`. The real usage block is the `argc < 5` guard printing "Usage: solve --kc-extremal ...", which section 3 row Q5 already cites correctly); `a1_q5.txt` |
...
| 8.7 | the "do not build `--kc-profile`" ruling was OVERTAKEN: the flag was built (with `--kc-profile-selftest`), is O3-independent (f/g point lookups, no ranking), and is the natural instrument for the Q-331-safe Q3/EW-1 | usage block solve.c:32449 (re-anchored 2026-09-12, Q-540: the previous `:16664ff` was wrong at every sha checked -- :16664 is a sidecar-write comment. `kc_profile_main` is at :32448 and its `argc < 5` guard prints "Usage: solve --kc-profile ..." at :32451) |
...
`agg` aggregator (`scripts/tr12_repro.sh:3640` and `:3645`; the helper itself is defined at `:3626`) from their legs, and their pinned values
...
**✅ Already public (25).** ⚠ *(Count corrected 2026-09-13, V3A-044#6: this header still read
**26**. It was never decremented when the two R-1 throughput anchors moved to 🔒 on 2026-09-12, so
the four class headers summed to 26 + 6 + 5 + 6 = **43** against the stated denominator of **42** —
a published audit tally that did not sum. The summary table above already carried the corrected
**25**; the headers now agree with it, with each other, and with the denominator.)* `N` and `N−1`, `N/2`, `N/24`
...
1. **The four reduced forms (§3.1).** *(2026-09-24: V5 is no longer reduced — its `(d, w)` axis landed 2026-09-23 — so this now concerns Q6 and V2.)* Q6, V2 and V5 ship in a *reduced* form because the atlas
...
2. **The Q1 labeling degeneracy (§9.1).** At full-31, `rank_O3(KW) = 0` is *forced* by KW-derived labels — the
   coordinate is vacuous by construction there. (Scope, Q-717: at n=9 the battery anchor's rank is 13056
   and the KW-orbit-rank verdict is NONVACUOUS; the vacuity is an n=31 fact, not a property of every rung.) The ruling is to publish it as a labeling theorem and
...
6. **Every `PENDING:` row.** Four remain: `--kc-coset-census`, `sat-c3min-driver`, `kissat`, and the
   ...
   exact failure mode this file exists to prevent. ⚠ *(2026-09-25, CX-93: the `kissat` PENDING
   landed for Q7 — `TR12_Q7_WITNESSES` is a PASS row on pinned witnesses and no battery row reads
   `PENDING:kissat` any more; `kissat` stays pending only for Q4(b)'s UNSAT legs, which the minimum
   no longer needs.)*
