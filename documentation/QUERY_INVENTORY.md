...
| **Q5** | functional extremals over SUPER (min/max of a shortlisted G-invariant functional) + explicit witness walk. | `PENDING:--kc-extremal` → `$SOLVE --kc-extremal FUNC "$FDIR" max\|min` *(the direction argument is REQUIRED: `kc_extremal_main` prints usage and returns 2 when `argc < 5`, `solve.c:35808` — the previous form in this cell would have exited 2 at every n, for a reason unrelated to the OOC refusal this row is about)* | f | SCAN-class, **one full Stage-F-shaped pass per functional** | `tr12/q5_<func>.txt` | `TR12_Q5=SKIP:wave3-not-budgeted` 🔴 **AND AT n=31, COST IS NOT THE ONLY GATE (Codex R5, 2026-09-11).** `kc_open` (`solve.c:20662`) delegates to `kc_open_as`, which returns the out-of-core reader whenever `n > KC_MEM_MAX_PAIRS` (`solve.c:20657`), and `kc_extremal_main` refuses an out-of-core f ladder immediately (`solve.c:35883`), *before* the invariance gate, the DP, the null-vs-g check, the witness and the certificate: *"v1 is IN-MEMORY ONLY … the streaming, eviction-resumable OOC extremal builder is a SEPARATE, UNBUILT item … it is the full-31 enabler."* So `--wave3` at n=31 **exits 2 and computes nothing**. Budget is an operator decision; an unbuilt builder is not, and reading this row as "money would buy it" is wrong. |
...
`solve.c:29340` gates `t_done` on `fkc->total.l0 <= (1ull << 27)` with the high limbs zero — that
...
🔴 **`--kc-raw` is load-bearing twice.** `kernel` is emitted only under `want_raw` (`solve.c:27965`),
...
| 8.3 | `--kc-extremal` LANDED as a flag; Q5's wave-3 SKIP unchanged | usage block solve.c:35808 (re-anchored 2026-09-12, Q-540: the previous `:16701ff` was wrong at every sha checked -- :16701 is `kc_g_heartbeat_sec`. The real usage block is the `argc < 5` guard printing "Usage: solve --kc-extremal ...", which section 3 row Q5 already cites correctly); `a1_q5.txt` |
...
`agg` aggregator (`scripts/tr12_repro.sh:3200` and `:3205`; the helper itself is defined at `:3186`) from their legs, and their pinned values
