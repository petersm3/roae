...
`solve.c:29325` gates `t_done` on `fkc->total.l0 <= (1ull << 27)` with the high limbs zero — that
...
| 8.3 | `--kc-extremal` LANDED as a flag; Q5's wave-3 SKIP unchanged | usage block solve.c:35793 (re-anchored 2026-09-12, Q-540: the previous `:16701ff` was wrong at every sha checked -- :16701 is `kc_g_heartbeat_sec`. The real usage block is the `argc < 5` guard printing "Usage: solve --kc-extremal ...", which section 3 row Q5 already cites correctly); `a1_q5.txt` |
