...
| **Q8** | k=1,000 exact-uniform SUPER samples with the pinned seed + the C3-rejection C15 subset (~121 expected) + records + cd\*; chi² uniformity gate. | `$SOLVE --kc-sample "$FDIR" 1000 $GALSEED --kc-record --kc-ooc --kc-cache-mb 196608 > $OUT/gallery/q8_super.tsv` · `$SOLVE --kc-sample "$FDIR" 1000 $GALSEED --kc-c3-max 387 --kc-record --kc-ooc > $OUT/gallery/q8_c15.tsv` · membership re-check of every sample via `--kc-member`; chi² in the consumer. Plumbing gate: `$SOLVE --kc-midn 13 --kc-chi2-samples 20000` | f | POINT | `tr12/gallery/q8_*.tsv` (TSV) + `q8_chi2.txt` | `TR12_Q8=PASS` |
...
| **XA-24** | §11 refinement: a **24-divisibility integrity self-check on every headline count**, Lean-kernel-backed (`twenty_four_dvd_*`). Dispositive and cheap. | `PENDING:atlas-consumer --gate-mod24` over every count in `atlas.json` | — | DERIVED, no compute | line in `tr12/xa_verdict.md` | `TR12_XA_MOD24=PASS` |
