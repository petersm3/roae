...
| **Q6** per-layer **per-(state,choice)** mass; argmax/argmin-nonzero *choices* | per-layer per-**distance-class** mass only | ⚠ **reduced form.** Ships as per-layer distance-class mass + per-slot pair marginals. §9 already rules the argmin "loneliest corridor" **figure fodder, not a headline claim** — so the reduced form does not cost a headline. **Do not silently publish the reduced table as the spec's table.** ⚠ *Corrected 2026-09-12 (Codex KCP5 #4, Opus disposition): this cell read "The per-choice argmax/argmin needs a new emitter." The logging build (`eb6ba6a4`) now writes `extrema` per (layer, class) with the complete transition witness — `x_q`, `x_o`, `x_cm`, `x_rid`, `x_qpair`, `x_fv`, `x_gv` (`solve.c:27873`) — so the argmax/argmin CHOICES are recorded and no new emitter is owed for them. Whether that plus `kernel`/`rid_mass` fully serves the spec's per-(state,choice) table is **not yet re-derived**; this row stays marked reduced until someone does the derivation against the real schema rather than against this sentence.* |
