...
- **[PREREG_H1_H3_TEST_2026_07_26.md](PREREG_H1_H3_TEST_2026_07_26.md)** — The frozen pre-registration implemented by `roae.py --prereg-h1h3`, disclosed 2026-09-04 and byte-identical to the row escrowed above (sha256 `ab09648c…`, 29,896 bytes), so a reader can check the spec against the code instead of taking the correspondence on attestation. K = 4 pre-declared predicates, their thresholds, the MDL accounting, a per-predicate circularity audit, and the pre-registered prediction that **all four FAIL** their bars. ⚠ Its front matter and §8 describe it as held privately; both were true at its 2026-07-26 freeze and are left unedited because changing them would break the escrowed hash.
...
- **[PREREG_CLASSA_QUERY_SET.md](PREREG_CLASSA_QUERY_SET.md)** — The class-A pre-registration: three questions (V4, XA-a, XA-b) about the compiled catalog, published **before** their answers exist, with the exact observable, decision rule, gates and command for each. ⚠ **Published for adversarial review and NOT YET FROZEN** — the freeze is a third-party timestamp, not this commit, and the file may still change until that lands. States its own limits at length, including which of its claims a reader can check without trusting us and which remain attested; records the four rows removed from the set, and why each was removed.
- **[QUERY_INVENTORY.md](QUERY_INVENTORY.md)** — The executable contract behind [TR-12](../reports/TR12_QUERY_PROGRAM.md): one row per query with its exact command line, the ladders it mounts, its effort class, its output artifact and its `KEY=value` verdict token, then the full run order that `scripts/tr12_repro.sh` implements. Records what EXISTS versus what is `PENDING:` against the compiled binary, and reports a **FINDING** wherever a claim's only support is an artifact that is not public rather than dressing the gap in generic prose. Its Appendix E names what a reviewer should attack first.
