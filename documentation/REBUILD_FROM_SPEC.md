...
✅ **Scope of that cross-check — divergence found 2026-09-02, CLOSED the same day.** "Both shipped verifiers" above means `verify.py` and `solve --verify`. On the morning of 2026-09-02 it did **not** extend to `verify.c --check-artifact`, the independent C instrument documented in [VERIFY.md](VERIFY.md): that mode read the 32-byte header, checked the magic, and then reported `SCOPE=validity_sortedness_dedup_only_NOT_completeness`, holding no version, reserved-byte or file-geometry test. Executed then on one-record artifacts built from King Wen — with `header[20]=0x5A`, with a declared format version of `2`, and with a header declaring 5 records over a 1-record body — `verify.py` returned rc=1, rc=2 and rc=2 with a named error in each case, while `verify.c --check-artifact` returned `ARTIFACT=PASS` on all three. ⚠ **[CORRECTED 2026-09-02 — this paragraph read "Until that is closed in `verify.c`" and instructed the reader not to treat a `verify.c` pass as agreement about the header. The gap closed the same day and the instruction went stale.]** `verify.c` now counts and reports `BAD_HDR_VERSION`, `BAD_HDR_RESERVED` and `BAD_GEOMETRY` (and `BAD_C3`) into its failure total, and `verify.py` prints the same four counters, so **all three shipped readers now agree about the header** and a `verify.c --check-artifact` pass may be read as header agreement. The three fixtures above are permanent negative controls in `tests.py::TestCheckArtifactControls`; re-executed 2026-09-02 (`python3 -m unittest tests.TestCheckArtifactControls`, 9 tests, all green), each asserts `ARTIFACT=FAIL` with its own `BAD_` token from **both** instruments, matched whole-line. `verify.c`'s `SCOPE=` line still bounds what a pass attests — validity, sortedness and dedup, never completeness — and that has not changed.
...
> shipped verifiers" means `verify.py` and `solve --verify`. At the time this addendum was written it
> did not extend to `verify.c --check-artifact`, which enforced neither the reserved field nor the
> version nor the file geometry — measured 2026-09-02, with the executed verdicts recorded under the
> Step-1 header table.
>
> ✅ **[CORRECTED 2026-09-02, later the same day — the sentence that closed this addendum read that
> the `verify.c` divergence was "disclosed and open" and awaiting a code change. It is closed.]**
> `verify.c` gained all three checks plus a C3 leg; both instruments now print `BAD_HDR_VERSION`,
> `BAD_HDR_RESERVED`, `BAD_GEOMETRY` and `BAD_C3` and agree token for token on the three fixtures,
> gated permanently by `tests.py::TestCheckArtifactControls`. The recipe was already correct and is
...
- **2026-09-02 (later the same day)**: The one open item the entry above left behind is **closed**, and the two places this document still told a reader it was open are corrected in place with dated markers, the superseded reading preserved in each. `verify.c --check-artifact` now enforces the format version, the reserved bytes and the file geometry — it counts `BAD_HDR_VERSION`, `BAD_HDR_RESERVED`, `BAD_GEOMETRY` and `BAD_C3` into its failure total, as `verify.py` does — so "both shipped verifiers" in the Step-1 header discussion and "all three C readers" in the Spec-gaps addendum no longer need their `verify.c` carve-out. The three fixtures the carve-out named are permanent negative controls in `tests.py::TestCheckArtifactControls`, re-executed for this row (`python3 -m unittest tests.TestCheckArtifactControls`, 9 tests green), each asserting the failure token from **both** instruments by whole-line match. No change to the recipe itself: the pair table, the constraint checks, the sort order, the dedup rule and the `403f7202…` anchor are untouched, and the reader-facing instructions in Steps 1-11 are unchanged.
