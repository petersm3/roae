...
### 100T d3 canonical results (2026-04-20) — superseded as deepest by 560T canonical (2026-06-08); findings below remain valid as 100T-scoped, 560T re-run pending

Running the same C1+C2+C3 enumeration at 10× the node budget (100 trillion vs 10 trillion) produced **3,432,399,298 canonical orderings** — roughly 4.86× the 10T count. The **560 T canonical** (sha `9a968fa2…`, 10,525,271,997 orderings, established 2026-06-08) is now the deepest published enumeration; the 100 T findings below are valid as 100 T-scoped and remain a strict subset of the 560 T data. The 560 T re-evaluation of each finding below is queued and will produce either reaffirmation (the same headline) or refinement (the boundary-minimum may grow from 5 toward 6 at 560 T; the shift-pattern conformance % may shift slightly; the C3-ceiling claim is robust because KW's membership in the canonical set is unchanged by deeper enumeration). Findings (100T scope):
