...
and `SOLVE_KNUTH_FIBER_PERM` (solve.c:36259-36318). They are named here now, but
...
- `threads` — number of pthreads to use. Default `nproc`, clamped to the
  sub-branch count and to a 256 ceiling (`SOLVE_THREADS`, solve.c:43301-43308). *(Corrected
...
Exits 0 on PASS and **40** on a sha mismatch (`solve.c:34800-34808`, "validation
...
> write anywhere in the block (solve.c:35192-35207). So it does **not** cover
...
`strtoll` under an endptr-must-be-NUL check (`--preflight`, solve.c:35785-35800),
...
default `560000000000000` (`--preflight`, solve.c:35785-35800).
...
> `--preflight`, solve.c:35769-35779).** This section previously described two defects as open; both are closed and the description below is what the code now does.
>
...
(solve.c:36914-36919) — then a **fixed, hand-maintained subset** of `SOLVE_*`
...
(`printf("SOLVE_DEPTH=%d\n", …)`, solve.c:36985) with no `export`, so a plain
...
> behavioral change to canonical enumeration" (solve.c:37078-37087), yet on a
> capable host it prints a YES verdict asserting a vectorised path will be
> selected (`avx512_ready`, solve.c:37105-37107). Measured on this tree:
> `solve.c` contains 17
...
> `kw_found_v` is printed (`solve.c:38884` for `--validate`, `:38341` for
> `--verify`) and is folded into neither verdict: `--validate` returns
> `errors > 0 ? 1 : 0` and prints `VALIDATE=PASS|FAIL` (`solve.c:38912-38913`);
> `--verify`'s `total_fail` (`solve.c:38465`) sums C1-C5, decode, sort and dup
...
> sorted-order loop clears `sorted_ok` and counts the error (solve.c:38777-38778);
...
> `@`, not `:`, so they cannot be read as line numbers), used the same way in the `validate_mode` engine comment at solve.c:38683-38693.
>
...
>   (`hdr_records`, solve.c:34843-34848) and the read loop is bounded by it (`n_records`, solve.c:34865),
...
>   landed in `--verify` only (`SOL_HEADER_SIZE`, solve.c:38288-38289); the three audit
...
>   `pidx = (rec[i] >> 2) & 0x3F` in the `--verify-rule2` scan loop (solve.c:34850-34860),
...
(`solve.c:32403`, `solve.c:32485`): `WRITTEN` means the certificate file is on disk and was
...
> into `layer_root` (solve.c:34442; the target is assembled at solve.c:34502-34503),
...
offset and calls `gzseek` (`SOL_RECORD_SIZE` stride, solve.c:38603-38604),
...
essentially the whole file. The `show_mode` block (solve.c:38497-38502) says
...
`PROOF: Position 2 determines positions 3-19` (solve.c:42043): for each valid branch it
...
`PROOF: All self-complementary branches produce valid orderings` (solve.c:41842):
...
own banner is
`PROOF: Positions 3-19 have exactly 2 budget-feasible candidates` (solve.c:41909): at each position 3-19 it tests all 30 unused
...
(`strcmp(sha_full, sha_56)`, solve.c:37419). There is no scope list and no baseline from
...
(solve.c:37234-37251). Note the practical consequence: it cannot catch a
...
exists, else `/tmp` (`SOLVE_REGRESS_DIR`, solve.c:37496-37499). The positional argument is a node
...
to walk (`manifest_path`, solve.c:37171). The scan target is hard-coded `.`
...
(`LC_ALL=C sort > %s`, solve.c:3357) and into the `wc -l < manifest_path` count (solve.c:37190), so a path
...
`auto_emit_shard_manifest_default()` (solve.c:43843 and solve.c:44286); it
...
*file*, not a directory — `manifest_path` at solve.c:37171), re-computes the sha256 of
...
(solve.c:37704-37719). Output is **one aggregate line** —
`<n_below> <n_total>`, the count of queries whose log-density is at or below
`--threshold` and the number scored (`solve.c:37732`) — not a per-record
...
|---|---|---|
| `SOLVE_THREADS` | `nproc` (`sysconf(_SC_NPROCESSORS_ONLN)`; 8 if that fails), then clamped to the sub-branch count and to a hard ceiling of **256** | Number of pthreads for enumeration. ⚠ Row corrected 2026-09-01: this cell previously capped the default at 128. That cap lives only in `manifest_thread_count()` (solve.c:3317-3322), a different function; the enumeration path is solve.c:43301-43308 |
...
| `SOLVE_SUB_BRANCH_PARALLELISM` | unset | Value domain is **`{single, force-parallel}`** only (solve.c:37874-37884): `single` forces the serial path, `force-parallel` forces the parallel path even at one worker. ⚠ Row corrected 2026-09-01: this cell previously advertised a numeric core count as the value; a number matches neither recognised string and changes nothing; worker count comes from `SOLVE_THREADS` / the positional `threads` argument, and the parallel path is taken whenever that is > 1 |
| `SOLVE_REGRESS_DIR` | `/mnt/work` if it exists, else `/tmp` | Directory for `--regression-test` / `--double-regression-test` artifacts (solve.c:37284, :37190-37193). ⚠ Row corrected 2026-09-01: previously `./` |
...
| 50 | **Regression-test / internal-consistency failure** — a phase failure or sha mismatch in `--regression-test` / `--double-regression-test`, or a startup King Wen self-check failure (`solve.c` has 19 `return 50` sites; e.g. :37236, :37290, :38050). ⚠ Row corrected 2026-09-01: this row previously attributed code 50 to a `--selftest` sha mismatch. `--selftest` returns **40** on sha mismatch (`solve.c:34804-34808`), not 50. |
...
- `shard_manifest.txt` — auto-emitted at promotion/startup (after
  ...
  only, not after every flush (`auto_emit_shard_manifest_default`, solve.c:43843 and solve.c:44286) — unless
  `SOLVE_SKIP_AUTO_MANIFEST=1`.
...
solve.c:34247: `gcc -O3 -pthread -fopenmp -o solve solve.c -lm -lz`.)*
...
Dispatched as a subcommand at `solve.c:33119`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-build`

Dispatched as a subcommand at `solve.c:33237`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-cert-selftest`

Dispatched as a subcommand at `solve.c:33104`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-count`

Dispatched as a subcommand at `solve.c:33248`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:32936`.*
...
```
*Grammar reproduced from `solve.c:33010`.*
...
```
*Grammar reproduced from `solve.c:33299`.*
...
```
*Grammar reproduced from `solve.c:32905`.*

#### `--kc-o3-rank` / `--kc-o3-unrank`

Dispatched together at `solve.c:32972`. `solve.c` carries one `Usage:` template for both,
naming `--kc-trace` and `--kc-bracket` (`solve.c:32975-32986`; the subcommand and its third
...
Dispatched as a subcommand at `solve.c:32971`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:32920`.*
...
⚠ **Added 2026-09-07 (Q-410).** The flag was dispatched at `solve.c:33101` and
...
Dispatched as a subcommand at `solve.c:33100`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33286`.*
...
```
*Grammar reproduced from `solve.c:33307`.*

#### `--kc-scan-selftest`

Dispatched as a subcommand at `solve.c:33105`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-selftest`

Dispatched as a subcommand at `solve.c:32903`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-t-build`

Dispatched as a subcommand at `solve.c:33065`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33050`.*
...
Dispatched as a subcommand at `solve.c:33049`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33253`.*

### Modifiers

| flag | arity | parsed at | sets |
|---|---|---|---|
| `--kc-bracket` | none (boolean) | `solve.c:32995` | `o3bracket` |
| `--kc-cert-out` | 1 string | `solve.c:25638` | `cert_out` |
| `--kc-chi2-samples` | 1 integer | `solve.c:32914` | `M` |
| `--kc-class-uniform` | none (boolean) | `solve.c:33191` | `class_uniform` |
| `--kc-dump` | 1 integer | `solve.c:25637` | `dump_max` |
| `--kc-expect-count` | 1 string | `solve.c:25639` | `expect` |
| `--kc-g-ooc` | none (boolean) | `solve.c:32953` | `gooc` |
| `--kc-oracle-repr` | none (boolean) | `solve.c:25634` | `check_repr` |
| `--kc-record` | none (boolean) | `solve.c:33192` | `want_record` |
| `--kc-roundtrips` | 1 integer | `solve.c:32913` | `R` |
| `--kc-scratch` | 1 string | `solve.c:32930` | `scratch` |
| `--knuth-dump-prefix` | see code | `solve.c:36003` | — |
| `--r11-verify` | see code | `solve.c:36506` | — |
| `--rc1c-verify` | see code | `solve.c:36479` | — |
...
| `--kc-braket` | `--kc-bracket` | `--kc-o3-rank` / `--kc-o3-unrank` | `solve.c:32997-33004` | ran with 0 bracket lines and exit 0 |
| `--kc-witnes` | `--kc-witness` | `--kc-extremal` | `solve.c:32348-32355` | produced `KC_EXTREMAL=OK` with no witness |
