...
and `SOLVE_KNUTH_FIBER_PERM` (solve.c:39889-39948). They are named here now, but
...
- `threads` — number of pthreads to use. Default `nproc`, clamped to the
  sub-branch count and to a 256 ceiling (`SOLVE_THREADS`, solve.c:46931-46938). *(Corrected
...
> write anywhere in the block (solve.c:38822-38837). So it does **not** cover
...
`strtoll` under an endptr-must-be-NUL check (`--preflight`, solve.c:39415-39430),
...
default `560000000000000` (`--preflight`, solve.c:39415-39430).
...
> `--preflight`, solve.c:39399-39409).** This section previously described two defects as open; both are closed and the description below is what the code now does.
>
...
(solve.c:40544-40549) — then a **fixed, hand-maintained subset** of `SOLVE_*`
...
(`printf("SOLVE_DEPTH=%d\n", …)`, solve.c:40615) with no `export`, so a plain
...
> behavioral change to canonical enumeration" (solve.c:40708-40717), yet on a
> capable host it prints a YES verdict asserting a vectorised path will be
> selected (`avx512_ready`, solve.c:40735-40737). Measured on this tree:
> `solve.c` contains 17
...
> `kw_found_v` is printed (`solve.c:42514` for `--validate`, `:42066` for
> `--verify`) and is folded into neither verdict: `--validate` returns
> `errors > 0 ? 1 : 0` and prints `VALIDATE=PASS|FAIL` (`solve.c:42542-42543`);
> `--verify`'s `total_fail` (`solve.c:42095`) sums C1-C5, decode, sort and dup
...
> sorted-order loop clears `sorted_ok` and counts the error (solve.c:42407-42408);
...
> `@`, not `:`, so they cannot be read as line numbers), used the same way in the `validate_mode` engine comment at solve.c:42313-42323.
>
...
>   (`hdr_records`, solve.c:38473-38478) and the read loop is bounded by it (`n_records`, solve.c:38495),
...
>   landed in `--verify` only (`SOL_HEADER_SIZE`, solve.c:41918-41919); the three audit
...
>   `pidx = (rec[i] >> 2) & 0x3F` in the `--verify-rule2` scan loop (solve.c:38480-38510),
...
(`f1c5_finalized_try_adopt`, solve.c:15545). The refusal is **not** fatal
...
| `hist` | per class `hb1..hb6`, the magnitude histogram of `w` at bucket `lgB`, `B = floor(log2 w)`: `hc` (count), `ho` (Σ orbit size), `hw` (Σ w), `hwo` (Σ w·orbit). Zero buckets skipped. Gates: `hc·2^B ≤ hw ≤ hc·(2^{B+1}−1)` and `ho·2^B ≤ hwo ≤ ho·(2^{B+1}−1)`, computed as shifts so no 192-bit headroom is assumed; `Σ_B hc == st_nzN`, `Σ_B ho == st_nzoN`, `Σ_B hwo == dN` |
...
n = 12/13 VM leg runs strict. Everything else above is chunk-local and refusing. The consumer
(`solve.py atlas_load`) re-sums every atlas-visible identity at load in all cases.
...
(`solve.c:35831`, `solve.c:35913`): `WRITTEN` means the certificate file is on disk and was
...
> into `layer_root` (solve.c:38072; the target is assembled at solve.c:38132-38133),
...
essentially the whole file. The `show_mode` block (solve.c:42127-42132) says
...
`PROOF: Position 2 determines positions 3-19` (solve.c:45673): for each valid branch it
...
`PROOF: All self-complementary branches produce valid orderings` (solve.c:45472):
...
own banner is
`PROOF: Positions 3-19 have exactly 2 budget-feasible candidates` (solve.c:45539): at each position 3-19 it tests all 30 unused
...
(solve.c:40864-40881). Note the practical consequence: it cannot catch a
...
exists, else `/tmp` (`SOLVE_REGRESS_DIR`, solve.c:41126-41129). The positional argument is a node
...
to walk (`manifest_path`, solve.c:40801). The scan target is hard-coded `.`
...
(`LC_ALL=C sort > %s`, solve.c:3360) and into the `wc -l < manifest_path` count (solve.c:40820), so a path
...
`auto_emit_shard_manifest_default()` (solve.c:47473 and solve.c:47916); it
...
*file*, not a directory — `manifest_path` at solve.c:40801), re-computes the sha256 of
...
(solve.c:41334-41349). Output is **one aggregate line** —
`<n_below> <n_total>`, the count of queries whose log-density is at or below
`--threshold` and the number scored (`solve.c:41362`) — not a per-record
...
| `SOLVE_SUB_BRANCH_PARALLELISM` | unset | Value domain is **`{single, force-parallel}`** only (solve.c:41504-41514): `single` forces the serial path, `force-parallel` forces the parallel path even at one worker. ⚠ Row corrected 2026-09-01: this cell previously advertised a numeric core count as the value; a number matches neither recognised string and changes nothing; worker count comes from `SOLVE_THREADS` / the positional `threads` argument, and the parallel path is taken whenever that is > 1 |
| `SOLVE_REGRESS_DIR` | `/mnt/work` if it exists, else `/tmp` | Directory for `--regression-test` / `--double-regression-test` artifacts (solve.c:40914, :40915-40918). ⚠ Row corrected 2026-09-01: previously `./` |
...
| `SOLVE_KNUTH_BOUNDARY_COND` | `0` (off; set `=1` to enable) | Per-boundary KW-agreement mass accumulators (31; the `--analyze` §[6] predicate on the estimator); conditional on the pin prefix if set. Estimator-only, sha-neutral. ⚠ Row corrected 2026-09-01: this cell read `1`. `static int knuth_bcond = 0;` (solve.c:5421) and the flag is set only by `if (getenv("SOLVE_KNUTH_BOUNDARY_COND") && atoi(...) == 1)` (solve.c:39817), so an `--estimate-knuth` run gets none of the 31 accumulators unless you set it. |
...
- `shard_manifest.txt` — auto-emitted at promotion/startup (after
  ...
  only, not after every flush (`auto_emit_shard_manifest_default`, solve.c:47473 and solve.c:47916) — unless
  `SOLVE_SKIP_AUTO_MANIFEST=1`.
...
Dispatched as a subcommand at `solve.c:36749`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-build`

Dispatched as a subcommand at `solve.c:36867`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-cert-selftest`

Dispatched as a subcommand at `solve.c:36733`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-count`

Dispatched as a subcommand at `solve.c:36878`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:36530`.*
...
Dispatched as a subcommand at `solve.c:36529`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-ladder-selftest`

Dispatched as a subcommand at `solve.c:36731`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:36929`.*
...
```
*Grammar reproduced from `solve.c:36499`.*

#### `--kc-o3-rank` / `--kc-o3-unrank`

Dispatched together at `solve.c:36566`. `solve.c` carries one `Usage:` template for both,
naming `--kc-trace` and `--kc-bracket` (`solve.c:36569-36580`; the subcommand and its third
...
* `--kc-trace` prints one `#o3-trace` row per placement (the f·g descent trace TR-12 Q3 /
  ...
  `KC_O3_TRACE` is printed at `solve.c:24475` (rank) and at `solve.c:24513` (unrank),
...
Dispatched as a subcommand at `solve.c:36565`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:36514`.*
...
*Grammar and description reproduced from `solve.c:25938`.*
...
⚠ **Added 2026-09-07 (Q-410).** The flag was dispatched at `solve.c:36730` and
...
Dispatched as a subcommand at `solve.c:36729`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:36916`.*
...
```
*Grammar reproduced from `solve.c:36937`.*

#### `--kc-scan-selftest`

Dispatched as a subcommand at `solve.c:36734`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-selftest`

Dispatched as a subcommand at `solve.c:36497`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-t-build`

Dispatched as a subcommand at `solve.c:36694`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:36679`.*
...
Dispatched as a subcommand at `solve.c:36678`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:36883`.*

### Modifiers

| flag | arity | parsed at | sets |
|---|---|---|---|
| `--kc-bracket` | none (boolean) | `solve.c:36589` | `o3bracket` |
| `--kc-cert-out` | 1 string | `solve.c:25958` | `cert_out` |
| `--kc-chi2-samples` | 1 integer | `solve.c:36508` | `M` |
| `--kc-class-uniform` | none (boolean) | `solve.c:36821` | `class_uniform` |
| `--kc-dump` | 1 integer | `solve.c:25957` | `dump_max` |
| `--kc-expect-count` | 1 string | `solve.c:25959` | `expect` |
| `--kc-g-ooc` | none (boolean) | `solve.c:36547` | `gooc` |
| `--kc-oracle-repr` | none (boolean) | `solve.c:25954` | `check_repr` |
| `--kc-record` | none (boolean) | `solve.c:36822` | `want_record` |
| `--kc-roundtrips` | 1 integer | `solve.c:36507` | `R` |
| `--kc-scratch` | 1 string | `solve.c:36524` | `scratch` |
| `--knuth-dump-prefix` | see code | `solve.c:39633` | — |
| `--r11-verify` | see code | `solve.c:40136` | — |
| `--rc1c-verify` | see code | `solve.c:40109` | — |
...
|---|---|---|---|---|
| `--kc-alt` | `--kc-alts` | `--kc-profile` | `solve.c:32365-32371` | ran with 0 `#alt` rows and `KC_PROFILE=OK` |
| `--kc-braket` | `--kc-bracket` | `--kc-o3-rank` / `--kc-o3-unrank` | `solve.c:36591-36598` | ran with 0 bracket lines and exit 0 |
| `--kc-witnes` | `--kc-witness` | `--kc-extremal` | `solve.c:35776-35783` | produced `KC_EXTREMAL=OK` with no witness |
