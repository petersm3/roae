...
solve --kc-scan F G OUT.json --kc-scan-threads T        # parallel layer pass; byte-identical at every T
solve --kc-scan-par-selftest                            # its reduction / cache-clone / T-invariance gate
...
solve --kc-dead-census FDIR GDIR|- [--kc-layers A B]     # atlas query 7: which constraint kills (dead-state census)
...
of them are refusals that exit 1 rather than modes that run:

* `FIBER_MODE=ACTIVE-UNRESTRICTED` — the W/m canonical-class (record) estimator
  is running, on the unrestricted fiber. This is the only value under which a
  `records_*` figure from a fiber run may be published.
* `FIBER_MODE=REFUSED-STRICT-WALK` — refused because a strict or scoring walk
  was requested alongside it. The estimator is unbiased for the record count
  only because the reciprocals of the fiber size sum to exactly 1 over a pair-order
  class, and that holds only when the walk can reach **every** member of the
  fiber. A strict walk prunes orientation candidates in-walk while the
  divisor still counts the unrestricted fiber, so each strict class contributes
  less than 1 and every `records_*` figure undercounts it — measured on the
  shipped witness as a 537.6× per-class undercount. Before 2026-09-04 this
  combination ran with no refusal at all; no published figure came from it.
* `FIBER_MODE=REFUSED-C5-BUDGET` — refused because a custom C5 budget was set.
  The fiber DP's within-budget semantics are validated only for the standard
  and RELAX_C5 budgets, so an arbitrary budget would be an unvalidated estimand
...
#### `solve_results.json` — the run summary schema (version `2.0`)

A single-VM run also writes `solve_results.json`. (In split enum/merge mode the enum VM never
reaches the writer, so its absence there is normal — see [DEPLOYMENT.md](DEPLOYMENT.md).) Every
rate below is a **percentage of `counts.c3_valid`**, not of `total_solutions`, and is `0` when
`c3_valid` is 0 — so a rate array of zeros can mean "no C3-valid solutions", not "no matches".

`run` — how the run was parameterised:

| key | what it holds |
|---|---|
| `start_utc` / `end_utc` | run window. `end_utc` is **computed as `start + elapsed_seconds`**, not read from the clock at write time |
| `time_limit_seconds` | the wall-clock limit that was in force; `0` = run to completion |
| `branches_total` | branches the run was decomposed into, against which `branches_completed` is read |
| `hash_table_initial_log2` | log2 of the dedup hash table's initial size — the parameter behind `counts.hash_dedup_collisions` |
| `cpu_cores` | online cores on the host (`_SC_NPROCESSORS_ONLN`), which is **not** the same as `run.threads` and is recorded so the two can be compared |
| `resumed_branches` | branches satisfied from `checkpoint.txt` rather than walked in this process. A non-zero value means this file summarises a resumed run |

`counts` — what the search found:

| key | what it holds |
|---|---|
| `nodes_explored` | total DFS nodes across all threads |
| `total_solutions` | complete orderings found, **before** the C3 filter |
| `c3_valid` | those satisfying C3. This is the denominator of every rate in the file |
| `unique_orderings` | distinct orderings after dedup — also what `output.solutions_bin_records` reports |
| `king_wen_found` | whether the King Wen sequence itself was among them |
| `hash_dedup_collisions` | hash collisions during dedup. Diagnostic for the table sizing above, not a correctness signal |

`position_match`, `adjacency`, `complement_distance`, `super_pairs`, `pair_frequency` — the
comparison of what was found against the King Wen sequence:

| key | what it holds |
|---|---|
| `position_match` | `.rates[32]` percentages and `.counts[32]` raw counts of solutions agreeing with King Wen at each position |
| `edit_distance` | `.histogram` (distance → count, zero buckets omitted) and `.closest[]`, the top solutions by distance. Each `closest` entry carries `diff_positions` — the positions where it differs, emitted **1-based**, unlike every other position index in this file — and the full 64-element `sequence` |
| `adjacency.per_boundary_rates` | 31 percentages, one per adjacent pair boundary |
| `adjacency.match_histogram` | number-of-matching-boundaries → count, zero buckets omitted |
| `adjacency.c6_satisfied` / `c7_satisfied` / `c6_and_c7_satisfied` | counts satisfying C6, C7, and both. The joint count is emitted separately because it is **not** derivable from the two margins |
| `complement_distance.kw_value_x64` | the King Wen complement distance as an **exact integer scaled by 64** — the value to compare against, since it carries no rounding |
| `complement_distance.kw_value` | the same number divided by 64 and printed to 4 decimals. Convenience only; prefer `kw_value_x64` for any equality test |
| `super_pairs.count` | how many super-pairs the run used; `.match_rates[32]` and `.mapping[32]` accompany it |
| `super_pairs.match_rates` | per-position super-pair agreement rates |
| `pair_frequency[32][]` | per position, up to the **top 3** pairs by frequency: `pair` (emitted **1-based**), `count`, `pct`, and `n_different` — the number of *distinct* pairs seen at that position, which is a property of the position and is therefore repeated identically in all three of its entries |

`output` — the bytes the run produced:

| key | what it holds |
|---|---|
| `solutions_bin_records` | records written; equals `counts.unique_orderings` |
| `solutions_bin_record_size` | bytes per record (`SOL_RECORD_SIZE`) |
| `SOLBIN_BYTES` | a **schema marker**, not a size: the constant string `LOGICAL-VERIFIED`, asserting that `solutions_bin_bytes` in *this* file holds the logical size (header + records). Until 2026-09-04 that key held the record-stream size while its name claimed the file size — always exactly `solutions_bin_header_size` short. The marker is how a consumer tells an archived results file's convention apart without guessing; see [CORRECTIONS.md](CORRECTIONS.md) |
...
beside the expected sha it echoed back. It attests one narrow thing: the
comparison is made against a **case-normalised** copy of the digest you passed,
so an upper-case or mixed-case argument is not a mismatch. `lowercase` is the
only value; the token exists because the alternative — a silent normalisation —
turns "your sha was typed in caps" into "your build does not reproduce", which
is the most expensive wrong answer this gate can give.
...
> ✅ **Output-honesty fixes landed 2026-09-04** (Codex v2 `solve.c@8073`, `@8136`, `@8137/@8093/@8149`,
> `@8341`, `@8343` — the adjudication's own finding identifiers, written with `@` because they are not line numbers in this tree). All are estimator-only and sha-neutral.
>
...
> lines 21451, 21063, 21456, 21065 and 21359, which now land in unrelated KC walk and RNG
> code (line 21451 was then a `memcmp`, line 21359 a `kc_splitmix64` call). `solve.c` grew by roughly 16,500
...
**Finalize-durability marker, and the digest the adopt path now compares
(⚠ CONTRACT CHANGE, 2026-09-10).** A layer is durable (rename + dir-fsync)
before the multi-hour catalog-sidecar pass that follows it, but the manifest —
the only thing the resume loader reads — advances only *after* that pass. An
eviction in that window used to discard a finished layer and force a complete
re-sweep (~44 h on full-31 layer 17, 2026-08-05). The `.finalized` marker
closes the window: it is written atomically right after the layer file is
durable, and a resume that finds one ADOPTS the layer instead of re-sweeping
it. It records the layer's decompressed-stream sha256 — the same digest
`--f1c5-layer-sha` prints — and so does the append-only
`<pfx>_layer_sha.ledger`.

Until 2026-09-10 the adopt path read that digest, **printed** it, and never
compared it; this document said so, deliberately, in the
`SOLVE_F1_FINALIZE_SHA` row ("adoption never depends on the digest"). That was
design-as-written, and it was wrong. What it costs was measured, not argued:
flip one byte of a finalized n=9 f layer 1 (byte 100, the low byte of
`off[2]`), resume, and the build adopts the altered layer, logs the ORIGINAL
digest for a file that no longer has it, and **exits 0 with
total = 18,768 instead of 26,112**. Nothing structural can see it — a bumped
offset is still monotone with the right endpoints, so the header check, the
byte-size check and `f1c5_v2_index_load` all pass. Neither can `--kc-g-check`
on a large fraction of instances: it merge-joins the f and g spans by key, so a
displaced entry whose key has no partner in the other ladder's span contributes
nothing and the cut identity still holds (measured: 3 of 4 sampled f-layer-5
displacements were silent, with the correct total and `KC_SCAN=OK`
downstream). Neither can `--kc-ladder-verify` when the eviction lands during
the sidecar pass — the window the marker exists for — because the resume
regenerates the sidecar from the altered bytes and the sidecar then agrees with
them.
...
**and** `t_layer_NN.bin` files in layer order; a layer file that exists but
cannot be read is an error (`ERROR: cannot open <file>`, exit 2), never skipped.
⚠ *(corrected 2026-09-25, Q-782: until then the DIR form tested each name with
`access(R_OK)` and silently SKIPPED an unreadable layer, exiting 0 on the rest,
which contradicted the exit-status sentence below; digest lines for readable
layers are unchanged.)* The digest itself is computed by the
...
alone. Beyond the identity fields (`type`, `version`, `label`, `arrangement`,
`engine_git`, `engine_source_sha`) and the five booleans `c1`…`c5`:

| key | what it asserts |
|---|---|
| `check_order` | the literal string `C1,C2,C3,C4,C5 (pinned)`. It is a constant, and that is the point: `first_violation` below is only meaningful against a fixed order, so the order is written into the artifact instead of being assumed by its reader |
| `perm_ok` | 1 iff the 64 values are a permutation of 0..63 — each in range and none repeated. 🔴 **It is checked first and it short-circuits.** When it is 0 the constraint booleans are all left at 0 and `first_violation` reads `C1`, because an arrangement that is not a permutation is "not even C1-checkable". Do not read `c2: 0` beside `perm_ok: 0` as a measured C2 verdict |
| `c3_value` | the C3 statistic itself: the sum over all 64 hexagrams of the distance between a hexagram's position and its complement's, `SUM_v abs(pos(v) − pos(v XOR 63))`. The boolean `c3` is exactly `c3_value <= 776`, so the certificate publishes the margin, not just the verdict — King Wen sits at 776, the ceiling |
| `c2_d5_count` | how many of the 63 adjacent boundaries have Hamming distance exactly 5. C2 is "no 5-line transition", so `c2` is `c2_d5_count == 0` and this key says how far from satisfied a failing arrangement is |
| `dist_hist` | the full boundary-distance histogram, seven counts for d = 0..6 over the 63 adjacent pairs. C5 is `dist_hist` equalling King Wen's multiset, which is `[0,2,20,13,19,0,9]` — re-derived from the built-in King Wen array on every run and hard-checked against that literal, so a drift in the derivation aborts rather than silently regrading |
| `first_violation` | the name of the first constraint that failed under the pinned order, or `NONE` when all five hold. One string, not a set: it is the *earliest* failure, and a certificate reporting `C2` says nothing about C3, C4 or C5, all of which were still evaluated and appear as their own booleans |
| `verdict_super` | `IN` or `OUT` for the walk **superspace**, C1 & C2 & C4 & C5 — deliberately without C3. This is the space the enumeration walks, so an arrangement can be `verdict_super: IN` and still be outside the constrained set |
| `verdict_c15` | `IN` or `OUT` for C15 = all five, i.e. `verdict_super` and `c3` together. This is the one the exit code follows: 0 for `IN`, 1 for `OUT` |
| `space_labels` | the literal `SUPER=C1&C2&C4&C5 (walk superspace); C15=C1-C5 (C3<=776)`. It exists so that the two verdicts above cannot be quoted without their definitions travelling with them |
...
Each of these is a whole line, to be consumed with `grep -qx` and never by matching a substring
or the `VERDICT:` prose line beside it. Every one of them distinguishes **"checked and agreed"**
from **"checked nothing"**, which is the distinction a two-valued PASS/FAIL token cannot carry.

| token | emitted by | values |
|---|---|---|
| `CERT_DIGEST` | `--verify-certificate` | `MATCHED` every recorded file digest in the certificate is well-formed and equals the digest re-derived from the file now. `MISMATCH` all digests are well-formed and at least one differs. `UNAVAILABLE` at least one side of at least one comparison is **not a well-formed sha** — a sentinel such as `unknown`, or an absent field — so that comparison proved nothing. `UNAVAILABLE` takes precedence over `MISMATCH`, and it is what stops `--kc-mutate` from reporting "all mutations caught" over a sentinel-vs-sentinel comparison: a mutation battery can prove the comparison is *wired*, never that the compared value was *meaningful* |
| `KC_LADDER_IDENTITIES` | `--kc-ladder-verify` | an integer: how many f\*g cut identities were **evaluated**. In practice `0` or `n+1` — it is set to `n+1` when a `GDIR` was supplied and `kc_g_check` actually ran, and left at `0` when no `GDIR` was given or the g ladder could not be opened. It is a census of what ran, not a tally of individual comparisons |
| `KC_LADDER_RESULT` | `--kc-ladder-verify` | `FAIL` at least one failure. `STRUCTURE-ONLY` zero failures but `KC_LADDER_IDENTITIES=0`: per-layer decompressed-stream shas, layer masses, the layer-stats sidecars, the boundary identities (f layer-`n` orbit mass == total; `g(0)` == total) and the hash chain were all checked, and **the f\*g cut identity — the V3 gate — was not run at all**. `PASS` zero failures **and** at least one identity evaluated. Read `STRUCTURE-ONLY` as "supply a `GDIR`", not as a pass |
| `KC_AR2_CHECKS` | `--kc-ar2` | an integer: how many battery checks executed. The witness leg needs a walk — given as argv, or the built-in King Wen walk when `n == 31` — and the exhaustive leg runs only when the space is small enough (`total <= 2^22`), so with neither available this is legitimately `0` |
| `KC_AR2_RESULT` | `--kc-ar2` | `FAIL` at least one check failed. `VACUOUS` zero failures out of **zero checks executed** — no witness walk and a space too large for the exhaustive leg, so nothing was verified. `PASS` zero failures and at least one check executed |

`STRUCTURE-ONLY` and `VACUOUS` exist for the same reason and were added by the same review class:
a gate that measured nothing must never be readable as agreement. Do not map either onto `PASS`
in a consuming script.

**Keys of the two H-tier certificate objects that a reader most often has to
interpret.** Both also carry the ladder identity (`n`, `N_total`, `pl_hash`,
`fdir`, `gdir`, `engine_git`, `engine_source_sha`).

`roae-h1-oracle-certificate` (from `--kc-oracle`):

| key | what it means |
|---|---|
| `n_files` | how many `BIN` inputs were streamed. Each contributes its own `file_<i>_*` block — path, stream sha256, header count, records, members, and the six defect tallies (`invalid`, `f0`, `malformed`, `dupclass`, `orderviol`, `c3_over`, plus `repr_mismatch` under `--kc-oracle-repr`). A verdict over zero files is not a pass, and this is the key that says so |
| `membership_semantics` | the literal definition the `members` tallies were computed against: *member ⟺ forward-valid AND every prefix state has f ≥ 1 (H1)*. Both halves matter — forward validity alone admits a prefix the ladder shows is dead — and writing it into the artifact is what lets a `members` count be compared across engine versions rather than assumed to mean the same thing |

`roae-h3b-rank-certificate` (from `--kc-o3-cert`). The four `neighbor_*` keys
are the **bracket**: they publish the immediate predecessor and successor of
the ranked walk under the O3 order, so a verifier can confirm the rank is
positioned correctly rather than merely self-consistent.

| key | what it means, and what re-verification checks |
|---|---|
| `neighbor_prev_rank` | the decimal `rank3 − 1`, or the string `NONE` at rank 0. `--verify-certificate` checks three separate things about it: that it really is `rank3 − 1` as decimal arithmetic; that unranking it reproduces `neighbor_prev_walk`; and that re-ranking **that** walk returns the same number — an unrank/rank round trip, not a restatement |
| `neighbor_prev_walk` | the walk at that rank, in the standard `entry,exit,…` form, or `NONE`. Re-verification additionally requires the independent comparator to place it **strictly before** the certified walk, so a bracket that unranks consistently but orders wrongly still fails |
| `neighbor_next_rank` | the same for `rank3 + 1`, or `NONE` when the certified walk is the last one (`rank3 + 1 == N_total`) |
| `neighbor_next_walk` | the same, and required to compare strictly **after** the certified walk |
| `class_rank_note` | a standing caveat, not a measurement: the certified `rank3` is a **walk** rank. The class rank — the number of distinct *records* preceding — is **not computed**; what the certificate gives instead is the class block `[class_first_rank3, class_first_rank3 + m)`. Quoting `rank3` as a record rank is the error this key exists to prevent |
| `c15_note` | the second standing caveat, and it is about **cost, not computability**: an exact **C15** rank/count was **priced and permanently declined** ([TR-12](../reports/TR12_QUERY_PROGRAM.md) §9), not ruled out. Every rank in this certificate is over the C1&C2&C4&C5 superspace, and any C15 figure elsewhere in the corpus is a labelled estimate. ⚠ **[CORRECTED 2026-09-19 — this read "a **C15** rank is not exactly computable (the C3 obstruction, TR-11 §10(ii))". That states a purchasing decision as a limit in principle, and it cites §10(ii) for a framing that section itself withdrew: [TR-11](../reports/TR11_EXACT_COUNTING_BY_SYMMETRY_QUOTIENT.md) §10(ii) **v1.5 (2026-07-21)** replaced "open structural obstruction … no feasible exact design in hand" with "the remaining barrier is footprint **cost**, not structure", on the machine-checked identity C3 = 16 + 8·G ([`lean/C3Decomposition.lean`](../lean/C3Decomposition.lean), `c3_slot_decomposition`), which lets a DP carry C3 as a bounded scalar; the bounded-state instrument that does so is built and documented above as `--f1-c3-hist --with-c5`. The 2026-09-05 pass that corrected this same claim at this file's §`--kc-profile` site and in [VERIFY.md](VERIFY.md) did not reach this row. **No dollar figure replaces the withdrawn claim, deliberately:** TR-11 **v1.10 (2026-07-22)** withdrew its own §10(ii) cost band, and TR-12 §9 redacts rather than restates the figures, per [CAMPAIGN_METHODOLOGY.md](CAMPAIGN_METHODOLOGY.md) §7 rule 9. Nothing an instrument reaches changed: the f/g/t ladders still carry no C3 channel, and the C15 population count remains the TR-4 estimate.]** |
...
**The atlas `gates` object — what it advertises, and what it does not.** `kc_h_scan_tail` runs
**twelve** distinct gate families; the JSON advertises **seven** of them (four until 2026-09-08).
The seven, in emission order, with the invariant each asserts:

| key | invariant asserted |
|---|---|
| `per_layer_flow_eq_N` | `flow[k] == N` at every layer `k` in `[0, n)` |
| `raw_marginal_sums_eq_N` | `sum over pairs of rawmarg[k][pair] == N` at every layer (`"not-emitted"` without `--kc-raw`) |
| `class_row_sums_eq_N` | **(2026-09-08)** `sum over the five distance classes of cls[k][d] == N` at every layer — the class row closes |
| `quotient_marginal_sums_eq_N` | **(2026-09-08)** `sum over quotient-frame slots of qmarg[k][q] == N` at every layer — the quotient row closes |
| `class_column_sums_eq_b0_N` | **(2026-09-08)** `sum over layers of cls[k][d] == b0[d] * N` for each distance class `d` — the class COLUMN closes |
| `branch_masses_sum_eq_N` | the branch atlas' per-branch masses sum to `N` |
| `t_root_eq_f_layer_sum` | `t(root) == sum_k fmass[k]` — the cross-chunk identity; `"not-run (requires --kc-tdir)"` when there is no t ladder |
| `entries_eq_header_ne`, `count_identities`, `outdeg_identities`, `hist_bounds_and_sums`, `digit_row_sums_eq_N`, `extrema_relookup`, `kernel_marginals_eq_cls_raw` | the logging tables' chunk-local gates (2026-09-11; see "The logging tables" under `--kc-layers`), re-run in the tail; `kernel_marginals_eq_cls_raw` is `"not-emitted"` without the raw frame |
| `tail_checks` (a sibling object, not a `gates` key) | `vertical_raw_eq_N`, `digit_cross_table_eq_cls_prefix`, `kernel_cross_layer_eq`, `kernel_rev_column_eq`, `kernel_g_invariance`: `"PASS"` / `"FAIL"` / `"n/a"` / `"not-run"`, and `fails`. Report-only unless `SOLVE_KC_SCAN_TAIL_STRICT=1` (the F3 rule) |

Each of the three new keys is gated against `fkc->total`, **not** against `flow[k]`, so it stays
meaningful when `flow` is itself the tampered value. Until 2026-09-08 the two row invariants were
checked only at n=9 inside `--kc-scan-selftest` and **nowhere in the path that attests full-31**;
the column invariant did not exist.

🔴 **Seven is not completeness, and the object must not be read as a census.** Five further gate
families run in the tail and reach the JSON only through `fails`:
`t-recursion prefix total != f-ladder layer masses`, `branch <i> leaves != g mass`,
`branch <i> t == 0 (t-ladder domain defect)`, `1 + sum of branch t != t(root)`, and
`t-ladder != small-n direct recursion`. Three of those five — the first, the second and the last —
are guarded by the direct t recursion, which the tail attempts only when `N <= 2^27`, so **at n=31
they do not run at all**; the other two run whenever `--kc-tdir` is given. Any failure among the
twelve increments `gate_fails`, and `gate_fails` is what `fails` reports. The object is therefore
**coarse but never false-positive**: by the emitter's standing convention every advertised field
collapses to the string `"see fails"` the moment *any* gate failed, including one it does not name.
Read `fails` first; the seven keys say which invariants were *asserted*, not how many exist.

🔴 **There is deliberately NO vertical `quotient_marginal` gate, and adding one would be
mathematically wrong.** The symmetric-looking counterpart `sum over layers of qmarg[k][q]` has no
closed form. `kc_glookup` reaches the g ladder through `kc_flookup`, which calls `f1_canon` on
**every** lookup, so `q` is a bit index into *this* layer's canonical mask and does not denote the
same slot at layer `k+1`; summing a quotient marginal down the layer axis adds together labels
drawn from different frames. The class axis has no such problem — `d` is a distance class, the
same object at every layer, and `kc_finish_init` asserts `sum_d b0[d] == n` on every ladder-open
path — which is why `class_column_sums_eq_b0_N` exists and its quotient counterpart does not.
Do not "complete the set."

**Chunk mode runs the two ROW gates and not the vertical one.** A `--kc-layers A B` chunk holds
only `[A, B)`, and a sum over a partial `k` range is not `b0[d] * N`. The column gate runs once, in
the tail, over the assembled table — which the merge reaches only after proving
`KC_SCAN_MERGE_COVERAGE=COMPLETE`. It is also the **only** gate in the set that can see a
mass-preserving *rearrangement*: swapping `cls[k][d1]` with `cls[k][d2]` inside one layer row
leaves `flow[k]` and both row sums untouched, so every row gate is blind to it. `--kc-layers-selftest`
legs R1, R2 and R3 show the three new gates able to fail, R3 by exactly that swap.
...
f and g ladders' combined **footprint** as then estimated; ⚠ *Q-725, 2026-09-24: the measured footprint is 3.29 TB + 8.27 TB = **11.57 TB**, per TR-12 §R.0's measured table, so even that estimate was low*) divided by a measured rate. The assumption is wrong, and it
...
attributable as they are:

| chunk key | what it records |
|---|---|
| `f_layer_sha_NN` (`f_layer_sha_00`, `f_layer_sha_01`, …) | the decompressed-stream sha256 of `f1c5_layer_NN.bin` as this chunk read it, one per layer in the chunk's own `[k_lo, k_hi)` range |
| `g_layer_sha_NN` (`g_layer_sha_01`, …, `g_layer_sha_04`, …) | the same digest for `g_layer_NN.bin`. A transition layer `k` reads g layer `k+1`, so the g indices are shifted by one and a chunk covering `[0,4)` records `g_layer_sha_01` through `g_layer_sha_04` |
| `engine_exe_sha` | the sha256 of the **running executable** that produced the chunk, read at run time from `/proc/self/exe` — not the source digest. It is what makes `KC_SCAN_MERGE_ENGINE=EXE-BOUND` possible, and it is what a build carrying `SOURCE_SHA=unknown` still has |
| `elapsed_sec` | wall seconds this chunk spent in the layer pass. Diagnostic and load-balancing only; no gate reads it, and it is one of the few fields that legitimately differs between two byte-equal reruns |

Chunk **schema 3** (2026-09-11, the scan LOGGING build; `"version": 3`, and the merge refuses any
other version through its identity leg) adds three things. A schema-2 chunk is void anyway: a new
binary changes `engine_exe_sha`.

| chunk key | what it records |
|---|---|
| `row_sha_NN` (one per layer in the range, emitted alongside `fmass_NN`, **before** `layers`) | the per-row content digest (build list L1): sha256 of a separated fixed-width header `k ‖ f_layer_sha_k ‖ g_layer_sha_{k+1} ‖ engine_exe_sha ‖ fmass_k` (each followed by `\n`) and then **exactly** the row's bytes from its opening `{` to its closing `}` — the bytes `kc_h_chunk_row` extracts, with the writer's trailing comma stripped. Minted from the in-memory table at write time. The merge recomputes it over the row it extracted, with the **chunk's own** `engine_exe_sha` (so a `SOURCE-BOUND` merge across two executables of one source still passes), and refuses on mismatch with `KC_SCAN_MERGE_ROWBIND=MISMATCH:k=NN` — the first failing `k` ascending in the first failing chunk in argv order — leaving no atlas. A byte-preserving textual swap of two `by_class` objects inside a chunk, a parser offset, a compensating pair of `fmass` edits: all refused here, before any cell is parsed. Rows are single lines; the writer asserts that and that each row carries exactly one `"flow":` and one `"marginal_quotient":` |
| `scan_threads` | the team size, now emitted for **every** chunk (schema 2 emitted it only for `T ≠ 1`, to keep a default chunk byte-identical to the pre-parallel writer; schema 3 spends that property). Still outside the identity set: chunks scanned at different `T` merge |
| `telemetry` | one object, after the identity fields and outside `KC_MERGE_ID`, never in the atlas (L9). Every key is prefixed `tm_` and is unique at its nesting level; every string passes through the JSON escaper; anything unavailable is `null`; no host name is recorded. Contents: `tm_t_start_utc`/`tm_t_end_utc`, `tm_scan_sec`, `tm_sec_digest_f`/`tm_sec_digest_g` (the leg-3 digest walls — design open item O2 measured for free), `tm_rows_sec`; `tm_config` (`tm_scan_threads_requested`, `tm_gcache_mb_per_thread`, `tm_cache_mb`, `tm_force_ooc`, `tm_unit_entries`, `tm_omp_schedule`, `tm_g_mode`, `tm_g_slots`, `tm_tables`, `tm_stats`, and `tm_g_counter_semantics`: the g counters are **block accesses** — every key/value probe of the binary search calls `kc_ooc_block`, so `tm_g_misses / tm_lookups` is the miss rate per lookup and `tm_g_hits` is per probe); `tm_host` (`tm_affinity_cpus`, `tm_rlimit_as`, `tm_rlimit_nofile`, `tm_uname_release`, `tm_uname_machine`, `tm_cpu_model`, `tm_nproc`, `tm_mem_total`, `tm_zlib_version`, `tm_compiler`, `tm_openmp`, `tm_glibc`, `tm_argv` as an array, `tm_env` = every `SOLVE_*` / `OMP_*` / `GOMP_*` variable as found); `tm_files` (`tm_f_layer_bytes_NN`, `tm_g_layer_bytes_NN` by `stat()`); `tm_sidecars` (`tm_f_sidecar_sha_NN`, `tm_g_sidecar_sha_NN`: the ladders' `own_sha256_decompressed`, `null` on rehearsal ladders); and `tm_layers`, one object per layer (`tm_k`, `tm_filled`, `tm_granted` — the team size actually granted — `tm_nm`, `tm_ne`, `tm_nblk`, `tm_units`, `tm_unit_entries_used`, `tm_dyn_chunk`, `tm_layer_wall_sec`, `tm_reduce_sec`, `tm_layer_cpu_user_sec`/`tm_layer_cpu_sys_sec`, `tm_rss_mb`/`tm_peak_rss_mb`, `tm_layer_io_read_bytes`) each with `tm_threads`, one object per thread (`tm_tid`, `tm_items`, `tm_entries`, `tm_lookups`, `tm_g_hits`, `tm_g_misses`, `tm_g_bytes_read`, `tm_g_sec_read`, `tm_g_sec_inflate`, `tm_g_max_inflate_sec` with its witness `tm_g_max_inflate_at_layer`/`tm_g_max_inflate_at_blk`, `tm_max_unit_sec` with `tm_max_unit_at_unit`/`tm_max_unit_at_blk`/`tm_max_unit_at_offset_sec`, `tm_f_blocks`, `tm_f_logical`, `tm_thr_wall_sec`, `tm_thr_cpu_user_sec`/`tm_thr_cpu_sys_sec`, `tm_thr_io_read_bytes`) |

Every layer row of a schema-3 chunk (and of an atlas at schema 2) also carries the logging tables
described under `--kc-scan` below (`counts`, `extrema`, `hist`, `digits`, `outdeg`, `kwrank`,
`rid_mass`, `kernel`), which the merge parses sequentially from their unique keys, **re-renders**
from the parsed table through the one row writer and requires to reproduce the extracted bytes
(a parser reading `d2` into `d1` passes the digest and the row sums; not this), and re-gates in the
tail. On `KC_SCAN_MERGE=OK` — and only then — the merge writes `<OUT>.merge_telemetry.json`
(`"type": "roae-kc-scan-merge-telemetry"`, `tm_schema`, `tm_t_end_utc`, `tm_merge_wall_sec`,
`tm_tail_sec`, `tm_row_digest_sec`, `tm_chunks` (how many were submitted), `tm_f_digest_sec` /
`tm_g_digest_sec` per layer, and `tm_manifest`: `tm_merger_exe_sha`, `tm_merger_git`,
`tm_atlas_sha256`, `tm_tdir`, `tm_t_root`, `tm_t_layer_sha_01`, and `tm_chunks_accepted`, one
`{tm_path, tm_k_lo, tm_k_hi, tm_sha256}` per accepted chunk in argv order). It is a sidecar, never
an atlas field, so "no atlas after a non-OK exit" keeps meaning "no output". The chunk's own
`telemetry` object likewise opens with `tm_schema`.
...
🔴 **Three further merge tokens are printed only on the ABORT path, and one only on the success
path — so a consumer that expects the same five lines every run will misread the output.**

| token | printed when | values, and what each actually means |
|---|---|---|
| `KC_SCAN_MERGE_IDENTITY` | abort path **only** (`COVERAGE=ABORTED`) | `MISMATCH` if any chunk disagreed with the merger on `type`, `version`, `N_total`, `pl_hash`, `b0`, `fdir`, `gdir`, `engine_git`, `n`, `start_exit`, `want_raw`, or on engine identity. `OK` means the abort was caused by something else — a ladder-digest mismatch or an unparseable chunk — **not** that the merge succeeded. There is no `KC_SCAN_MERGE_IDENTITY` line at all on a successful merge |
...
compiler, flags and libc, which the source sha does not cover. A sentinel (`unknown`, absent,
malformed) never matches on either axis — before 2026-09-04 the comparison was `"unknown"` against
`"unknown"`, and chunks from two different binaries merged into a fully-gated but wrong atlas.
...
`--kc-scan-selftest` is the separate reduced-n battery for the **unchunked** `--kc-scan`
path (`kc_scan_selftest_main`): positive legs plus negative legs N1/N2 that flip a byte inside an
f layer and require the scan to fail closed, then restore it and require the scan to pass again.
It prints one `PASS`/`FAIL` line per leg and ends with `KC_SCAN_SELFTEST=PASS|FAIL`, where `FAIL`
means at least one leg failed; exit **0** / **1**. It is argv-dispatched only and is never reached
from `--selftest`.

#### Parallel `--kc-scan` (`--kc-scan-threads`, added 2026-09-11)

```
solve --kc-scan FDIR GDIR OUT.json [...] [--kc-scan-threads T] [--kc-gcache-mb-per-thread C]
solve --kc-scan-par-selftest
```

`--kc-scan-threads T` (or `SOLVE_KC_SCAN_THREADS`; the flag wins; **default 1**) runs the layer pass
as an OpenMP team of `T` threads **inside each f layer**. The layer's entries are cut into
contiguous work units (one v2 block of 65,536 entries by default) handed out with
`schedule(dynamic, CH)`, `CH ≈ units/(8·T)`. Each thread seeks its own two f streams to its unit,
finds the unit's first mask by binary search on the offset table, rebuilds that mask's orbit
expansion, and accumulates into its own zeroed table. The master reduces the thread tables into
the layer row by exact 192-bit addition after the team joins, and only then prints the per-layer
`[kc-scan] layer …` line, so stdout is unchanged. **The atlas and every chunk are byte-identical at
...
(fds and index arrays) is shared read-only, and the slots, bucket table, inflate scratch and
counters are the thread's own. `C` sizes each clone. The default is the g reader's own size
(`--kc-cache-mb`, else `SOLVE_KC_CACHE_MB`, else 2048 MB), so **at `T = 64` pass both a small
`--kc-cache-mb` and an explicit `--kc-gcache-mb-per-thread`**. Resident memory is roughly
`--kc-cache-mb + T·C + indexes + T·(two f-stream index copies)`. The f and t readers are always
opened with the minimal cache (the 4-slot floor): the scan streams f by path and makes about `2n`
t lookups. An in-memory g needs no clone; the threads share it read-only.

Every chunk carries an informational `"scan_threads": T` next to `elapsed_sec` (schema 3; schema 2
emitted it only at `T ≠ 1`). It sits **outside** the merge's identity set, so chunks scanned at
different `T` merge.

**The logging tables (2026-09-11; build list `SCAN_LOGGING_BUILD_LIST_2026_09_11.md` REV2).** The
scan is the one expensive pass, and a quantity computable only inside it cannot be recovered without
paying for the pass again. Every layer row of the atlas (schema `"version": 2`) and of every chunk
therefore carries, after `marginal_raw`, eight further objects. All are exact, order-independent
sums or extrema under one pinned total order, so the atlas stays byte-identical at every thread
count; every key is unique at every nesting level and none is spelled like `flow`, `dN`, `qN` or
`pairN`, which the merge reads by first occurrence.

| row key | contents |
|---|---|
| `counts` | `st_entries` (f entries visited — gated equal to the layer header's `n_entries`), `st_lookups`, `st_pruned` (budget-pruned candidates), `st_zero_g`, `st_dead` (entries with no nonzero continuation), `st_dead_fmass`/`st_live_fmass` (their orbit-weighted f mass, accumulated independently, summing to `fmass[k]`), `st_nz1..st_nz6` and `st_nzo1..st_nzo6` (nonzero transitions per class, canonical and orbit-weighted). Gates: `st_pruned + st_lookups == 2(n−k)·st_entries`, `st_lookups == Σ st_nz + st_zero_g` |
| `extrema` | per class `ext1..ext6`, the `max` and `min` (nonzero) transition product `w = f·g` with its witness `x_w`, `x_cm`, `x_lastc`, `x_rid`, `x_q`, `x_o`, `x_qpair` (= `pl[q]`, quotient-frame), `x_fv`, `x_gv`; `null` when the class has no nonzero transition at this layer. Ties are broken by the smaller `(cm, key, q, o)`, the same order the cross-thread reduction uses. Gate (chunk write **and** tail): each factor re-looked-up on the ladders equals the witness's, `w == f·g`, and the witness is legal (`q ∉ cm`, class admissible, `popcount(cm) == k`, `rsum[rid] == k`, canonical `cm`) |
| `hist` | per class `hb1..hb6`, the magnitude histogram of `w` at bucket `lgB`, `B = floor(log2 w)`: `hc` (count), `ho` (Σ orbit size), `hw` (Σ w), `hwo` (Σ w·orbit). Zero buckets skipped. Gates: `hc·2^B ≤ hw ≤ hc·(2^{B+1}−1)` and `ho·2^B ≤ hwo ≤ ho·(2^{B+1}−1)`, computed in KCP4 E7's subtraction form — `lo = hc << B`, then `hw ≥ lo` and `hw − lo ≤ lo − hc` — which needs no multiply and so assumes no 192-bit headroom (a shift that would not fit fails the gate rather than aborting); `Σ_B hc == st_nzN`, `Σ_B ho == st_nzoN`, `Σ_B hwo == dN`. ⚠ *Corrected 2026-09-12 (Codex KCP5 #3, adjudicated by Fable): both gates previously computed the HALF-OPEN form `hc ≤ (hw >> B) < 2·hc`, which accepts `hw` up to `hc·2^{B+1} − 1` — a window of width `hc − 1` above the stated maximum. The bounds printed here were always the intended ones; the code now matches them.* |
| `digits` | per class `dg1..dg6`, `j0..j<b0[d]>`: walk mass through f entries whose residual records `j` class-d transitions already used (from the entry's admissible nonzero mass). Gates: each row sums to `N`; `j0 == N` at layer 0 |
| `outdeg` | `odC` = `{dc, df, dw}`: entries with exactly `C` nonzero admissible children, their f mass and the walk mass through them (`od0` always emitted). Gates: `od0` equals `st_dead`/`st_dead_fmass` and carries zero walk mass; `Σ dc == st_entries`, `Σ C·dc == Σ st_nz`, `Σ dw == N`, `Σ df == fmass[k]` |
| `kwrank` | the reference walk's step (`kw_src` = `KW` at n = 31, `O3-MIDPOINT` — the TR-12 battery's anchor `unrank_O3(floor(N/2))` — at every other n; `kw_exit`, `kw_cls`, `kw_w` = its own transition product at this layer, derived by the scan from its ladders and re-derived by the merge, which refuses a disagreeing row) and per class `kwr1..kwr6` the exact rank bins `lt`/`eq`/`gt` of every nonzero `w` against `kw_w`, each `{rc, ro, rw}` (count, Σ orbit, Σ w·orbit). Gates: the bins partition `st_nzN`/`st_nzoN`/`dN`; the reference walk's own transition sits in its class's `eq` bin (`rc ≥ 1`) |
| `rid_mass` | `r<rid>`: walk mass through f entries at residual `rid`, for the rids of this layer (`rsum[rid] == k`); the full residual joint (catalog asset B9), 6,048 cells at n = 31 in total across layers. Gates: the layer's rids sum to `N`; `r0 == N` at layer 0; every `digits` cell equals the matching marginal of `rid_mass` (two independent accumulations) |
| `kernel` | raw-frame only (with `marginal_raw`): `m<a>_<b>`, the raw transition kernel `M_k[a][b]` — orbit-expanded walk mass through the layer's transitions with raw exit hexagram `a` and raw entry hexagram `b`. Zero cells skipped. Gates: total `N`; class marginal `== dN`; pair marginal `== pairP` (a second expansion of the same map); mass only at admissible `(a, b)` and, at layer 0, only from `start_exit` |
...
chunk, because the merge refuses chunks from any other build. So the identities that are theorems
about the ensemble rather than by-construction identities of adjacent statements — `Σ_k
marginal_raw[k][p] == N` (Q-500), `Σ_j j·digits[k][d][j] == Σ_{k'<k} dN[k']`, the kernel's
cross-layer identity `Σ_b M_k[a][b] == Σ_{a'} M_{k−1}[a'][partner(a)]`, its rev-column identity
and its G-invariance — ship as **report-only** checks with their proofs attached
(`FABLE_SCAN_LOGGING_LIST_REVIEW_2026_09_11.md` §2) until they are green at n = 12/13 on v2
out-of-core ladders; `SOLVE_KC_SCAN_TAIL_STRICT=1` makes each failure a `GATE FAIL`, and the
n = 12/13 VM leg runs strict. Everything else above is chunk-local and refusing.

⚠ **Corrected 2026-09-12 (Codex KCP5 #1, adjudicated by Fable).** This paragraph read: *"The
consumer (`solve.py atlas_load`) re-sums every atlas-visible identity at load in all cases."*
**That was false.** `atlas_load` refused on `gates.fails` and validated the documented count
fields, but read **none** of the row tables this build added — not `tail_checks`, and not
`counts`, `extrema`, `hist`, `digits`, `outdeg`, `kwrank`, `rid_mass` or `kernel`. Combined
with report-only tail checks, an atlas could carry `tail_checks.fails ≥ 1` beside
...
**Instrumentation and measurement knobs** (stderr, `KEY=value`; all off by default because the
TR-12 battery captures `--kc-scan` stderr into a golden):

| variable | default | effect |
|---|---|---|
| `SOLVE_KC_SCAN_STATS` | `0` | `1` prints one `KC_SCAN_STAT k=… tid=…` line per (layer, thread) and one `KC_SCAN_LAYER_STAT k=…` line per layer: `items`, `entries`, `lookups` (g lookups issued), `g_hits`, `g_misses`, `g_bytes_read`, `g_sec_read`, `g_sec_inflate`, `f_blocks`, `f_logical`, `wall_sec`, `cpu_user_sec`/`cpu_sys_sec` (`getrusage`, per thread and per process), `io_read_bytes` (`/proc/thread-self/io` / `/proc/self/io`; `-1` = unavailable), and per layer `rss_mb`/`peak_rss_mb`, `g_mode` (`in-memory`/`shared`/`clone`), `g_slots`, `granted` threads |
| `SOLVE_KC_SCAN_START_BLOCK` | unset | **measurement run**: scan each layer from block `b` |
| `SOLVE_KC_SCAN_STOP_AFTER_BLOCKS` | unset | **measurement run**: scan at most `B` blocks per layer (in total across the team) |
| `SOLVE_KC_SCAN_BLOCK_STRIDE` | `1` | measurement only: take every `s`-th block (the design's interleave run M5). Refused without one of the two knobs above |
| `SOLVE_KC_SCAN_UNIT_ENTRIES` | `65536` | entries per work unit. A test lever: small units (e.g. `7`) make every n=9 layer, which is under one block, split across threads mid-mask. Results are identical for every value |
| `SOLVE_KC_SCAN_SELFKILL` | unset | **test hook** for the H-4 kill-and-resume drill: `team:K` makes the process `raise(SIGKILL)` inside the thread team at transition layer `K`, as soon as any thread finishes its first work unit of that layer while the rest of the team is mid-flight; `reduce:K` raises it inside the reduction, after the first thread slice of layer `K` has been added and before the next; `merge:K` (2026-09-11) is honoured by `--kc-scan-merge` only and raises it inside the merge's chunk loop after `K` chunks (`K ≥ 1`) have been consumed and before anything is written at OUT — an external `kill -9` never landed inside a merge that finishes in under a second. The scan parses `merge:K` and ignores it; the merge ignores `team:`/`reduce:`. External timing cannot land a signal inside a microsecond reduction. Prints one stderr line naming the kill point. Malformed values are refused (exit 2). Never set it in production |
| `SOLVE_KC_SCAN_TABLES` | `1` | `0` skips the logging tables' accumulations (build list L10) so their per-lookup cost can be measured against the same pass with them on. The table would be incomplete by construction, so this is a **measurement run**: whole layers, `KC_SCAN_CHUNK=MEASUREMENT`, no chunk and no atlas; `--kc-scan-selftest` refuses it like the other measurement knobs |
| `SOLVE_KC_SCAN_TAIL_STRICT` | `0` | `1` turns the five report-only tail checks (and the per-layer `kernel_g_invariance` check in chunk mode) into refusing gates — the n = 12/13 VM leg G18. Strict parse: anything else is refused (exit 2) |

Setting either measurement knob makes the run a **measurement**. It is a partial layer pass that
prints the stats lines and `KC_SCAN_CHUNK=MEASUREMENT`, exits 0, and **writes no chunk and no
atlas**: `OUT` is not opened, created or removed, and the tail and gates are skipped. A partial
table must never reach disk. A malformed value of any of these variables is refused (exit 2),
never read as its default. `--kc-scan-selftest` and the other in-process callers refuse the two
measurement knobs.

`--kc-scan-par-selftest` checks the reduction with thread values ≥ 2⁶⁴ and ≥ 2¹²⁸. Its expected
values are derived by hand, and two cells are also pinned to independently computed decimals. It
checks that the reduction is order-independent, that `(2¹⁹²−1)+1` aborts with exit 70 while a sum
reaching exactly `2¹⁹²−1` does not, and that a clone shares no mutable pointer with its source or
its siblings while sharing the read-only layer table. `kc_ooc_free` on a clone aborts. Finally it
runs the in-process layer table at `T ∈ {1,2,3,8}` with 7-entry units and 4-slot clones over
in-memory v1, out-of-core v1 and out-of-core v2 g readers; each must equal the `T=1` reference
exactly, and the source reader must be untouched by clone traffic. It ends
`KC_SCAN_PAR_SELFTEST=PASS|FAIL`; exit **0** / **1**. Design: `roae-private`
`KC_SCAN_PARALLEL_DESIGN_2026_09_11.md` (Claude Fable); implementation by Claude (Opus).
...
**The atlas's `frames` object, and the t-unit caveat.** Every number in the
atlas belongs to one of two frames, and the atlas states which rather than
leaving it to the reader:

| key | what it declares |
|---|---|
| `marginals_quotient` | the marginals reported by default are **canonical-quotient**: orbit-weighted, with pair labels that are quotient-frame identities and **not** raw pair identities. Two rows carrying the same label in this frame are the same orbit, not the same raw pair |
| `marginals_raw` | the raw-frame marginals, obtained by explicit G-expansion. 🔴 They are emitted only under `--kc-raw`; without that flag the key's value ends `— NOT EMITTED (pass --kc-raw)`, so the key is always present and its absence can never be mistaken for a zero. `flow`, by contrast, is RAW-exact in both modes, because the distance class is G-invariant |
| `t_units_note` | the standing scope statement for every `t_*` figure in the object: `t(s) = 1 + sum_c t(s.c)` counts search-tree size in **valid-prefix units** of the pruned DFS. Mapping those to `SOLVE_NODE_LIMIT` node-counter semantics is a *separate* convention, certified by `--kc-t-cert` (the XA(iii) / W0-D certificate) — and explicitly **not** claimed here. A t-unit figure quoted as a node count without that certificate is an unwarranted conversion |

### --kc-t-cert — the t-unit node-accounting certificate

Argument grammar: see the `--kc-t-cert` entry in the derived subcommand tables below.

**What it is.** `kc_t_cert_main` builds the f ladder and the t ladder from scratch at n=9 and
n=13, checks the t recurrence against an **independent** brute-force DFS — `kc_brute` /
`kc_t_brute_subtree`, which count one node per recursive visit and share no code with the
ladder — and writes a `roae-kc-t-node-convention-certificate` JSON. Exit **0** every gate held /
**1** one or more gate failed / **2** the output file could not be written. The run ends with a
`[kc-t-cert] VERDICT: PASS|FAIL (N gate failures)` **prose** line: there is no `KEY=value` token
here to `grep -qx`, so gate on the exit code or on `gates.fails` in the JSON.

**The `convention` object — what a t unit IS.** These fields are *declarations*, not
measurements: they pin the counting convention that the rest of the certificate is evidence for.

| key | what it pins |
|---|---|
| `unit` | one t unit = one valid oriented prefix = one search-tree node of the pruned DFS over the C1C2C4C5 **superspace** |
| `recurrence` | `t(s) = 1 + sum over valid oriented placements c of t(s.c)`, with `t = 1` at depth `n` |
| `root_counted` | `true` — the empty prefix is itself one node. This is the `1 +` in the recurrence, and it is what the two branch gates check rather than assume |
| `orientation_branching` | joint/explicit: one child edge is one **(pair, orientation)** placement; there is **no** separate orientation-only node level |
| `pair_branching` | and no pair-only intermediate node either — the two halves of a placement are never charged as two nodes |
| `leaf` | a full walk (depth `n`) counts **1** node, not 0 |
| `dead_ends_counted` | `true` — a prefix that reaches no completion still counts as a node |
| `space` | `C1C2C4C5-SUPERSPACE (no C3 filter)`; C3 does not prune anything counted here |
| `reference_dfs` | names `kc_brute_rec` as the comparison side and records that per-state subtrees are counted by the **same** code path, so the exhaustive leg is not comparing two dialects of one implementation |
| `solve_node_limit_mapping` | 🔴 explicitly **NOT CLAIMED HERE**. Nothing in this certificate maps a t unit onto the production enumerator's `SOLVE_NODE_LIMIT` counter; that mapping is the W0-D worker run's job, and this instrument is its input, not its answer |

**The measured fields.** `n9` and `n13` carry the numbers the gates were computed from, so a
reader can recompute each gate from the certificate alone rather than trusting `gates`.

| key | in | what it holds |
|---|---|---|
| `N_walks` | both | the number of full walks. ⚠ At n=9 this is the **literal `26112`** written into the emitter, not a value the run measured; at n=13 it is the ladder's own `total` as a decimal string. Only the n=13 field is evidence |
| `t_root` | both | `t(root)` read off the t ladder — `u64` at n=9, decimal string at n=13 |
| `brute_dfs_nodes` | n9 | total visits of the independent brute DFS over the same superspace; the right-hand side of `n9_t_root_eq_brute_nodes` |
| `nodes_by_depth` | n9 | an `n+1` element array: entry `k` is the number of DFS visits at depth `k`, tallied by a separate recursive walk that consults no ladder |
| `states_cross_checked_exhaustively` | n9 | how many t-ladder memo cells were individually re-derived against the brute DFS — the size of the exhaustive leg's evidence. It is **every** stored state, not a sample |
| `brute_subtree_nodes` | n9 branch rows | the brute DFS's node count for that one child subtree, beside the ladder's `t` for the same state |
| `global_pair` | branch rows | the placed pair's index in the **global** 0..31 pair labelling (`kc->c.pl[q]`), not its index within this ladder's `n` active pairs. The branch atlas emitted by `--kc-scan` uses the same key with the same meaning |
| `sum_f_layer_masses` | n13 | the f layer masses summed over all depths `0..n`; the right-hand side of `n13_t_root_eq_sum_masses` |
| `brute_depth_cap` | n13 | the deepest depth the n=13 brute recount actually reached. **Depths beyond it are unchecked**, and publishing the cap is how the certificate says so |
| `brute_cap_rule` | n13 | how that cap was chosen: `largest cap with cumulative visits <= 2e8`. `kc_t_pick_cap` also stops at the first layer mass too large for one 64-bit limb |
| `nodes_by_depth_to_cap` | n13 | the per-depth visit counts, `brute_depth_cap + 1` of them — the truncated counterpart of `nodes_by_depth` |

**The `gates` object — the seven invariants, and which size each runs at.** Four run at n=9 and
three at n=13; a single `--kc-t-cert` run performs all seven, and there is no flag that selects a
size. Each is `true`/`false`, and `fails` is the count of `false` ones — the same number the exit
code is derived from (`rc = fails ? 1 : 0`).

| key | n | invariant asserted |
|---|---|---|
| `n9_t_root_eq_brute_nodes` | 9 | `t(root)` read off the ladder fits in one 64-bit limb **and** equals the independent brute DFS's total visit count. The ladder's summary number and a from-scratch walk of the same space agree |
| `n9_depth_counter_eq_f_masses` | 9 | at **every** depth `k` in `0..n`, a third counter — a plain DFS that only tallies visits per depth and reads no ladder — equals the f layer mass `M9[k]` in 192-bit arithmetic, **and** those counts sum to the brute DFS total. This is a *distribution* check: a defect that moved nodes between depths without changing the total fails here and passes the gate above |
| `n9_exhaustive_per_state` | 9 | for **every** state stored in the t ladder — every (depth, mask, exit, residue) memo cell, `states_cross_checked_exhaustively` of them — the stored `t` fits in one limb and equals the brute DFS subtree node count re-derived for that same state. The only leg that checks the ladder's **interior** rather than its totals |
| `n9_branches` | 9 | over the root's admissible depth-1 children (one per surviving (pair, orientation) placement), each child's ladder `t` equals its brute subtree count, **and** `1 + sum of the children's t == t(root)`. The leading `1 +` is `root_counted: true` being verified, not assumed |
| `n13_t_root_eq_sum_masses` | 13 | `t(root) == sum over k of the f layer masses`, in 192-bit arithmetic. There is **no** brute reference at this size: this is the ladder's two independent summary paths — the t recurrence and the f layer masses — agreeing with each other |
| `n13_depth_limited_brute` | 13 | the brute per-depth visit counts equal the f layer masses for `k` in `0..brute_depth_cap` **only**. The same invariant as `n9_depth_counter_eq_f_masses`, truncated to the depths a ~2×10⁸-visit budget reaches; deeper layers are not checked by this gate |
| `n13_branches` | 13 | every depth-1 branch has a **non-zero** `t` (a zero would be a t-ladder domain defect, not merely a small subtree) **and** `1 + sum of the branches' t == t(root)`. Unlike `n9_branches` there is deliberately **no** per-branch brute cross-check at n=13 — those subtrees are far too large to walk |

🔴 **The two sizes carry different weight, and the object does not say so.** At n=9 the reference
is genuinely foreign code and the check is exhaustive over every memo cell; at n=13 nothing
outside the ladder confirms it except the recount to `brute_depth_cap`, and the remaining depths
rest on the ladder's internal agreement with itself. `"n13_t_root_eq_sum_masses": true` is a
consistency result, not a corroboration. Read the n=13 legs as spot checks — which is exactly
what the module header calls them — and read `brute_depth_cap` before quoting them.
...
`--kc-c3-max` is **rejected** with an explicit error: the KC f/g/t ladders carry
no running-G channel, so a C3-conditioned extremal sweep cannot be answered from
them. That is an **instrument** boundary, not a mathematical one — C3 *is*
DP-optimisable (running slot-gap sum G, range ±496, orbit-invariant, machine-checked
as `runningG_orbit_invariant` in `lean/PruneGInvariance.lean`, implemented as
`--f1-c3-hist --with-c5`), and the full-31 run of that DP was priced and then
permanently declined on cost (TR-12 §9). ⚠ **[CORRECTED 2026-09-11 (V3A-134#10)** —
this read "C3 is not DP-optimisable, only a monotone in-path prune", which the same
binary's `--f1-c3-hist` refutes.]
...
the same fields plus the ladder identity (`n`, `start_exit`, `N_total`, `pl_hash`,
`fdir`, `gdir`, `engine_git`, `engine_source_sha`) and `null_vs_g`, the
`--kc-gdir` cross-gate's verdict (`"CONSISTENT"`, `"INCONSISTENT"`, or `null`
when no `--kc-gdir` was given or the functional was refused). Output is a
certificate, not a proof. `solve.py --kc-x-recheck` refuses a certificate with
no `start_exit` (`FAIL-no-start-exit`), a `start_exit` other than the G-fixed
`0` or `63` (`FAIL-bad-start-exit`), a `null_vs_g` of `"INCONSISTENT"`
(`FAIL-null-vs-g`), and a non-null `gdir` with no `null_vs_g` (`FAIL-no-null-vs-g`).
Until 2026-09-24 the cross-gate's verdict was on stdout only, so a certificate
written after `KC_EXTREMAL_NULL_VS_G=INCONSISTENT` looked like a passing one
(Q-771).
...
`KC_EXTREMAL=OK` (KCQ03 #3). The token is also printed on the refused-functional path, so a
`--kc-json` requested for a non-invariant functional still says whether its (valueless)
...
non-invariant control to be refused with no value emitted; **K6b** pins the
invariance gate's *coverage* — the gate records a per-`g` verdict for every
frame map it visits into a vector the selftest pre-fills with a not-visited
sentinel, so a scan that stops at 23 of the 24 maps fails the row, and each
verdict is compared against one recomputed from `el[g].hmap` and the reference
weights (K6 alone could not see this: every registry row's first counterexample
is at `g <= 1`); **K7** records, as
...
surface, the certificate JSON, `list`, and the refusals — **K10** runs *both*
directions and **pins the number**: the whole `extreme_value` /
`constant_on_space` / `opposite_extreme` line, the `witness_value` line, and
the certificate's `direction`, `extreme_value` and `witness_value` are matched
against the brute extrema computed in-process, so a run that prints the
minimum under `max` with every token green now fails. It prints one
...
**Keys of that certificate that are not simply the printed values repeated:**

| key | what it means |
|---|---|
| `functional_class` | the registry's own class label for the functional — `edge-additive` for every v1 row, `edge-additive (alias of dclass:1)` for `graycode`, and `NON-INVARIANT (negative control)` for `posyang0`. It records **which DP the value came from**, and v1 supports only the edge-additive one: a class-(b) functional would need extra state and is not in this registry |
| `g_invariant` | the verdict of the pre-DP brute-force check described above, over all 24 frame maps × 64 last × 64 entry × n steps. `false` and the tool emits **no value at all** and exits 1, so a certificate carrying an `extreme_value` always carries `g_invariant: true` beside it — the keys `extreme_value`, `constant_on_space` and the whole witness block are written only inside the `true` branch |
...
```
solve --kc-dead-census FDIR GDIR|- [--kc-layers A B] [--kc-out OUT.tsv]
                              [--kc-local-max R] [--kc-ooc] [--kc-cache-mb MB]
solve --kc-dead-census-selftest
```

Atlas query **7** — *which constraint kills?* — of the 2026-09-21 atlas generation
pass. The atlas's `rid_mass` and `digits` tables carry **live** walk mass only, and its
`counts` table carries the dead f-mass per layer without a residual axis, so the
question cannot be answered from the atlas. This subcommand answers it from the
ladders in one **same-layer streaming pass** per layer (the `--kc-g-check` pattern:
one sequential read of the f and g entry lists, a two-pointer merge on the key-sorted
spans, no random access), so the late layers a full-31 census needs stream at
decompression speed.

A stored f-state `s = (k, cm, lastc, rid)` is **dead** (doomed) when `f(s) > 0` and
`g(s) == 0`: some prefix reaches it, no completion leaves it. An f entry with no g
entry reads `g == 0`, as does an explicit zero; both are counted (`g_absent`,
`g_zero`). For every dead state the census records its **local** situation, which
needs no lookup:

| column | meaning |
|---|---|
| `layer` | the transition layer *k* (a dead state at *k* has *k* pairs placed) |
| `kill` | `C2` — every candidate successor is a distance-5 entry (the C2 prune); `C5` — every candidate is C2- or C5-blocked with **no** C2 among them; `MIXED` — both C2- and C5-blocked candidates, nothing admissible; `DEEP` — at least one **admissible** candidate, every one of which is itself dead (`g(s) = Σ g(s∘c) = 0`): the death is decided further down |
| `c5_classes` | the classes among the C5-blocked candidates (`d1+d3`, or `-`) |
| `exhausted` | the classes whose residual digit sits **at** its budget `b0[d]` (literal: a zero-budget class reads exhausted from layer 0, which is why the n=9 fixture shows `d3+d6` everywhere; at n=31 every budget is positive) |
| `states` | canonical dead states in the cell (Σ over a layer = the atlas's `st_dead`) |
| `prefixes` | orbit-weighted f-mass `f(s)·orbit(cm)` = raw doomed prefixes at those states (Σ = `st_dead_fmass`) |
| `c2_blocked`, `c5_blocked`, `admissible` | the candidate-successor counts, weighted the same way (blocked **edges** per prefix, summed) |

The `#L` rows give per-layer totals — `dead_states`, `dead_prefixes`, `live_states`,
`live_prefixes`, `fmass`, `fresh_dead_prefixes` — and the `#range-*` rows the totals
over the requested range by kill type and by C5 class. **Fresh deaths** are exact from
two same-layer passes, no `k+1` lookup: every admissible extension of a dead prefix is
dead and every prefix has one parent, so
`fresh(k) = dead_prefixes(k) − Σ_{dead s at k−1} f(s)·orbit(cm)·admissible(s)`; the
column reads `-` when layer *k−1* is outside the range. `--kc-layers A B` is the
half-open range `[A, B) ⊆ [0, n)` (default all); `--kc-out` writes the same table to a
file, verdict tokens appended, and a failed run leaves no file.

**The g-free shortcut, proved and gated rather than assumed.** Row 7 of the
generation pass conjectured that at `k ≥ 27` the census "may be computable from the f
layer plus the local predicate, no g needed". It is, exactly: `g(s)` is the number of
completions of *s* under the same transition rule the brute DFS uses (Stage-G gate
GA4), so `g(s) > 0` iff an exhaustive search over the remaining `n−k` pairs reaches a
leaf. With `GDIR` given as `-` the census decides liveness by that search
(`kc_dead_alive_local`) and **refuses** any layer with more than `R` remaining pairs
(`--kc-local-max`, default 5: at most `5!·2⁵ = 3840` leaves per state). The selftest
requires the g-ladder census and the g-free census to agree **cell for cell** on every
layer both can serve, and the atlas-query run script cross-checks the two on the real
n=31 ladders at layers 27–30.

Output ends with `#provenance`, `KC_DEAD_CENSUS_IDENTITY=OK|FAIL` (the identity
`dead + live == fmass` at every layer) and `KC_DEAD_CENSUS=OK|FAIL`. Exit **0** ok /
**1** a failed identity or an f/g mask-list mismatch / **2** usage, ladder-open, a bad
range, or a refused g-free layer. Unknown options are rejected, never ignored.

`--kc-dead-census-selftest` is the reduced-n gate: it builds n=9 f and g ladders in a
scratch directory and checks the g-ladder census, **cell for cell**, against an
**independent** forward brute force over the whole prefix tree — one that decides death
by its own subtree leaf count and computes `exhausted`/`kill` from an explicit
per-class step histogram (no `rid` decoding, no ladder): D1 prefixes per cell, D2
distinct canonical states per cell, D3 fresh deaths per layer against the brute
"dead child of a live parent" count, D4 the g-free census against the g-ladder census
on every layer with ≤ 5 remaining pairs (and refusal beyond), D5 the identity at every
layer, D6 an argv leg through the real dispatcher. Two negatives prove the comparator
has power: N1 a corrupted census cell **is** caught; N2 a brute tally keyed **wrongly**
(exhausted at `b0−1`) is **rejected**. A planted source mutant with that wrong key
turns D1 red (measured 2026-09-24). Ends with `KC_DEAD_CENSUS_SELFTEST=PASS|FAIL`; exit
**0** / **1**; well under a second; argv-dispatched only and **never** reached from
`--selftest`.

### --kc-witness-walks

```
solve --kc-witness-walks FDIR GDIR ATLAS.json [--kc-out OUT.tsv]
                                [--kc-ooc] [--kc-cache-mb MB]
solve --kc-witness-walks-selftest
```

Atlas query **9** — *what do the most- and least-crowded corridors look like as
walks?* The atlas's L4 `extrema` table names, per transition layer *k* and distance
class *d*, the max-`w` and min-nonzero-`w` **transition** under the pinned total order
(`kc_h_ext_better`): a source state `(k, cm, lastc, rid)`, the pair `q` placed with
orientation `o`, and `w = f(src)·g(dst)`, the number of walks crossing that transition.
The witness is a state plus a step; the walks are not in the atlas. This realises
**one** walk per witness — **backward** through f exactly as `kc_enum_rec` descends
(the first predecessor whose `last` is an element of a pair still in the mask, whose
class is spendable, and whose `f > 0`; at most `2(j−1)` lookups per layer), then **the
witness step**, then **forward** through g taking the first admissible successor with
`g > 0` at every step (the first completion; at most `2(n−j)` lookups per layer).

Each walk is then **checked, not trusted**, and every check is a column: `realised`,
`valid` (`kc_validate`, forward semantics independent of the descent), `through` (its
layer-*k* state equals the witness **and** `E[k]` is the witness's exit), `f_ok`,
`g_ok`, `w_ok` (`f(src)`, `g(dst)` and their product re-looked-up and equal to the
atlas's `x_fv` / `x_gv` / `x_w`); a failing row fails the run. The witness fields are
read from the atlas JSON in place with the `kcj_*` reader `--kc-scan-merge` uses (each
layer row holds exactly one `"extrema"` block, in layer order; a `--kc-scan` chunk
works too). Duplicates are realised, not merged: one transition can be the witness of
several `(class, max|min)` cells, and each row names its own cell. The walk column is
the `e,x,…` form `--kc-profile` and `--check-arrangement` accept (prefix `63,0,` for the
latter).

Output ends with `#provenance`, `KC_WITNESS_WALKS_N=<rows>`,
`KC_WITNESS_WALKS_OK=<rows passing every check>` and `KC_WITNESS_WALKS=OK|FAIL`; with
`--kc-out` the same table plus tokens goes to a file, removed on FAIL. Exit **0** /
**1** any failing row / **2** usage, ladder-open or a malformed atlas. This is a
certificate about the ladders, not a claim about the corridors: TR-12 §9 already rules
that a minimum-mass corridor "distinguishes nothing by itself", and nothing here reopens
that.

`--kc-witness-walks-selftest` is the reduced-n gate: it builds n=9 f and g ladders and
a real atlas through `--kc-scan`'s own driver, then W0 the witness count equals an
independent count of `x_w` fields in the file, W1 every realised walk validates, W2
every one is a member of the exhaustive brute enumeration of all 26,112 walks, W3 every
one passes through its witness state **and** takes the witness step, W4 `f`, `g`, `w`
agree with the atlas, W5 an argv leg through the real dispatcher whose row count equals
the witness count. Two negatives: N1 a **valid** walk that deviates at the witness step
(another admissible `g > 0` successor, completed from there) is **rejected** with
`through=0`; N2 a corrupted walk (a repeated exit) is rejected as invalid. A planted
source mutant that realises through the flipped orientation turns W3 red (measured
2026-09-24). Ends with `KC_WITNESS_WALKS_SELFTEST=PASS|FAIL`; exit **0** / **1**;
under a second; argv-dispatched only and **never** reached from `--selftest`.
...
O(N) and O(1) seek cost in one sentence and named the gz canonical as its O(1) example.)*
...
first non-fixed pair is `(p1, o1)`. The fixed pair-0 (`63→0` at slot 0) per C4 is implicit.
...
below it ranges over the records of the file it was given and nothing else.**
A `solutions.bin` is a node-budgeted C1–C5 slice — a lower bound over a
reproducible slice, not the C1+C2 population — so a minimum, a maximum or a
uniqueness observed here is a property of that file, not a theorem. `FILE` is
the only value the token takes today; it exists so that a consumer reading the
output cannot silently promote a file-scoped extremum into a population claim.
...
neighbour at each step, which is a **non-uniform** sampler over *open*
Hamiltonian **paths** in Q₆ — cycle closure is not enforced. Every rate it
reports therefore bounds that sampler's own induced distribution and says
nothing about the uniform ~10²² Hamiltonian-**cycle** (true Gray-code) family.
`INDUCED` is the only value emitted; a run that ever sampled the cycle family
uniformly would need a different one, and until then the token is the standing
refusal to let this null be quoted as the Gray-code null.
...
| `SOLVE_PER_TASK_NODE_LIMIT` | `0` (off) | Per-task cap (depth-3 sub-branch granularity for parallel `--sub-branch`). ⚠ Row corrected 2026-09-01: read "derived". `static long long per_task_node_limit = 0;` — *"0 = off (preserves prior behavior + canonical shas)"* (solve.c:1568) |
...
| `SOLVE_DISK_MARKER` | `solutions.sha256` | Marker filename `--disk-precheck` looks for on the mount. Its presence is REPORTED, never treated as proof of disk identity; its size and first field are printed verbatim. ⚠ **[CORRECTED 2026-09-07 — this read "Custom marker filename for `--disk-precheck` to test free space against (otherwise a default probe path is used)", which was wrong twice: the default is not "(none)", and free space is never tested against the marker.]** |
| `SOLVE_DISK_MARKER_SHA` | (none) | 64-hex digest the marker file's first field must equal. UNSET (the default): the marker leg reports only, and a missing, empty, unreadable or non-64-hex marker is a WARN. SET: a mismatch, or a marker with no readable 64-hex first field, exits **5** — the same wrong-disk code the UUID leg uses. It mirrors `--disk-precheck`'s optional third positional argument: identity is reported by default and asserted only when the operator supplies the expected value. Added 2026-09-07, because the marker leg previously printed "present: PASS" for a bare `stat()` — a zero-byte file satisfied it — under a comment claiming it "proves the mount holds the canonical disk's contents". A `stat()` cannot prove that, and a solver-data disk was destroyed by a wrong-disk operation on 2026-05-06. |
...
| `SOLVE_F1_KEEP_LAYERS` | 0 | `--f1-exact-c1c2c4c5` / `--f1-out-of-core`: when `=1`, retain **every** layer file `0..n` instead of rolling the two-layer window (the default drops layer `k-2` as the window advances). The preserve-all-layers substrate for the knowledge-compiler query tool and a full on-disk ladder for archival. Peak disk becomes the **full** ladder (full-31: **3.29 TB MEASURED** — 3,293,894,951,830 B by `du -sb FDIR` on 2026-08-02, [TR-12](../reports/TR12_QUERY_PROGRAM.md) §R.0's measured table; a 4 TB volume holds it with little headroom), not the ~1×-largest-layer transient. ⚠ **[CORRECTED 2026-09-24 (Q-725) — this cell gave a 2.5 to 2.7 TB *projection* from the 2026-07-23 partial measurement (1.624 TB at `k = 0..16`, 17 of 32 layers, plus a mask-palindrome projection) and said to plan a 4 TB disk. The completed ladder measured 3.29 TB, above the whole projected range.]** This row sizes the **f** ladder only: the g ladder (`--kc-g-build`) is a separate **8.27 TB** and the t ladder **3.48 TB**, both measured (same TR-12 table); f+g+t together need ~15 TB. The flag only suppresses the `k-2` unlink; the count and the layer bytes are unchanged → **Sha-neutral**. Emits a `[f1c5] KEEP-LAYERS` stderr banner. |
...
| `SOLVE_F1_ADOPT_UNVERIFIED` | unset (`0`) | Operator escape hatch for the adopt-time digest gate (2026-09-10), and for ONE case: a `.finalized` marker that records **no** digest, i.e. a ladder built with `SOLVE_F1_FINALIZE_SHA=0`. Unset, such a layer is not adopted and is re-swept; `=1` adopts it, unattested, with a `WARN` and `F1C5_ADOPT_DIGEST=UNVERIFIED`. It is read only on that branch, so it **cannot** skip a comparison that is possible: a recorded digest that disagrees with the layer on disk refuses the adoption whatever this is set to. Listed here rather than in the operator tables because setting it makes a ladder unattested at that layer, and every count derived from it inherits that. Sha-neutral (module code; adoption is a resume-path optimisation, not an arithmetic input). |
...
| `SOLVE_F1_FINALIZE_SHA` | `1` | OOC v2 layer finalize (f/g/t builders): compute the decompressed-stream sha256 inline during the finalize concat (same digest as `--f1c5-layer-sha`) and record it in the post-finalize `.finalized` marker + the append-only `<pfx>_layer_sha.ledger`. `=0` degrades the marker to size+identity only, and **therefore disables adoption**: the adopt path now requires the digest. ⚠ **Row corrected 2026-09-10** — it read "`=0` degrades the marker to size+identity only (adoption never depends on the digest)". That parenthesis described the code accurately and the design was the defect: a one-byte edit of a finalized layer was adopted, logged under the OLD digest, and produced a wrong total with exit 0 (18,768 for 26,112 at n=9). The adopt path now compares the digest and refuses on mismatch — see the finalize-durability paragraph under `--f1-exact-c1c2c4c5`, and `SOLVE_F1_ADOPT_UNVERIFIED`. |
...
> that followed took the built-in defaults — depth **2** (`solve_depth`, `solve.c:1573`) and no node
...
([DEPLOYMENT.md](DEPLOYMENT.md):197, :1336, :1360) — still ample.
...
```
Usage: solve --kc-o3-rank FDIR GDIR "e,x,..." [--kc-trace] [--kc-bracket] [--kc-ooc] [--kc-cache-mb MB]
Usage: solve --kc-o3-unrank FDIR GDIR RANK [--kc-trace] [--kc-bracket] [--kc-ooc] [--kc-cache-mb MB]
```

Rank and unrank in the ratified citable order **O3** over the C1&C2&C4&C5 SUPERSPACE;
both ladders are required. The two modifiers each add an emission:

* `--kc-trace` prints one `#o3-trace` row per placement (the f·g descent trace TR-12 Q3 /
  EW-1 ride on) and then the whole-line token **`KC_O3_TRACE=OK|FAIL`** — `OK` when every
  per-step flow identity (sum of `g` over all admissible successors equals `g` of the parent)
...
* `--kc-bracket` emits the rank r−1 / r / r+1 neighbour certificate (TR-12 Q1).

Unknown options are **rejected** with exit **2**, never ignored — see *Rejected near-miss
spellings* below for why that sentence is in this document.
...
and is never agreement: at least one input stream was not checked at all (unreadable, bad magic,
wrong container version, or a short read mid-stream), so there is no verdict over its contents.
`FAIL` means the records *were* streamed and a check on them failed, or a requested
`--kc-cert-out` could not be written. Until this landed the subcommand emitted **no token at all**,
and the human-readable `[kc-oracle] VERDICT:` line was printed *above* the certificate block — so a
certificate that failed to write exited 1 with `VERDICT: PASS` as the last verdict on stdout. The
...
with exit **2** and nothing is computed (`… does not accept: --kc-c3-max (--kc-c3-max
needs --kc-record here)`). Until 2026-09-24 it was accepted and silently ignored
(RCQ02 F4, Q-765); `scripts/q326_kc_query_surface_gate.sh` pins both arms.
...
Three strings appear in `solve.c` that are **not** flags and are documented here so that a
reader who types one learns why it is refused rather than concluding it is a feature. Each is
a one-character near-miss of a real modifier, and each names the reason the parser now
fails **closed**: until 2026-09-08 an unrecognised modifier on these three subcommands was
silently ignored, so the misspelling ran the command *without* the behaviour asked for and
still printed an `OK` token (Codex KCQ03 #4). The remedy was not to accept the misspelling
but to reject every unknown option with exit **2**, and the comments at the rejection
sites record the specific string that had been observed.
...
harness that matches `KC_PROFILE=OK` / `KC_EXTREMAL=OK` / exit 0 would, before the fix,
have passed on exactly the run that did not do what its row claimed.
