...
and `SOLVE_KNUTH_FIBER_PERM` (solve.c:36776-36835). They are named here now, but
...
- `threads` — number of pthreads to use. Default `nproc`, clamped to the
  sub-branch count and to a 256 ceiling (`SOLVE_THREADS`, solve.c:43818-43825). *(Corrected
...
Exits 0 on PASS and **40** on a sha mismatch (`solve.c:35317-35325`, "validation
...
> write anywhere in the block (solve.c:35709-35724). So it does **not** cover
...
`strtoll` under an endptr-must-be-NUL check (`--preflight`, solve.c:36302-36317),
...
default `560000000000000` (`--preflight`, solve.c:36302-36317).
...
> `--preflight`, solve.c:36286-36296).** This section previously described two defects as open; both are closed and the description below is what the code now does.
>
...
(solve.c:37431-37436) — then a **fixed, hand-maintained subset** of `SOLVE_*`
...
(`printf("SOLVE_DEPTH=%d\n", …)`, solve.c:37502) with no `export`, so a plain
...
> behavioral change to canonical enumeration" (solve.c:37595-37604), yet on a
> capable host it prints a YES verdict asserting a vectorised path will be
> selected (`avx512_ready`, solve.c:37622-37624). Measured on this tree:
> `solve.c` contains 17
...
> `kw_found_v` is printed (`solve.c:39401` for `--validate`, `:38341` for
> `--verify`) and is folded into neither verdict: `--validate` returns
> `errors > 0 ? 1 : 0` and prints `VALIDATE=PASS|FAIL` (`solve.c:39429-39430`);
> `--verify`'s `total_fail` (`solve.c:38982`) sums C1-C5, decode, sort and dup
...
> sorted-order loop clears `sorted_ok` and counts the error (solve.c:39294-39295);
...
> `@`, not `:`, so they cannot be read as line numbers), used the same way in the `validate_mode` engine comment at solve.c:39200-39210.
>
...
>   (`hdr_records`, solve.c:35360-35365) and the read loop is bounded by it (`n_records`, solve.c:35382),
...
>   landed in `--verify` only (`SOL_HEADER_SIZE`, solve.c:38805-38806); the three audit
...
>   `pidx = (rec[i] >> 2) & 0x3F` in the `--verify-rule2` scan loop (solve.c:35367-35377),
...
(`solve.c:32719`, `solve.c:32801`): `WRITTEN` means the certificate file is on disk and was
...
> into `layer_root` (solve.c:34959; the target is assembled at solve.c:35019-35020),
...
offset and calls `gzseek` (`SOL_RECORD_SIZE` stride, solve.c:39120-39121),
...
essentially the whole file. The `show_mode` block (solve.c:39014-39019) says
...
`PROOF: Position 2 determines positions 3-19` (solve.c:42560): for each valid branch it
...
`PROOF: All self-complementary branches produce valid orderings` (solve.c:42359):
...
own banner is
`PROOF: Positions 3-19 have exactly 2 budget-feasible candidates` (solve.c:42426): at each position 3-19 it tests all 30 unused
...
(`strcmp(sha_full, sha_56)`, solve.c:37936). There is no scope list and no baseline from
...
(solve.c:37751-37768). Note the practical consequence: it cannot catch a
...
exists, else `/tmp` (`SOLVE_REGRESS_DIR`, solve.c:38013-38016). The positional argument is a node
...
to walk (`manifest_path`, solve.c:37688). The scan target is hard-coded `.`
...
(`LC_ALL=C sort > %s`, solve.c:3357) and into the `wc -l < manifest_path` count (solve.c:37707), so a path
...
`auto_emit_shard_manifest_default()` (solve.c:44360 and solve.c:44803); it
...
*file*, not a directory — `manifest_path` at solve.c:37688), re-computes the sha256 of
...
(solve.c:38221-38236). Output is **one aggregate line** —
`<n_below> <n_total>`, the count of queries whose log-density is at or below
`--threshold` and the number scored (`solve.c:38249`) — not a per-record
...
|---|---|---|
| `SOLVE_THREADS` | `nproc` (`sysconf(_SC_NPROCESSORS_ONLN)`; 8 if that fails), then clamped to the sub-branch count and to a hard ceiling of **256** | Number of pthreads for enumeration. ⚠ Row corrected 2026-09-01: this cell previously capped the default at 128. That cap lives only in `manifest_thread_count()` (solve.c:3317-3322), a different function; the enumeration path is solve.c:43818-43825 |
...
| `SOLVE_SUB_BRANCH_PARALLELISM` | unset | Value domain is **`{single, force-parallel}`** only (solve.c:38391-38401): `single` forces the serial path, `force-parallel` forces the parallel path even at one worker. ⚠ Row corrected 2026-09-01: this cell previously advertised a numeric core count as the value; a number matches neither recognised string and changes nothing; worker count comes from `SOLVE_THREADS` / the positional `threads` argument, and the parallel path is taken whenever that is > 1 |
| `SOLVE_REGRESS_DIR` | `/mnt/work` if it exists, else `/tmp` | Directory for `--regression-test` / `--double-regression-test` artifacts (solve.c:37801, :37190-37193). ⚠ Row corrected 2026-09-01: previously `./` |
...
| 50 | **Regression-test / internal-consistency failure** — a phase failure or sha mismatch in `--regression-test` / `--double-regression-test`, or a startup King Wen self-check failure (`solve.c` has 19 `return 50` sites; e.g. :37236, :37290, :38050). ⚠ Row corrected 2026-09-01: this row previously attributed code 50 to a `--selftest` sha mismatch. `--selftest` returns **40** on sha mismatch (`solve.c:35321-35325`), not 50. |
...
- `shard_manifest.txt` — auto-emitted at promotion/startup (after
  ...
  only, not after every flush (`auto_emit_shard_manifest_default`, solve.c:44360 and solve.c:44803) — unless
  `SOLVE_SKIP_AUTO_MANIFEST=1`.
...
solve.c:34764: `gcc -O3 -pthread -fopenmp -o solve solve.c -lm -lz`.)*
...
Dispatched as a subcommand at `solve.c:33636`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-build`

Dispatched as a subcommand at `solve.c:33754`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-cert-selftest`

Dispatched as a subcommand at `solve.c:33621`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-count`

Dispatched as a subcommand at `solve.c:33765`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33418`.*
...
```
*Grammar reproduced from `solve.c:33492`.*

#### `--kc-g-selftest`

Dispatched as a subcommand at `solve.c:33417`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-ladder-selftest`

Dispatched as a subcommand at `solve.c:33619`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33816`.*
...
```
*Grammar reproduced from `solve.c:33387`.*

#### `--kc-o3-rank` / `--kc-o3-unrank`

Dispatched together at `solve.c:33454`. `solve.c` carries one `Usage:` template for both,
naming `--kc-trace` and `--kc-bracket` (`solve.c:33457-33468`; the subcommand and its third
...
* `--kc-trace` prints one `#o3-trace` row per placement (the f·g descent trace TR-12 Q3 /
  ...
  `KC_O3_TRACE` is printed at `solve.c:24346` (rank) and at `solve.c:24384` (unrank),
...
Dispatched as a subcommand at `solve.c:33453`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33402`.*
...
*Grammar and description reproduced from `solve.c:25809`.*
...
⚠ **Added 2026-09-07 (Q-410).** The flag was dispatched at `solve.c:33618` and
...
Dispatched as a subcommand at `solve.c:33617`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33803`.*
...
```
*Grammar reproduced from `solve.c:33824`.*

#### `--kc-scan-selftest`

Dispatched as a subcommand at `solve.c:33622`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-selftest`

Dispatched as a subcommand at `solve.c:33385`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-t-build`

Dispatched as a subcommand at `solve.c:33582`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33567`.*
...
Dispatched as a subcommand at `solve.c:33566`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33770`.*

### Modifiers

| flag | arity | parsed at | sets |
|---|---|---|---|
| `--kc-bracket` | none (boolean) | `solve.c:33477` | `o3bracket` |
| `--kc-cert-out` | 1 string | `solve.c:25829` | `cert_out` |
| `--kc-chi2-samples` | 1 integer | `solve.c:33396` | `M` |
| `--kc-class-uniform` | none (boolean) | `solve.c:33708` | `class_uniform` |
| `--kc-dump` | 1 integer | `solve.c:25828` | `dump_max` |
| `--kc-expect-count` | 1 string | `solve.c:25830` | `expect` |
| `--kc-g-ooc` | none (boolean) | `solve.c:33435` | `gooc` |
| `--kc-oracle-repr` | none (boolean) | `solve.c:25825` | `check_repr` |
| `--kc-record` | none (boolean) | `solve.c:33709` | `want_record` |
| `--kc-roundtrips` | 1 integer | `solve.c:33395` | `R` |
| `--kc-scratch` | 1 string | `solve.c:33412` | `scratch` |
| `--knuth-dump-prefix` | see code | `solve.c:36520` | — |
| `--r11-verify` | see code | `solve.c:37023` | — |
| `--rc1c-verify` | see code | `solve.c:36996` | — |
...
|---|---|---|---|---|
| `--kc-alt` | `--kc-alts` | `--kc-profile` | `solve.c:29328-29334` | ran with 0 `#alt` rows and `KC_PROFILE=OK` |
| `--kc-braket` | `--kc-bracket` | `--kc-o3-rank` / `--kc-o3-unrank` | `solve.c:33479-33486` | ran with 0 bracket lines and exit 0 |
| `--kc-witnes` | `--kc-witness` | `--kc-extremal` | `solve.c:32664-32671` | produced `KC_EXTREMAL=OK` with no witness |
