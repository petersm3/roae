...
and `SOLVE_KNUTH_FIBER_PERM` (solve.c:36164-36223). They are named here now, but
...
- `threads` — number of pthreads to use. Default `nproc`, clamped to the
  sub-branch count and to a 256 ceiling (`SOLVE_THREADS`, solve.c:43206-43213). *(Corrected
...
Exits 0 on PASS and **40** on a sha mismatch (`solve.c:34705-34713`, "validation
...
> write anywhere in the block (solve.c:35097-35112). So it does **not** cover
...
`strtoll` under an endptr-must-be-NUL check (`--preflight`, solve.c:35690-35705),
...
default `560000000000000` (`--preflight`, solve.c:35690-35705).
...
> `--preflight`, solve.c:35674-35684).** This section previously described two defects as open; both are closed and the description below is what the code now does.
>
...
(solve.c:36819-36824) — then a **fixed, hand-maintained subset** of `SOLVE_*`
...
(`printf("SOLVE_DEPTH=%d\n", …)`, solve.c:36890) with no `export`, so a plain
...
> behavioral change to canonical enumeration" (solve.c:36983-36992), yet on a
> capable host it prints a YES verdict asserting a vectorised path will be
> selected (`avx512_ready`, solve.c:37010-37012). Measured on this tree:
> `solve.c` contains 17
...
> `kw_found_v` is printed (`solve.c:38789` for `--validate`, `:38341` for
> `--verify`) and is folded into neither verdict: `--validate` returns
> `errors > 0 ? 1 : 0` and prints `VALIDATE=PASS|FAIL` (`solve.c:38817-38818`);
> `--verify`'s `total_fail` (`solve.c:38370`) sums C1-C5, decode, sort and dup
...
> sorted-order loop clears `sorted_ok` and counts the error (solve.c:38682-38683);
...
> `@`, not `:`, so they cannot be read as line numbers), used the same way in the `validate_mode` engine comment at solve.c:38588-38598.
>
...
>   (`hdr_records`, solve.c:34748-34753) and the read loop is bounded by it (`n_records`, solve.c:34770),
...
>   landed in `--verify` only (`SOL_HEADER_SIZE`, solve.c:38193-38194); the three audit
...
>   `pidx = (rec[i] >> 2) & 0x3F` in the `--verify-rule2` scan loop (solve.c:34755-34765),
...
(`solve.c:32312`, `solve.c:32394`): `WRITTEN` means the certificate file is on disk and was
...
> into `layer_root` (solve.c:34347; the target is assembled at solve.c:34407-34408),
...
offset and calls `gzseek` (`SOL_RECORD_SIZE` stride, solve.c:38508-38509),
...
essentially the whole file. The `show_mode` block (solve.c:38402-38407) says
...
`PROOF: Position 2 determines positions 3-19` (solve.c:41948): for each valid branch it
...
`PROOF: All self-complementary branches produce valid orderings` (solve.c:41747):
...
own banner is
`PROOF: Positions 3-19 have exactly 2 budget-feasible candidates` (solve.c:41814): at each position 3-19 it tests all 30 unused
...
(`strcmp(sha_full, sha_56)`, solve.c:37324). There is no scope list and no baseline from
...
(solve.c:37139-37156). Note the practical consequence: it cannot catch a
...
exists, else `/tmp` (`SOLVE_REGRESS_DIR`, solve.c:37401-37404). The positional argument is a node
...
to walk (`manifest_path`, solve.c:37076). The scan target is hard-coded `.`
...
(`LC_ALL=C sort > %s`, solve.c:3357) and into the `wc -l < manifest_path` count (solve.c:37095), so a path
...
`auto_emit_shard_manifest_default()` (solve.c:43748 and solve.c:44191); it
...
*file*, not a directory — `manifest_path` at solve.c:37076), re-computes the sha256 of
...
(solve.c:37609-37624). Output is **one aggregate line** —
`<n_below> <n_total>`, the count of queries whose log-density is at or below
`--threshold` and the number scored (`solve.c:37637`) — not a per-record
...
|---|---|---|
| `SOLVE_THREADS` | `nproc` (`sysconf(_SC_NPROCESSORS_ONLN)`; 8 if that fails), then clamped to the sub-branch count and to a hard ceiling of **256** | Number of pthreads for enumeration. ⚠ Row corrected 2026-09-01: this cell previously capped the default at 128. That cap lives only in `manifest_thread_count()` (solve.c:3317-3322), a different function; the enumeration path is solve.c:43206-43213 |
...
| `SOLVE_SUB_BRANCH_PARALLELISM` | unset | Value domain is **`{single, force-parallel}`** only (solve.c:37779-37789): `single` forces the serial path, `force-parallel` forces the parallel path even at one worker. ⚠ Row corrected 2026-09-01: this cell previously advertised a numeric core count as the value; a number matches neither recognised string and changes nothing; worker count comes from `SOLVE_THREADS` / the positional `threads` argument, and the parallel path is taken whenever that is > 1 |
| `SOLVE_REGRESS_DIR` | `/mnt/work` if it exists, else `/tmp` | Directory for `--regression-test` / `--double-regression-test` artifacts (solve.c:37189, :37190-37193). ⚠ Row corrected 2026-09-01: previously `./` |
...
| 50 | **Regression-test / internal-consistency failure** — a phase failure or sha mismatch in `--regression-test` / `--double-regression-test`, or a startup King Wen self-check failure (`solve.c` has 19 `return 50` sites; e.g. :37236, :37290, :38050). ⚠ Row corrected 2026-09-01: this row previously attributed code 50 to a `--selftest` sha mismatch. `--selftest` returns **40** on sha mismatch (`solve.c:34709-34713`), not 50. |
...
- `shard_manifest.txt` — auto-emitted at promotion/startup (after
  ...
  only, not after every flush (`auto_emit_shard_manifest_default`, solve.c:43748 and solve.c:44191) — unless
  `SOLVE_SKIP_AUTO_MANIFEST=1`.
...
solve.c:34152: `gcc -O3 -pthread -fopenmp -o solve solve.c -lm -lz`.)*
...
Dispatched as a subcommand at `solve.c:33028`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-build`

Dispatched as a subcommand at `solve.c:33146`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
Dispatched as a subcommand at `solve.c:33157`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:32845`.*
...
```
*Grammar reproduced from `solve.c:32919`.*

#### `--kc-g-selftest`

Dispatched as a subcommand at `solve.c:32844`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-ladder-selftest`

Dispatched as a subcommand at `solve.c:33011`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33208`.*
...
```
*Grammar reproduced from `solve.c:32814`.*

#### `--kc-o3-rank` / `--kc-o3-unrank`

Dispatched together at `solve.c:32881`. `solve.c` carries one `Usage:` template for both,
naming `--kc-trace` and `--kc-bracket` (`solve.c:32884-32895`; the subcommand and its third
...
Dispatched as a subcommand at `solve.c:32880`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:32829`.*
...
*Grammar and description reproduced from `solve.c:25570`.*

⚠ **Added 2026-09-07 (Q-410).** The flag was dispatched at `solve.c:33010` and
...
Dispatched as a subcommand at `solve.c:33009`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33216`.*

#### `--kc-scan-selftest`

Dispatched as a subcommand at `solve.c:33014`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-selftest`

Dispatched as a subcommand at `solve.c:32812`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.

#### `--kc-t-build`

Dispatched as a subcommand at `solve.c:32974`; takes **see code**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:32959`.*
...
Dispatched as a subcommand at `solve.c:32958`; takes **none**.
`solve.c` carries no `Usage:` string for it, so no argument grammar is asserted here.
...
```
*Grammar reproduced from `solve.c:33162`.*

### Modifiers

| flag | arity | parsed at | sets |
|---|---|---|---|
| `--kc-bracket` | none (boolean) | `solve.c:32904` | `o3bracket` |
| `--kc-cert-out` | 1 string | `solve.c:25590` | `cert_out` |
| `--kc-chi2-samples` | 1 integer | `solve.c:32823` | `M` |
| `--kc-class-uniform` | none (boolean) | `solve.c:33100` | `class_uniform` |
| `--kc-dump` | 1 integer | `solve.c:25589` | `dump_max` |
| `--kc-expect-count` | 1 string | `solve.c:25591` | `expect` |
| `--kc-g-ooc` | none (boolean) | `solve.c:32862` | `gooc` |
| `--kc-oracle-repr` | none (boolean) | `solve.c:25586` | `check_repr` |
| `--kc-record` | none (boolean) | `solve.c:33101` | `want_record` |
| `--kc-roundtrips` | 1 integer | `solve.c:32822` | `R` |
| `--kc-scratch` | 1 string | `solve.c:32839` | `scratch` |
| `--knuth-dump-prefix` | see code | `solve.c:35908` | — |
| `--r11-verify` | see code | `solve.c:36411` | — |
| `--rc1c-verify` | see code | `solve.c:36384` | — |
...
|---|---|---|---|---|
| `--kc-alt` | `--kc-alts` | `--kc-profile` | `solve.c:29008-29014` | ran with 0 `#alt` rows and `KC_PROFILE=OK` |
| `--kc-braket` | `--kc-bracket` | `--kc-o3-rank` / `--kc-o3-unrank` | `solve.c:32906-32913` | ran with 0 bracket lines and exit 0 |
| `--kc-witnes` | `--kc-witness` | `--kc-extremal` | `solve.c:32257-32264` | produced `KC_EXTREMAL=OK` with no witness |
