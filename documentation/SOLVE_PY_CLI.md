...
--atlas-select LIST          Comma list of q3,q6,v1,v2,v5,xa,q10a,a2,a3,a5 (default: all).
                             `a5` is the orbit-column check (`TR12_A5_ORBIT_COLUMNS`) and,
                             since 2026-09-10, an INVENTORY leg: at n = 31 the union of pairs
                             carrying `marginal_raw` across all layers must be the complete
                             31-pair set. Before that, `{pair3, pair7, pair11}` alone PASSED
                             both A-5 checks, and an invalid pair was reported as
                             `SKIP:no-raw` -- i.e. "we have no data" -- rather than as invalid.
                             Those are opposite states and are no longer conflated,
...
| `--joint-density CHUNKS_DIR OUT_MD` | P2 stage 3: KDE joint density over the 7 informative dims + bootstrap CI on KW's percentile. ⚠ **[CORRECTED 2026-09-22 (V3A-055#1) — this cell reproduces figures that are WITHDRAWN AS EVIDENCE.** [DISTRIBUTIONAL_ANALYSIS.md](DISTRIBUTIONAL_ANALYSIS.md):329-331 states that this pipeline *"reproduces the figures **withdrawn as evidence** in 2026-07-26 and retained only in HISTORY.md; it does *not* reproduce the ≈ 30% de-circularized result."* The word "informative" entered at `b75adb68` on 2026-04-22, **before** the 2026-07-26 withdrawal, so it is stale text rather than a `--joint-density-v2` regression. The command still runs and the dimension count is unchanged; what is wrong is that this row presented it with no withdrawal warning. See `--joint-density-v2` below, and the de-circularized 2-dim re-run at DISTRIBUTIONAL_ANALYSIS.md:217.]** | [SOLVE_C_CLI.md#--joint-density-solvepy-only](SOLVE_C_CLI.md#--joint-density-solvepy-only) |
...
| 1 | A verifier reported at least one mismatch (`--f4p-verify`, `--f6-verify`, `--dav-verify`, `--dav2-verify`, `--db1-verify`, `--vdb-verify`, `--perm-verify`, `--rc4b-verify`, `--rc1c-verify`, `--r11-verify`, `--r11-builder-verify`, `--r7-verify`, `--books-verify`, `--trigram-verify`, `--registry-verify`, `--extended-selftest`, `--h2-verify`, `--h2-mass`), or a command's **own** argument validation rejected its input (e.g. `--perm-verify` given other than 64 integers).; or `--atlas-selftest` did not reach `ATLAS_CONSUMER=PASS`. |
| 2 | `argparse` usage error — unknown flag, missing argument value, or a value outside a flag's declared choices (e.g. `--branch-yield-depth 4` → `invalid choice: 4 (choose from 1, 2, 3)`). **Malformed command lines exit 2, not 1**; a wrapper that branches only on 1 will misclassify them. `--atlas-queries` also exits 2 when it **refuses its input** (not an atlas, a float where a count belongs, no RAW marginals, an unknown `--atlas-select` value) — a deliberate refusal to publish a number it cannot vouch for, not an argparse error. |
