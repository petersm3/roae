...
Long ago — traditionally about 3,000 years, though modern scholarship debates when the ordering was fixed — someone in ancient China, or successive generations of practitioners, arranged 64 symbols (called [hexagrams](https://en.wikipedia.org/wiki/Hexagram_(I_Ching))) in a specific order. This ordering is called the [King Wen sequence](https://en.wikipedia.org/wiki/King_Wen_sequence). There are more possible arrangements of 64 things than there are atoms in the universe — roughly 10^89 of them (a 1 followed by 89 zeros; the observable universe holds about 10^80 atoms). Most of the reduction that follows is one elementary step: roughly 44 of the ~51 orders of magnitude come from the pair rule (Rule 1) alone, and only about 7 from the four constraints measured on top of it. But somehow, whoever arranged them picked one specific arrangement:
...
The tallest blocks (█) are distance 6. The shortest (▁) are distance 1. ▅ is distance 4. The distance-5 block (▆) never appears — that transition never occurs. ⚠ **[CORRECTED 2026-09-25 (Q-700, Codex V3A-056#6) — this read "No block reaches height 5 (▅)".** ▅ is the distance-4 block and appears 19 times. The glyphs come from `roae.py`'s table `SPARK = " ▁▂▃▅▆█"`, indexed by distance 0–6, which skips ▄, so distance 5 would be ▆. Counted 2026-09-25 on the sparkline above: ▁ 2, ▂ 20, ▃ 13, ▅ 19, ▆ 0, █ 9 — the appendix's transition counts, and the sparkline equals the table applied to King Wen's 63 distances.]** See [all 63 transitions](#appendix-all-63-transitions) in the appendix.
...
Canonical enumerations using `solve.c` at the 10T node budget find hundreds of millions of unique orderings satisfying Rules 1-5. At the depth-3 partition (158,364 sub-branches): **706,427,594**. At depth-2 (3,030 sub-branches): **286,357,503**. Both enumerations are partial in the sense that each sub-branch hits its per-sub-branch node budget rather than completing naturally — so the true count under exhaustive enumeration is unknown and likely larger; an unbiased Monte-Carlo estimate (Knuth random-probe) now puts the total at ≈10³⁸ (≈3×10³⁷ distinct-canonical) — see [SEARCH_SPACE_SIZE.md](SEARCH_SPACE_SIZE.md). ⚠ **[WITHDRAWN 2026-08-24 — the ≈3×10³⁷ distinct-canonical figure in the sentence just above exceeds its own 31! ≈ 8.2228×10³³ ceiling by ~4,013×; see documentation/CORRECTIONS.md]** Only Position 1 (pair 0) is universally locked — the same pair appears in every valid ordering. The remaining 31 positions, as measured in the three tracked analyzer logs of canonical slices (d2 10T, d3 10T, d3 100T), look like this:

| Positions | Distinct pairs observed (d2 10T / d3 10T / d3 100T) | King Wen's pair, share of records (d2 10T / d3 10T / d3 100T) | In these slices |
|-----------|---------------|-------------------|-----------|
| 1 | 1 / 1 / 1 | 100% / 100% / 100% | Fixed by C4's definition |
| 2 | 28 / 28 / 28 | 2.74% / 3.01% / 3.04% | Branch-dependent |
| 3–4 | 31 and 3 / 31 / 31 | 2.10% and 0.02% / 2.85–4.45% / 2.90–4.32% | Open at both d3 scales; King Wen's pair a minority |
| 5–18 | 3–4 / 4–6 / 4–6 | 0.07–11.41% / 0.01–9.14% / 0.01–9.07% | A narrow menu, but King Wen's pair is never the most common one |
| 19–20 | 3 / 4 / 4 | 39.43–39.56% / 21.58–21.97% / 25.43–25.80% | A narrow menu |
| 21 | 12 / 4 / 8 | 41.93% / 23.45% / 28.14% | Moves with partition and budget |
| 22–31 | 14 / 12–14 / 15 | 6.63–15.43% / 7.27–25.60% / 6.99–24.84% | Freest part of the slice |
| 32 | 7 / 7 / 7 | 21.15% / 20.92% / 20.24% | |

**Caveats on these rates:** The shares are computed from a partial, non-uniform sample: budgeted slices, not the valid space. The distinct-pair counts are §[2] and the shares are §[21] of `runs/20260418_10T_d2_fresh/analyze_output.log.gz` (286,357,503 records), `runs/20260418_10T_d3_fresh/analyze_output.log.gz` and `runs/20260419_100T_d3_d128westus3/analyze_output.log.gz` (3,432,399,297 records). A share is the fraction of one slice's records that carry King Wen's pair at that position, so it reflects where that search spent its node budget; it is not a probability over valid orderings. Each search branch was explored to an unknown depth (none completed), so shallower solutions are overrepresented, and a pair count is what the slice reached — a complete enumeration can only add pairs. The rates should be treated as indicative, not definitive. At positions 3–18 King Wen's pair is not the most common pair in any of the three logs; at positions 19–32 it is the most common at a few positions in some logs (for example positions 23, 29 and 31 in all three). ⚠ **[CORRECTED 2026-09-24 (Q-764) — this table previously gave positions 3–18 a floor of two pairs each with King Wen's pair matching in nearly every record, positions 19–20 an even split, positions 21–32 a floor of seven to sixteen pairs each at low match rates, and position 2 a floor of sixteen pairs with King Wen's pair under one percent, and called the first band highly constrained. No tracked analyzer log supports those figures: at positions 3–18 King Wen's pair holds 0.01–11.41% of records across the three logs and is never the modal pair, and position 2 admits 28 pairs in all three. The figures above are re-derived from §[2] and §[21] of the three logs; the sibling sites in [SOLVE.md](SOLVE.md) were corrected the same day. See documentation/CORRECTIONS.md.]** ⚠ **[SCOPED 2026-09-25 (Q-800) — the d3 10T column is not measured on the d3 10T canonical this document quotes. `runs/20260418_10T_d3_fresh/analyze_output.log.gz` reads **706,422,987** records: it analyzes the **deprecated** `f7b8c4fb…` artifact, a pre-resume-fix undercount. The current d3 10T canonical `b85c8871…` holds **706,427,594** (4,607 more), and that is the figure given above and in the numbers-at-a-glance table below, which is correct. The d3 10T shares and pair counts in the table have not been re-derived on `b85c8871…`; see [CANONICAL_HASHES.md](CANONICAL_HASHES.md) §Deprecated and documentation/CORRECTIONS.md.]**
...
converter). Note also how the funnel splits across the rows: roughly 44 of the ~51 orders of magnitude
are gone at step 1 (C1 alone), and only about 7 more come from steps 2–5, the constrained measurement. ⚠ **[WITHDRAWN 2026-08-24 — the ≈3.3×10³⁷ orientation-dedup figure above exceeds its own 31! ≈ 8.2228×10³³ ceiling by ~4,013×; see documentation/CORRECTIONS.md]***
...
**Open questions this work surfaces:** see [CRITIQUE.md §Open questions](CRITIQUE.md). In brief: Costas arrays at order 64 are closed, negatively — they exist (Golomb G₃ at q = 67), and C1 excludes every one of them in two lines (see [CRITIQUE.md](CRITIQUE.md) §"Missing analyses"); Latin-square's 57.96% C2 rate invites parallel analysis of whether KW's own adjacency geometry has an analogous within/between decomposition; whether any published order-64 Costas arrays accidentally satisfy C1+C2+C3 is a concrete follow-up. ⚠ **[CORRECTED 2026-09-25 (Q-700 / Q-758, Codex V3A-056#3 and V3A-015#5) — this paragraph opened "Costas arrays at order 64 remain untested (infeasible at 64! candidates)", and the follow-up sentence just above is answered: no. Under the binary value convention the reversal mates of 1, 13, 19 and 31 are 32, 44, 50 and 62, each 31 larger. C1 makes all four pairs adjacent, which gives four displacement vectors from the two-element set {(1, +31), (1, −31)}. One repeats, so no C1 sequence is a Costas array, and no published array can satisfy C1+C2+C3. The follow-up sentence is kept because it is the question this note answers.]**
...
Complementation maps King Wen pairs to King Wen pairs: if A and reverse(A) form a pair, then complement(A) and complement(reverse(A)) also form a pair. Eight of the 32 pairs are their own complement pair (distance 1). The other 24 fall into 12 couples. In 7 couples the complement pair sits in the same order, so both members point at the same distance, which gives the groups of two (e.g., 5↔35 AND 6↔36). In 5 couples it sits in reverse order, so the two members point at distances one apart on either side of the pair's spacing (e.g., 3↔50 at 47 and 4↔49 at 45). One of those, 38↔39, lands at distance 1, which is why distance 1 holds nine pairs. ⚠ **[CORRECTED 2026-09-25 (Q-700, Codex V3A-056#4) — this paragraph read "Multi-pair distances always come in groups of 2" and said both members of a pair always point at the same distance.** Executed 2026-09-25 on `solve.binary_hexagrams`: the two members' complement distances differ for 10 of the 32 pairs (47/45, 7/5, 27/25, 21/19 and 3/1), and the 32 distances are the table below, including nine at distance 1.]**
