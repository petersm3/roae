...
- C6+C7 together eliminate 99.995% of solutions but leave thousands of non-KW survivors. **The number of boundary constraints needed for uniqueness grows with partition depth** (SUPERSEDES earlier "4 always suffice" framing):
  ...
  - **d3 560T: re-evaluation queued.** The 560 T deeper enumeration may surface additional 5-subset-indistinguishable KW look-alikes, in which case the minimum will grow toward 6; analysis pending on the warm copy.
  
  **Partition-stable finding**: boundaries **{25, 27}** appear in every working minimum set at all three partition depths tested to date. **Partition-dependent**: the remaining boundaries AND the minimum count itself. Only `{25, 27}` can be cited as generally mandatory; "4 boundaries minimum" is true only at 10T scales, not 100T. The minimum may continue to grow at 560T+. See [SOLVE.md](SOLVE.md) for the full per-dataset analysis.
