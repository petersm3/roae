...
*The mean distance between complementary hexagrams in King Wen is 12.125. **Scope-dependent finding:** at the C1+C2 reference scope, this is the **3.9th percentile** of complement distances — KW is in the lowest 4%. At the C1+C2+C3 canonical scope (3.43B orderings at 100T d3), KW is at the **C3 ceiling = 12.125 exactly**, with ~340M orderings tying at this value (minimum cd in that population is 6.625, i.e., x64 = 424). The threshold 12.125 is King Wen's exact value — extracted from the sequence, not derived independently. The "low-percentile" and "ceiling cohort" framings are both true at their respective scopes; the threshold itself is reverse-engineered, not a structurally significant value.*
...
- C6+C7 together eliminate 99.995% of solutions but leave thousands of non-KW survivors. **The number of boundary constraints needed for uniqueness grows with partition depth** (SUPERSEDES earlier "4 always suffice" framing):
  ...
  - **d3 560T (2026-06-11)**: greedy minimum drops back to **4**, set `{4, 27, 25, 21}` applied in order; cumulative survivor counts 51,404 → 481 → 14 → 1. *Unordered* working-4-set count = 0 (vs 8 at 11.2T, 4 at 742M). §[7] proves no 3-tuple works (best `{4, 25, 27}` leaves 15 survivors), so 4 is the true minimum.
  
  **Partition-stable finding**: boundaries **{25, 27}** appear in every greedy minimum at all four partition depths tested (d2 10T, d3 10T, d3 100T, d3 560T). **Not scale-stable**: the unordered working-4-set count (8 → 4 → 0 across 11.2T → 742M → 560T) AND the greedy-ordered count (non-monotone: 4 → 5 → 4 across 10T → 100T → 560T). The durable structural claim is "**4 boundaries suffice via greedy-ordered application**," reaffirmed at the deepest scale. See [SOLVE-SUMMARY.md](SOLVE-SUMMARY.md) §"560T canonical results" + [SOLVE.md](SOLVE.md) for the full per-dataset analysis.
...
*Empirically confirmed at the full d3 560T canonical (10,525,271,997 records, sha `9a968fa2…`, current deepest): 100.000000% of records have odd wrap-around — exactly as the theorem requires. (`solve --verify` enforces C4 + C5 on every record, so the parity is a necessary consequence.) Theorem confirmed at canonical scale. Which odd value the wrap-around takes (d=1 vs d=3) is budget-dependent and is not separately tabulated at 560T.*

*Note (McKenna's 25/75 observation).* McKenna and McKenna (1975, *The Invisible Landscape*, Chapter 9) state "a perfect ratio of three to one; three even integers to each odd integer," giving the count as "fourteen threes and two ones constitute sixteen instances of an odd integer occurring out of a possible sixty-four" — i.e., the **circular** reading (64 transitions including the wrap-around s₆₃ → s₀). The 16/64 = 25% odd / 75% even split is exact under the circular reading and follows directly from this theorem combined with C5. The 63 linear transitions give 15/63 = 23.81% odd (approximate but not exact). McKenna's empirical observation is mathematically equivalent to this theorem; he discovered it before its proof was articulated here. See CITATIONS.md for the verified source attribution.

*Future option (not currently part of the spec).* If King Wen were treated as a circular sequence and the wrap-around distance were further constrained to its KW-specific value `d(s₆₃, s₀) = 3`, the search space would be measurably reduced (the d=1 minority would be filtered out). That minority fraction grows with node budget — sub-1% at shallow budgets, single-digit-percent by 11.2T — so at 560T/1120T canonical scales the reduction would plausibly be larger still (not separately measured at 560T). This is NOT currently in the formal spec because (a) the weak form (odd parity) is already a theorem and adds no constraint, and (b) the strict form (d=3 specifically) would be reverse-engineered from KW similar to C3/C6/C7, and adopting it would invalidate every existing canonical sha by tightening C5. Documented here as an option for future spec evolution.
...
**Result (Minimum adjacencies, canonical 2026-04-18; updated 2026-06-11):** The minimum number of boundary constraints needed to uniquely determine King Wen is **partition + scale-dependent and non-monotone with scale**. At d2 10T and d3 10T the greedy-ordered minimum is **4 boundaries**; at d3 100T it grew to **5**; at d3 560T it drops back to **4** (set `{4, 27, 25, 21}` applied in cumulative order — boundary 4 alone eliminates 99.999% of non-KW). §[7] proves no 3-tuple works at any scale tested. **Partition-stable**: boundaries `{25, 27}` appear in every greedy minimum at every partition tested. **Not stable**: the other boundaries in the minimum AND the *unordered* working-4-set count (8 at d3 10T; 0 at d3 100T and d3 560T). At d2 10T the 4 unordered working 4-sets are `{2,21,25,27}`, `{2,22,25,27}`, `{3,21,25,27}`, `{3,22,25,27}` — structure `{25, 27} ∪ one-of-{2, 3} ∪ one-of-{21, 22}`. At d3 10T there are 8 unordered working 4-subsets with boundaries from `{1..6}` combined with `{25, 27}`. The durable claim is "**4 boundaries suffice via greedy-ordered application**," reaffirmed at the deepest scale. *See SOLVE-SUMMARY.md §"560T canonical results" + SOLVE.md for the full per-dataset analysis.*
