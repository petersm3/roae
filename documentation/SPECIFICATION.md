...
**Status: descriptive, not explanatory** — a KW-derived slot pin, priced at net ≈ 0 bits (see the standing table above). `solve.c` calls C6/C7 the "legacy adjacency constraints"; the knowledge compiler does **not** use them — it compiles C1∩C2∩C4∩C5 only. ⚠ The knowledge compiler is **not in `main`'s `solve.c`** (`grep -c -- '--kc-' solve.c` returns 0): the `--kc-*` surfaces live on the `v4-compiler` snapshot branch, which [BRANCH_REGISTRY.tsv](BRANCH_REGISTRY.tsv) classes *snapshot — do not cite*; see [VERIFY.md](VERIFY.md) §"⚠ NOT AVAILABLE IN THIS TREE" and §"Branch note (T3 sampler)". *(Availability qualifier added 2026-09-01: both C6/C7 status blocks described an absent subsystem in the present tense with no such note, so a reader cloning the canonical sha to inspect it finds nothing.)*
