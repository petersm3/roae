...
| `python3 verify.py --f1u192-binary-roundtrip` | builds the n=9 layer files with `solve`, then reads the final layer back **from raw bytes in Python** (72-byte header, `masks u32[]`, `off u64[]`, `keys u32[]`, `vals` as 24-byte little-endian limb triples) and checks its mass against this file's own independent count. `solve`'s own write→resume round-trip **cannot** do this: writer and reader share any limb-order defect and cancel it exactly — measured, see below. Absence of a binary FAILS |
