...
| mode | what it checks |
|---|---|
| `python3 verify.py [solutions.bin]` | the RECORDS: re-decodes every record and re-checks C1–C5 (C4 in its **oriented** spec form) + record-format conformance + order/dups. See §"What the records path actually enforces" below. **Fail-closed on an empty population (2026-09-03):** an artifact whose header declares **zero records** exits `VERIFY=ERROR` rc 30 (`VERIFY ERROR: … contains ZERO records — nothing was verified`) instead of the `VERIFY PASS: all 0 records satisfy C1-C5, sorted, no duplicates` rc 0 it printed until that day — measured on a 32-byte header-only file, raw and gzipped; `solve.c --verify` has rejected the same case since 2026-08-27 (Q-285), so the two verifiers now agree on it. Verdicts are whole-line tokens — `VERIFY=PASS\|FAIL\|ERROR`, `KW_PRESENT=YES\|NO`, `KW_REQUIRED=YES\|NO` — and King Wen's presence stays **informational** unless `--expect-kw` is passed (operator ruling 2026-09-03; item 4 of that section) |
...
| `python3 verify.py --check-kw-pair-adjacency` | (added 2026-08-16) **reproduction command for a classical fact and for a NEGATIVE result.** Re-verifies that King Wen seats every hexagram beside its own partner — 32 pairs, **28 by reversal, 4 by complement**, zero unrelated, zero hexagrams whose partner is not adjacent. **That rule is not ours and not new**: it is 非覆即變, stated by Kong Yingda 孔穎達 (574–648) with earlier lineage via Yu Fan 虞翻 — see [CITATIONS.md#kongyingda](CITATIONS.md#kongyingda). The command exists so a reader can confirm the premise without taking anyone's word for it. It then draws the consequence for excavated evidence: the head/tail symbols of the Shanghai Museum Chu bamboo *Zhouyi* ([Pu Maozuo 2003](CITATIONS.md#pu2003)) **cannot distinguish** "the symbol respects reversal" — Pu's claim — from "the symbol is merely constant on contiguous King Wen blocks", because blocks and orbits coincide by construction of the sequence. It reports `SHANGBO_TESTABLE_PAIRS=3`, `SHANGBO_AGREEMENTS=3`, `SHANGBO_DISAGREEMENTS=0`, `SHANGBO_UNOBSERVED_PAIRS=6` and `SHANGBO_DISCRIMINATING_PAIRS=0`. **Corrected 2026-09-03 from `=9`:** until then the routine held nine King Wen *number* pairs and **no symbol value anywhere in the file**, so the count was a `len()` and the exit status turned solely on adjacency — a mistranscribed or genuinely disagreeing symbol left the output byte-identical. The observed symbols are now embedded from the per-slip 考釋 and compared, and a pair counts as testable only when **both** members carry an observed value. 🔴 **What that correction did NOT change — and what will not be changed:** the **exit status still turns solely on adjacency**. The routine ends `return 0 if adjacency else 2`, so `SHANGBO_SYMBOL_TEST=FAIL` exits **0** (measured 2026-09-07 on a flipped-symbol negative control — 豐 55 set to `black_L_faded_red` gives `SHANGBO_AGREEMENTS=2`, `SHANGBO_DISAGREEMENTS=1`, `SHANGBO_SYMBOL_TEST=FAIL`, rc **0**). That fail-open rc is **by design and was adjudicated correct**: a symbol disagreement is a finding about Pu Maozuo's reading of the manuscript, not a defect in this repository, and folding it into the exit status would make a genuine philological discovery present itself as a broken build. The rc answers only the classical adjacency question (`KW_PARTNER_ADJACENCY`). Per this file's whole-line-token rule the **token is the check**, so **callers must gate on `python3 verify.py --check-kw-pair-adjacency \| grep -qx 'SHANGBO_SYMBOL_TEST=PASS'` and never on the exit status** — and note `VACUOUS`, printed when no pair has both symbols, is not a pass. **An impossibility argument, not a criticism of his reading, and not a failed search.** ⚠ **[CLOSED HISTORICAL NOTE — scope tightened 2026-09-01, CLOSED 2026-09-03. The state described below is superseded; do not read it as current.** Between those two dates this row carried a standing gap, recorded here because the correction it prescribed has since shipped. The row had previously reported Pu's claim as holding "9 testable pairs / 9 agreements / 0 disagreements", worded as a figure the command checks, and **at that time the command did not check it**: `verify.py` then held those nine entries as King Wen *numbers* only and carried no symbol values anywhere in the file, so `SHANGBO_TESTABLE_PAIRS` was a `len()` of the transcribed list, the agreement census was **Pu's reading as transcribed** rather than anything this repository recomputed, and a mistranscribed or oppositely-classified symbol left the command's output byte-identical. The note then prescribed the fix: embed the per-slip observed values — page-cited to Pu 2003's 考釋, observed only — print `SHANGBO_AGREEMENTS=`/`SHANGBO_DISAGREEMENTS=` beside the existing tokens, and pin it with a flipped-value negative control. **That work landed on 2026-09-03 and the prescription is spent.** At HEAD `verify.py` carries `SHANGBO_OBSERVED_SYMBOL`, compares the two members of each pair, and prints `SHANGBO_SYMBOL_TEST=PASS\|VACUOUS\|FAIL`; the flipped-symbol control moves the output (`SHANGBO_DISAGREEMENTS=1`, `SHANGBO_SYMBOL_TEST=FAIL`), so this row now has the reproduction command it then lacked. The three assertions in this note — that the file carries no symbol values, that a wrong symbol leaves the output byte-identical, and that the claim has no reproduction command — were true when written and have been **false since 2026-09-03**; they survive only as the record of what was corrected. What the 2026-09-03 work did **not** touch is the exit status, deliberately — see the preceding paragraph. **The impossibility result was never affected either way**: it needs only the King Wen numbers, which are published.]** Reads **no files** |
| `python3 verify.py --check-classical-groups` | (added 2026-08-16) **the group actions the CLASSICAL literature attests, and how King Wen scores against each — none of the rivals are ours.** Sources: **⟨comp,rev⟩** and **⟨rev,swap⟩** both from 吳澄 Wu Cheng (1249–1333) 《易纂言外翼》卷一〈卦對第二〉 ([#wucheng](CITATIONS.md#wucheng)); **⟨comp,swap⟩** generated by complement and the **within-hexagram** exchange of upper and lower trigram, which is 虞翻 Yu Fan's 兩象易 ([#yufan](CITATIONS.md#yufan)); complement is his 旁通, but no source cited here assembles the two into this group. ⚠ **[CORRECTED 2026-09-24 (Q-753) — this rival was attributed to 焦循 Jiao Xun's 《易圖略》八卦相錯圖. [CITATIONS.md](CITATIONS.md#jiaoxun) itself records that 八卦相錯 is a different operation: an exchange of lower trigrams between **two** hexagrams, which Jiao explicitly distinguishes from 虞翻's 兩象易 (「此與相錯似近，而非」). The command implements the one-hexagram swap, so the orbit census and the 24/64 below are unchanged; only the attribution moves. The ruling rests on CITATIONS.md's description, not on a fresh reading of Jiao's text. Adjudicated by Fable (Codex V3A-061#2).]** **The result:** ⟨comp,swap⟩ has the *identical* orbit profile to ⟨comp,rev⟩ — 20 orbits, 8×2 + 12×4 — yet King Wen seats partners adjacently **64/64** under ⟨comp,rev⟩ and only **24/64** under ⟨comp,swap⟩. At the sharper pairing-rule level: reversal-with-complement-fallback (= C1, 非覆即變) scores **64/64**; every **complete** alternative pairing rule tested scores **12–16/64** (`RULE_COMP_ALONE`, `RULE_SWAP_THEN_COMP` and `RULE_COMP_OF_REV` at 16/64, `RULE_SWAP_ALONE` at 12/64). Bare reversal is the one exception and is not a rival: it is the rule C1 *completes*, scoring **56/64** (`RULE_REV_ALONE`) and leaving exactly the eight self-reverse hexagrams unpaired — 64 − 8. *(Corrected 2026-09-01: this sentence read "every alternative tested scores 12–16/64", a universal that the cited command refutes in its own output on every run.)* And this is **exact, not sampled**: of the **3.845×10⁴⁶** involutions on the 64 with eight fixed points, **exactly 70** score 64/64 — a fraction of **1.8×10⁻⁴⁵** — and all 70 agree with reversal off the degenerate hexagrams. **Those 70 are one rule under 70 labellings**, since 'fixed point' versus 'swapped pair' is a vacuous distinction exactly where the two operations coincide. **And the hexagrams where that freedom lives are exactly [吳澄's two degenerate classes](CITATIONS.md#wucheng)** — 正對不反易者四 + 正對兼反易者四. So a structurally indistinguishable rival, drawn from the same tradition, does not fit — the choice is a property of the sequence, not an artifact of looking for symmetry. Also re-derives 吳澄's own 「共十八對」 as a reading check (24 orbits − 6 meeting the 八純卦 = 18 ✓). **Scope: this is about PAIRING. It changes no enumeration and no published count.** Reads **no files** |
...
| `python3 verify.py --check-parity-alternation` | (added 2026-08-16) **re-derives the theorem's figures in [PARITY_ALTERNATION.md](PARITY_ALTERNATION.md)** — a paper-citable finding that until today published its central numbers with no way for a reviewer to re-derive them (**GATE 25 LEG 2 flagged the file; this is the answer to that flag**). Recomputes the 63 transition distances and the multiset `{1:2, 2:20, 3:13, 4:19, 6:9}` from the KW table, confirms the **15** odd transitions, confirms the pair parity class is **well defined** rather than assuming Lemma 1 (corrected 2026-09-02, prose batch P42: well-definedness is that document's Lemma 1; its Lemma 3 is the transition-parity result), confirms the **16/16** class split that `C(32,16)` presupposes, measures KW's own alternation count (**15**), checks C4 pins pair `{63,0}` to the even class, and counts the 15-change arrangements **twice by routes sharing no code** — a DP over (position, evens used, last class, changes) against the closed form `2·C(15,7)² = 82,818,450` — recovering the published `601,080,390` total and `×7.2578` reduction. **Scope: it attests the theorem's FIGURES; the theorem is a proof and is not re-proven here. Two figures on that page sit outside this command — the 48-element relabeling group (`python3 solve.py --symmetry-completeness`, leg SC-7) and Moore's 16/18 KW compliance (`python3 -c "import solve; print(18 - len(solve.h2_parity_slots(list(solve.binary_hexagrams))))"`), each named with its own reproducer there.** Reads **no files**, ~1 s |
...
| `python3 verify.py --recount-rung-layers N` | (N = 9, 13 or 16) gates the **per-layer masses** published in `reports/FULL31_EXACT_AGGREGATES.md` against an independent recount by the plain budgeted DP, with `B0` re-derived. The rung *totals* were already gated; until this mode the *intermediate* layers were gated by nothing. A missing or unparsable table FAILS rather than passing quietly. n=9 and n=13 run in under 20 s; **n=16 takes ~90 s and ~1 GB** (measured, 2-core host) and is the deepest rung the plain DP reaches in-process |
...
| `python3 verify.py --f1u192-binary-roundtrip` | three arms: **v1 raw** and **v2 per-block zlib** (the format `--f1-out-of-core` writes, and therefore what every real Stage F / 560T layer is made of), each building the n=9 layers with `solve` and reading the final layer back **from raw bytes in Python** (72-byte header, `masks u32[]`, `off u64[]`, `keys u32[]`, `vals` as 24-byte little-endian limb triples) and checks its mass against this file's own independent count. `solve`'s own write→resume round-trip **cannot** do this: writer and reader share any limb-order defect and cancel it exactly — measured, see below. Absence of a binary FAILS. A third arm sweeps every kept n=16 v2 layer for block-seam structure — no rung at n≤13 reaches the 65,536-entry block size (widest n=13 layer: 11,102), so without it the block seam would never be crossed; the arm fails if it sweeps layers and none crossed it. Prints `F1U192_V2_LAYOUT=GATED` on success. **`SOLVE_BIN` overrides the binary path here too** (`verify.py:2888`, same form as `--f1-dec-roundtrip`); this row previously said only "Absence of a binary FAILS", leaving a reader without `./solve` in the repo directory with a failure and no documented way out, one line below the row that names the override. *(Line citation stale by 2026-09-21, re-pinned 2026-09-25 (Q-791): the override is `os.environ.get("SOLVE_BIN")` at `verify.py:2888` for this mode and `:2635` for `--f1-dec-roundtrip`; grep the string rather than trusting either number. Both modes executed 2026-09-21 with `SOLVE_BIN` pointing at a fresh build: `all 113 renderings exact`, `F1U192_V2_LAYOUT=GATED`, `F1U192_INTERMEDIATE_MASS=GATED`; with `SOLVE_BIN` pointing at a missing path, `*FAIL* no solve binary at …` rc 1.)* |
...
| `python3 verify.py --twins-bisect SOLUTIONS_BIN` | (added 2026-09-02, TR-5 §5 code half) derives the **24 record keys (KW + 23 twins)** from the 48 valid σ — the same classification `--recount-finite` gates — and reports each one `PRESENT`/`ABSENT` in the given file as whole-line `TWIN_KW=` / `TWIN_NN=` tokens plus `KW_PRESENT=`, `TWINS_PRESENT=`, `TWINS_ABSENT=`, `TWINS_BISECT=DONE`. A raw `solutions.bin` is **bisected** on the primary sort key ([SOLUTIONS_FORMAT.md](SOLUTIONS_FORMAT.md) §Sort order; ~35 reads per key on the 560T canonical, so the 336 GB file is never streamed — `TWINS_SORT_ASSUMED=YES` states that the ABSENT verdict rests on the sort order `verify.py PATH` certifies); a gzip-framed file cannot seek and is scanned linearly (`TWINS_METHOD=scan`). The presence pattern is a **measurement** and never fails the tool: a budgeted shard legitimately lacks KW. Measured 2026-09-02 on a 135,780-record 1e8-node run (sha `403f7202…`): `KW_PRESENT=YES`, `TWINS_ABSENT=23`, 0.1 s; bisect and scan agree on every key. **An empty population is refused, not measured (2026-09-03):** a header declaring zero records prints `[*FAIL*] header declares ZERO records -- every ABSENT would be vacuous` and `TWINS_BISECT=FAIL` rc 1, where until that day it printed 24 vacuous `ABSENT` lines; a file whose size disagrees with its declared count fails the same way (both measured on a 32-byte header-only file) |
| `python3 verify.py --sigma-isomorphism-all48 [--sigma-limit N]` | (added 2026-09-02, TR-5 §3(ii) code half) walks the exact C1–C5 tree below **every one of the 48 σ image prefixes** (22 pairs each; all 48 prefixes are distinct, and the hard-coded prefix `--recount-subtree` uses must be among them) and gates each against `tree_nodes` 9,422,793 / canonical leaves 16,504. **~27 s per σ in CPython, ~21.5 min for all 48** (measured 2026-09-02) — deliberately its own flag, because folding it into `--recount-subtree` would falsify that row's published ~1–2 min. `SIGMA_ISOMORPHISM_ALL48=PASS` is printed only after all 48 match; `--sigma-limit N` is a smoke test whose verdict token reads `PARTIAL` and can never read `PASS` |
| `python3 verify.py --orbit-cv TABLE` | (added 2026-09-04, TR-5 §3(iii) code half) recomputes the **orbit-CV statistics** from the archived 65,281-cell Knuth per-cell estimate table (10⁵ probes per cell; whitespace columns `p1 o1 p2 o2 p3 o3 tree_nodes leaves_c1c2c4c5 leaves_canonical relerr`, `.gz` or plain, ending in the generator's `[percell] DONE <n> rows` stamp, which is **required** — its absence is a truncated table, not a table to skip). Orbits are taken under the 48 valid σ (`_sigma_valid_perms`, the classification `--recount-finite` gates), which the tool asserts equal the centraliser of the reverse permutation in S₆ — the group the 2026-07 measurement used. Gates `ORBIT_CLASSES_MET=4183`, `ORBIT_LEAVES_CANONICAL_WITHIN_CV_MEDIAN=0.1118` and `ORBIT_LEAVES_CANONICAL_POP_CV=0.7202` (published as 4,183 / 0.112 / 0.72) and prints `ORBIT_CV=PASS` only when every gate matched; the table's median `relerr` (0.1192) is printed but **not** gated, because the noise-floor figure it once stood beside was withdrawn (`RF-4e81ce69`). ~1.4 s. Reads no `solutions.bin`. Measured 2026-09-04: a deleted row, a perturbed value, a truncated `.gz`, a renamed header column, a duplicated cell, a missing file and a table without its `DONE` stamp each print `ORBIT_CV=FAIL` rc 1 |
| `python3 verify.py --orbit-coverage TABLE [--orbit-coverage-se-scale F]` | (added 2026-09-24, Q-696 / Codex V3A-082#2 — the code half of METHODS.md's reading-rule calibration) the **empirical coverage of the Knuth estimator's printed interval**, scored on the same 65,281-cell per-cell table `--orbit-cv` reads (same reader, same 48-σ orbit action, same `DONE`-stamp requirement). Cells in one G-orbit have the same true count, so each cell's `leaves_canonical` estimate is scored against the **leave-one-out mean of its orbit-mates**, the band widened by √(1 + SE_proxy²/SE²) for the proxy's own error. Three intervals are scored — the printed Wald 95% (`est ± 1.96·SE`, i.e. `est·(1 ± 1.96·relerr)`), the retired fixed ±20% band, and the log-normal `est·exp(±1.96·relerr)` — over the full table by printed-relerr subset, as bare `KEY=value` lines. Scope of the calibration: `leaves_canonical` at depth-3 cells, printed relerr 5–30% (`BIN_30_50_N=19`, `BIN_50_UP_N=0`); a figure beyond 30% relerr is uncalibrated here. The tokens: `RELERR_GE_10_N=47103`, `RELERR_GE_10_WALD95_COV=94.4`, `RELERR_GE_10_FIX20_COV=85.8`, `RELERR_GE_10_LOGN95_COV=94.9`, `RELERR_GE_10_MEDIAN_Z=-0.043`, `RELERR_GE_10_ABSZ_Q90=1.67` / `_Q95=2.02`, `RELERR_GE_10_WALD_MISSES_LOW_SIDE=85`; `RELERR_GE_20_*` (n 1,353: 92.8 / 69.9 / 96.2); the bins `BIN_10_15_*`, `BIN_15_20_*`, `BIN_20_30_*` (Wald 94.5 / 94.1 / 92.7, fixed ±20% 89.0 / 77.8 / 69.9); `ORBITS_K_GE_5=3727` and `MEDIAN_PRINTED_SE_OVER_REPLICATE_SD=1.031`, with `ORBITS_K_GE_5_ZERO_SD=5`: 3,727 of the 3,732 orbits with ≥5 cells enter the SE/SD ratio; the other 5 (40 cells, relerr 13–16%, each 8 cells that differ only in the orientation and order of two of their three pairs) carry byte-identical estimates — the same walk under the fixed base seed — so they are excluded from the ratio and score z = 0 in the coverage; excluding them from the coverage as well moves no figure by more than 0.1 point (BIN 15–20%: Wald 94.0, fixed band 77.7). Every one of those published figures is **gated** at its printed precision, and `Q696_COVERAGE=PASS` prints only when all match (otherwise `FAIL`, rc 1). `--orbit-coverage-se-scale F` is the **positive control**: every printed relerr is multiplied by F at parse time (so the relerr subsets move with it) and the run ends `Q696_COVERAGE=CONTROL`, never `PASS`; measured 2026-09-24, `RELERR_GE_10_WALD95_COV` = 72.3 / 94.4 / 99.9 at F = 0.5 / 1 / 2, so a halved or doubled SE is visible to the instrument. ~1 s. Reads no `solutions.bin`. `--orbit-coverage-se-scale` given without `--orbit-coverage` is refused: `Q696_COVERAGE=REFUSED`, rc 2. The structural table defects `--orbit-cv` rejects (a missing file, a truncated `.gz` with no `DONE` trailer) print `Q696_COVERAGE=FAIL` rc 1 — but a **single perturbed cell** is not one of them: one cell's `leaves_canonical` ×1.5 leaves both `ORBIT_CV=PASS` and `Q696_COVERAGE=PASS` (measured 2026-09-24; an aggregate at 1 dp cannot see one cell, while every 10th or every 1000th row ×1.5 fails). Per-cell integrity is not what this instrument measures; the SE-scale control is |
...
| `./verify --check-layers DIR [max_k] [run.out]` | (C side, NEW 2026-07-23; orbit-weighted mass added same day) the **on-disk layer files themselves**, read against [F1C5_LAYER_FORMAT.md](F1C5_LAYER_FORMAT.md) with no `solve.c` code: header/`pl_hash`/`b0` vs the manifest, `masks`/`off` layout, per mask popcount/range/**canonicity** (numeric minimum of its orbit), and per entry the key packing, `rid < R`, ascending order, nonzero value, and the **sum invariant** (rid mixed-radix digits sum to `k`). With the TR-11 §2 group **derived independently in-file** (the 48 `C_{S6}(rev)` bit-perms → 24 induced pair-permutations → the run's distinct restricted perms, closure-verified), it re-derives the §Reading-recipe **orbit-weighted mass** `Σᵢ sᵢ·(\|G\|/\|stab(maskᵢ)\|)` from the layer bytes at **every** layer and, when a `run.out` is given, compares it to `solve.c`'s reported `mass=` per layer — the full-scale counterpart of the small-`k` plain-DP mass check. Entry-streaming and `O(nm)` memory, so it reaches every layer on disk — and at the final layer (full-31) the summed value bytes must equal the published **40**-digit count and be `≡ 0 (mod 24)`. Handles v1 (raw) and v2 (per-block zlib). **Layout invariants added 2026-09-03 (Codex V2-F59 #5), each checked before any block is inflated:** v1 `pad = 0`; exact file size against the spec formula (v1 `80 + 12·nm + 28·ne`, v2 `kblk_base + kidx[nblk] + vidx[nblk]`); v2 `kidx`/`vidx` monotone and every compressed block within `compressBound` before it is read — until then a trailing byte, or a nonzero v1 pad, produced output byte-identical to a clean run. **Layer-0 content added 2026-09-24 (Codex V3A-137#3):** the root entry's key must be `start_exit << 16` and its value 1, as the spec says; until then only its shape (`nm = 1`, mask 0, one entry) was checked, and a root value of 2 or a wrong start exit passed. `--check-layers-selftest` (**14 legs**; 11 until 2026-09-24, prints `LCSELFTEST_RESULT=PASS\|FAIL`) synthesizes v1+v2 fixtures, a corrupted case, a **non-trivial-stabilizer** mass fixture (pair-orbit `3.0`, `\|stab\|=2`, orbit `3 < \|G\|=6`), a wrong-reported-mass case, a non-canonical-mask case, and — since 2026-09-03 — a v2 file with one trailing byte, a v2 file with non-monotone `kidx`, a v1 file with one trailing byte and a v1 file with `pad = 0x5A`, and — since 2026-09-24 — a root value of 2 and a root key naming the wrong start exit, each of which must FAIL, plus the restored root, which must PASS — no real data needed. `--scan-selftest` runs the same two root corruptions through both readers. **Campaign-VM tool** (run where the layers live). |
...
| `python3 verify.py --check-t5-c3 SOLUTIONS_BIN CHUNKS_DIR` | (added 2026-08-20) independently recomputes `c3_total` for **every** record of the T5 mega-sample and compares it against the parquet that the `solve.py` pipeline produced. The two routes are disjoint: this one uses `C3 = 16 + 8·G` over the 12 complement-couples' **slot gaps** (machine-checked in [lean/C3Decomposition.lean](../lean/C3Decomposition.lean)) — slot map only, no transition walk, no path, orientation-independent — while `solve.py --compute-stats` walks the ordering. It exists because T5's load-bearing figure, `P(C3 ≤ 776) = 12.1288%`, came out of a single pipeline, and a pipeline agreeing with itself is not a check. All 1,000,000 records, never a subsample; ~5 s. Verdict `T5_C3_AGREE=PASS`/`=FAIL`, matched with `grep -qx`; a single swapped pair-index in one record is caught and its index printed. **The one non-stdlib mode:** it imports `numpy` and `pyarrow` function-locally to read the parquet; on a host without them it stops with `T5_C3_AGREE=ERROR` rc 2 naming the missing module (measured with `python3 -S`), and a run that compares **zero** records — a header declaring 0 beside parquet chunks holding 0 rows — is `T5_C3_AGREE=FAIL zero records compared -- nothing was verified` rc 1, not the `T5_C3_AGREE=PASS` rc 0 it was until 2026-09-03. **Scope: `verify.py` imports nothing from `solve.py`, so this is IMPLEMENTATION-independent — but both are Python, so it does NOT discharge the two-LANGUAGE half of the cross-check gate.** *(Input form, measured 2026-09-21: until that day this mode opened `SOLUTIONS_BIN` raw only and answered `T5_C3_AGREE=FAIL bad magic in …` rc 1 on a gzip-framed artifact — the default form, and the one every other record-reading mode of this file accepts. It now streams through `gzip.open` when the two-byte magic is `1f 8b`; on the repo sample with its own `--compute-stats` chunks it prints `T5_C3_AGREE=PASS`. Pinned by `tests.py::TestP2GzipInputIsTheDocumentedInput`.)* *(Same day: a missing or unreadable `SOLUTIONS_BIN` is `T5_C3_AGREE=ERROR cannot read PATH: …` rc 2 — the `ERROR` value this row already documents for a missing module; until then it was a `FileNotFoundError` traceback with no token. Pinned by `tests.py::TestMissingInputIsRefusedNotCrashed`.)* |
...
header, no shared traversal, no copied table of *results* — the King Wen **input**
`KW[64]` is deliberately the same 64-entry literal in `solve.c`, `verify.c` and
`verify.py`, an acknowledged data-level closure anchored externally, not an
independence claim; `verify.c`'s header says so in the same words), recomputes the engine's per-layer
...
|---|---|---|---|
| `--t3-stats`, `--t3-membership` | the T3 exact-uniform draw sample: 16 streams × 62,500 draws = 10⁶ draws (~107 MB gzipped) | one KC-sampler invocation per stream — arguments `--kc-sample <f-dir> 62500 <seed_i> --kc-record --kc-ooc --kc-cache-mb 384`, with `SOLVE_F1_OOC_READ_MB=1`, against the Stage F **f-ladder** — the sixteen `<seed_i>` values are **published below**, so this is a runnable recipe rather than a template. **On `main`** since 2026-07-17 — see the corrected branch note below the table *(this cell read "Not on `main`" until 2026-09-11; the prose note was corrected and the table row was not, so the page contradicted itself in two places)* | **≈12.6 h wall** on one D16als_v7 (16 lanes, Premium P50 f-disk) against a **3.1 TB** f-ladder — plus building the branch that carries the sampler. Seed-deterministic: the same seeds, f-ladder and binary regenerate the same draws — and **the seeds are now published** (§"The sixteen T3 seeds" below), so that promise is checkable instead of asserted |
...
`git show origin/main:solve.c | grep -c -- '--kc-'` returns **563** at `92516f8d`. *(Stated with its sha deliberately: the first pass wrote 562, measured at `a2a7054d`, and the very commit carrying the correction moved it to 563 — a bare count is a claim with a hidden timestamp.)* The note was
...
checkout of another branch, and the ~12.6 h of compute is the whole cost.
*(A second correction, 2026-09-11, found by F-5 round 5: the first pass at this note left the
original sentence — "Regenerating the T3 sample therefore costs a checkout and build of that branch
on top of the ~12.6 h of compute" — standing immediately after the sentence that refutes it. Two
adjacent sentences said opposite things. A correction that does not delete what it corrects is not a
correction.)* The arguments above are deliberately
...
⚠ **[CORRECTED 2026-09-21 — the sentence immediately above is the THIRD survivor of the same
stale claim, and it contradicts the two corrections that precede it.** If the `--kc-*` subcommands
are on `main`, the command does run against this ref's binary; measured on a fresh build of
`b2f6ac51`'s `solve.c`, `solve --kc-sample` prints `Usage: solve --kc-sample DIR [args] … [--kc-record]`,
i.e. every argument in the recipe is accepted here. The no-prefix rendering was a rationale for a
state that no longer holds; it is left as written, per this file's own rule that a correction records
rather than rewrites. The same stale claim was also carried by `verify.py` itself — in the `--t3-stats`
and `--t3-membership` `--help` text and in the `GENERATING THE INPUT` block both modes print at
runtime — and was corrected in code the same day; `tests.py::TestCliHelpDescribesTheCode` now
asserts that those three texts say "on main" only while `solve.c` literally dispatches `--kc-sample`.]**
...
4. **King Wen's presence was print-only.** `King Wen: YES/No` was never folded
   ...
   complete canonical. *(2026-09-02, code batch V-1:)* the records pass now also
   prints whole-line tokens — `VERIFY=PASS|FAIL`, `KW_PRESENT=YES|NO`,
   `KW_REQUIRED=YES|NO` — and a PASS without King Wen says so in its own
   sentence, so the SCOPE of a default PASS is machine-readable (`grep -qx`)
   rather than inferred from prose. The default itself is unchanged.
...
built on `verify.c`'s `KW[]` (the shared input literal — see the independence note
under the mode table), its own `build_pairs()` and `hamming()`, with the C5
...
`C3 > 776` was completed rather than reported `INCOMPUTABLE`: `./verify --check-repr FILE`
(the fixture as `FILE`) printed `AGREE=1`, `INCOMPUTABLE=0`, `CHECK_REPR=PASS`, rc = 0 on the `C3 = 1080`
...
checkpointing (`--ie-checkpoint`; a checkpoint row failing its hash refuses the resume,
`IE_CHECKPOINT=CORRUPT` rc 1); `--ie-negctl` is a must-differ negative control — **enforced, not
narrated**, see the option synopsis below for `IE_NEGCTL=INAPPLICABLE` / `NO_TARGET`; `--ie-probe NSAMP`
sizes a full run. Every run ends with a `compare :` line and `IE_COMPARED=0` or `=1`: a full-31 run with
no pins, or with `--ie-pin-c6c7` as its only pins, auto-compares to the published exact and a
disagreement is `*** MISMATCH ***` rc 1. Until 2026-09-03 the pinned form compared nothing, so
three forged checkpoints summing to N = 0 resumed under `--ie-pin-c6c7` to `RESULT: pass complete;
all in-run gates hold` rc 0 with no comparison line at all (Codex V2-F59 #1). Validation ladder and
the 2026-07-25 full-scale MATCH: TR-11 §10(vi).
...
Two implementations, no shared code; both were gated against the same fourteen
controls below (a positive plus thirteen negatives; this sentence read "seven" until
2026-09-03 — the negative count before the six rows added 2026-09-02) and agree
counter-for-counter.
...
- `--ie-pin` / `--ie-pin-c6c7` — the pinned-step (T3) variant for `|C1∩C2∩C4∩C5∩C6∩C7|`.
  `--ie-pin-c6c7` as the only pin set, budget on, auto-compares to the published exact
  `516880238445773965371923491676160` (as `--dp-count` already did; added to this route
  2026-09-03); any other pin set has no published exact and needs `--ie-expect`.
...
- `--ie-negctl` — a negative control. A control that does not change the answer is not a control.
  Both routes now refuse to narrate that (2026-09-03): on a rung whose budget has d2 == d4 — the
  published n=13 rung `3.0,4.0,6.2@0`, B0 = (1,6,0,6,0) — the d2/d4 swap is the identity, and
  `--ie-negctl`/`--dp-negctl` exit 1 with `IE_NEGCTL=INAPPLICABLE` / `DP_NEGCTL=INAPPLICABLE`; a
  control with **no comparison value** (no default published target and no
  `--ie-expect`/`--dp-expect`) exits 1 with `IE_NEGCTL=NO_TARGET` / `DP_NEGCTL=NO_TARGET`;
  `--dp-no-budget --dp-negctl` is `DP_NEGCTL=INAPPLICABLE` (nothing to swap); and
  `--ie-mod wrap --ie-negctl --ie-expect N` now compares (before that day the wrap-only control
  compared nothing). Every run prints `IE_COMPARED=0|1` / `DP_COMPARED=0|1` and a `compare :` line
  naming what, if anything, the count was compared to. Measured 2026-09-03 on the n=9 rung
  `3.0,3.1,3.2@0` (B0 = (2,5,0,2,0), d2 ≠ d4): `--ie-negctl` alone → `IE_NEGCTL=NO_TARGET` rc 1;
  `--ie-negctl --ie-expect 26112` → `vs published 26112 : DIFFERS (negative control passed)` rc 0
  under both `--ie-mod all` and `--ie-mod wrap`; `--dp-negctl` alone → N = 38400,
  `DP_NEGCTL=NO_TARGET` rc 1; `--dp-negctl --dp-expect 26112` → `DIFFERS (negative control
  passed)` rc 0; the n=13 rung → `INAPPLICABLE` rc 1 on both routes.
...
budgets, and **the count MUST differ** — that is the control's whole content, and since
2026-09-03 it is enforced rather than printed (see the negative-control bullet under Route B for
`DP_NEGCTL=INAPPLICABLE` / `NO_TARGET` and `DP_COMPARED=`). A `--dp-checkpoint` row failing its hash
refuses the resume with `DP_CHECKPOINT=CORRUPT` rc 1 — the IE route has had that gate since Q-286;
...
`LC_RESUME`-replayed one** (the g-build GA9 invariant, restated at the replay site in `verify.c`),
which is what makes the resumed output comparable to an unresumed one **per layer** — and only per
layer, see below. ⚠ A replayed layer is **asserted from the prior run, not re-read** — `census_got[k]`
is 0 for replayed layers. **A resumed run therefore attests less than a full one;
do not report it as a from-scratch verification.** Since 2026-09-03 the program enforces this
itself: the record is `LC_RESUME_V2` (its header pins `n` and `pl_hash`, so a record from another
run is refused; each row carries an FNV-1a hash of its text, and a row failing it refuses the whole
resume with `LC_RESUME=CORRUPT` rc 1; a pre-V2 file has no row hashes and fails the header compare),
the final label is **not** identical between a straight-through and a resumed run — it reads
`N RE-DERIVED from bytes in this run … M REPLAYED from the LC_RESUME record of a prior run
(asserted, NOT re-read here)` and prints `LAYERS_REDERIVED=` / `LAYERS_REPLAYED=` — and a run in
which **every** present layer was replayed exits nonzero, because it re-derived nothing. The
`re-derived at every layer` wording is printed only when nothing was replayed. (Before that day a
...
⚠ **The three `-D` flags are not decoration and this file is the reason they are here.** (`-DGIT_BRANCH` added 2026-09-12: `tr12_repro_gate.sh:13-14` records that the build line published here defined no branch, so a reproducer's binary emitted `branch=unknown` against expected blocks that diffed the field verbatim — this is the fix that comment asks for. The TR-12 goldens are unaffected: `tr12_repro.sh:388` normalises the trailer to `branch=<BRANCH>`.) The
...
any query-program work — capture your own baseline with `./solve --selftest | grep 'Actual sha256'`
before editing and compare that line after, with exit status 0 both times. ⚠ **[CORRECTED 2026-09-24
(Q-753) — this read `./solve --selftest | sha256sum`, which never matches itself: the first line of
the selftest's output names its random `mktemp` directory, so two runs of one unchanged binary hash
differently. Measured 2026-09-24 on a build of this repository's `solve.c`: two runs, rc 0 both, four
lines of stdout each, whole-output digests different, the `Actual sha256` line identical. Adjudicated
by Fable (Codex V3A-061#5).]** **Never copy a sha out of a document**, this one included.

### Stage 1 — the n=9 reduced-universe battery

**It costs nothing.** Any 2-core machine; no campaign
...
🔴 **It is not, however, "the whole correctness argument" — this passage said that until
2026-09-07 (Codex MQ1 §1), and the claim was stronger than the mechanism.** The battery mixes
two kinds of block. A few are graded against an **independently derived** value — an exact
count, or an identity recomputed by a second route — and those do test correctness. Most are
**stability pins**: a block captured from a first run, compared byte-for-byte on every run
after. A stability pin that matches proves the engine still emits what it emitted before,
which is a regression check. **If the first capture was wrong, every later run agrees with it
forever.** `scripts/tr12_repro.sh --n9 --regen` both generates and grades, so the diff is the
only place that distinction is visible — which is why the instruction is to READ the diff
before committing it, never to regenerate until it is quiet. Recorded as **CX-39**.
...
|---|---|---|
| `--q6-extremes-oracle WALKS.txt` | The **independent Q6 reading-(B) extremes oracle**. `state = (placed-set, LAST ENDPOINT)`, `choice = the UNORDERED pair`, `mass = unweighted raw walk count`. This function **is** the operational definition of reading (B) — the English specification demonstrably is not, having produced two different wrong implementations before this oracle rejected both. Consumes a walk list (one walk per line, comma-separated, as `solve --kc-enum-desc` prints); the harness **regenerates** that list rather than committing it. Shares no code, constant or data structure with the engine's extremes path. | `Q6_EXTREMES_ORACLE=OK\|FAIL` *(2026-09-21: a missing `WALKS.txt` prints `Q6_EXTREMES_ORACLE=FAIL (cannot read PATH: …)` rc 1 and a non-integer field `Q6_EXTREMES_ORACLE=FAIL (PATH:LINE: not a comma-separated integer walk: …)` rc 1 — the mode's existing `FAIL (reason)` form; both were tracebacks with no token until that day. Pinned by `tests.py::TestMissingInputIsRefusedNotCrashed`.)* |
| `--q1-labeling-oracle WALKS.txt` | The **O3 labeling theorem, executed** rather than argued. Ranks one anchor walk under THREE labelings of the same order family (pair-vector lex, then orientation lex) over the fully enumerated universe: `O3` as implemented (label = global KW pair index), `O3-KW9` with labels derived from the anchor itself — the full-31 situation in miniature — and `O3'` intrinsic and KW-free. Needs `--q1-anchor`. The audit's F1 objected that *"a verifier that cannot show rank 0 on the relabeled universe has not tested the claim"*; this is that verifier, and it fails in **both** directions — change the fixture triple and `O3` fails, change the relabeling and `O3-KW9` stops being 0. Asserts the universe size first, because truncating the walk list removes only walks ranked *above* the anchor and would otherwise pass. `n>9` is refused, not attempted: 26,112 walks at n=9, 2.06e12 at n=13. | `Q1_LABELING_ORACLE  PASS`, else `Q1_LABELING_FAIL` and exit 1 |
| `--q1-anchor WALK` | The comma-separated anchor walk for `--q1-labeling-oracle`. Required by it; meaningless alone. | — |
...
| `--nuclear-selftest` | Independent gate on the **nuclear-hexagram (互卦)** operation. Derives it from the **classical line definition** — lower trigram = lines 2,3,4; upper = lines 3,4,5 — working on an explicit bottom-first line list and reassembling, deliberately **not** replicating the engine's shifts, so a wrong shift in `solve.c:6269 f5_nuc` cannot be reproduced here by construction *(the line number is stale — `f5_nuc` is defined at `solve.c:6816` as of 2026-09-21; the function name is the stable anchor, and `verify.py`'s own `--help` for this flag dropped the number the same day)*. Compares on all 64 hexagrams. ⚠ Note for anyone extending it: under a deliberate perturbation the *structural* checks (乾/坤 fixed points, image size 16) **stayed green** — only the per-hexagram comparison caught the fault. Invariants constrain; they do not identify. | `NUCLEAR_SELFTEST=PASS\|FAIL` |
...
(pass `--kc-raw`).
**Added 2026-09-21:** `ATLAS_ORBIT_FRAMES=ERROR (cannot read PATH: …)` rc 2 for a missing, unreadable
or non-JSON atlas — a new value on this token, chosen over `=SKIP` because every grader in this
repository reads SKIP as not-a-failure, and over the `FileNotFoundError` traceback it replaced because a
traceback prints no token at all. Pinned by `tests.py::TestMissingInputIsRefusedNotCrashed`.
...
qualification is carried at `CLAIM_TO_ARTIFACT.md:34`, `CITATIONS.md:416` and `:1501`,
...
4. **The chunked merge path — this is what a real n=31 run uses.** The scan in step 3 is a single
   ...
   f and g ladders' combined **footprint** as then estimated; ⚠ *Q-725, 2026-09-24: the measured
   footprint is 3.29 TB + 8.27 TB = **11.57 TB**, per TR-12 §R.0's measured table, so even that
   estimate was low*) divided by a measured rate. The assumption is wrong, and it
...
battery**, and that sentence of TR-12 is withdrawn until the witness row exists. ⚠ *(corrected
2026-09-25, CX-93: the witness row exists. `a0_q7_witnesses` verifies the two PINNED witnesses
(`reports/evidence/q7_witnesses/`, produced once with kissat 4.0.4) with no solver on PATH —
`--check-arrangement` IN SUPER and IN C15, not KW, every enforced rule re-scored 0 — so
`TR12_Q7_WITNESSES=PASS` and the skip pin no longer depends on `command -v kissat`; the live re-solve
is the opt-in row `a0_q7_resolve` (`--q7-resolve`, pinned `TR12_Q7_RESOLVE=SKIP:opt-in`), which never
compares bytes. The serial number itself is produced by `a2_q7_ranks` at n = 31 and has not been
computed by any run yet. `d5_04_q7_witnesses_gate.sh` now pins the new contract — red on a planted
C1 break, an IN-C15 rule break, King Wen as the witness, a missing and a malformed file; green on the
pinned files; output byte-identical with and without a solver.)* *Q6 / Q10a shell
