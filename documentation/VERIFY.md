...
⚠ **The C3 caveat in the `--check-repr` table row above applies to this
implementation identically, and was executed against it (2026-09-02).**
`vc_repr_of_key` applies the forced opening, the HD-5 exclusion and the C5 budget
only, so a key whose pair sequence has `C3 > 776` is completed rather than
reported `INCOMPUTABLE`; `./verify --check-repr` printed `AGREE=1`,
`INCOMPUTABLE=0`, `CHECK_REPR=PASS`, rc = 0 on the `C3 = 1080` fixture. The
controls in the paragraph above cannot show that, for the same structural reason
the `--check-artifact` controls table cannot: each perturbs a record until an
*existing* check fires, and there is no C3 check to fire.
...
⚠ **What this table structurally cannot show (added 2026-09-01).** Every row here perturbs a
record until an *existing* counter fires. That design can only ever exercise predicates the code
already implements — it is blind, by construction, to a constraint with no counter at all. There
is currently one: **C3**. Both implementations certify `ARTIFACT=PASS` on a record whose pair
sequence violates `Σ|pos(v) − pos(v⊕63)| ≤ 776`, and every row above still behaves exactly as
documented, because there is no `BAD_C3` to fire. *(Extended 2026-09-02: the same omission sits in
the **repr oracle** as well — `repr_of_key` / `vc_repr_of_key` implement a C4/C2/C5 completion
predicate where `RecordConvention.lean`'s is C2/C3/C5 — so `--check-repr` returns a completion, and
`INCOMPUTABLE=0`, for a key the definition says cannot be completed. Executed in both languages on
the same `C3 = 1080` fixture. So the gap is two modes and four code paths, not one mode; found by
sweeping the sibling class of the `--check-artifact` finding rather than by a fresh charge.)*
Closing it means a C3 leg in **all four** of those paths, a `BAD_C3` counter kept
counter-for-counter across the `--check-artifact` pair, an `INCOMPUTABLE`-side C3 rejection in the
repr pair, an eighth row in the table above, and a repr-side control beside it; this note stands
until then. It is the **third instance** of the class the A3
audit above names — an independent verifier inheriting an enumerator-enforced invariant instead of
re-deriving it (the other two are that section's items 1 and 2) — and the first to appear in a
*new* instrument: `--check-artifact` landed 2026-08-15, fourteen days after that audit fixed the
class on the records path. The rule that should have carried forward: **any new record-reading
mode must enumerate its C1–C5 coverage against SPECIFICATION.md and justify each omission in its
docstring.**
