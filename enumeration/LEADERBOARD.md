...
> Canonical sha256 hashes, record counts, and reproducibility parameters are listed in [`../CANONICAL_HASHES.md`](../CANONICAL_HASHES.md). The deepest published partial enumeration is the d3 100T canonical: v1 produces 3,432,399,297 orderings (`915abf30…`) and **v2 produces 3,663,580,914 orderings** (`cc4a5377…`, established 2026-05-23 — **+6.74% over v1 at the same node budget**, see CANONICAL_HASHES.md and HISTORY.md for the campaign narrative). The d3 10T and d2 10T canonicals are at smaller budgets but at fully-validated reference counts. Older figures (742M hash-bug, 31.6M filename-collision bug) appear only as historical context. The difference between d2 and d3 counts is a partition-strategy effect, not a constraint difference. The 100T : 10T ratio of 4.86× (v1) reflects diminishing returns in the search tree (linear node budget yields sublinear new-orderings); the v2 lineage shows the same diminishing-returns shape with the constant offset preserved by deeper pruning.
>
