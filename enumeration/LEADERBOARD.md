...
>
> **Second novel finding (2026-04-20 100T baseline; updated 2026-06-11 with full 560T `--analyze`):** the **boundary-minimum is partition + scale-dependent, NOT monotone in scale**. At d2 10T and d3 10T, 4-boundary subsets uniquely identify KW. At d3 100T, NO 4-subset worked; greedy-optimal was a 5-set **{1, 4, 21, 25, 27}**. At d3 560T (current deepest), **the greedy-ordered minimum drops back to 4** with set **{4, 27, 25, 21}** (boundary 4 alone eliminates 99.999% of non-KW; 27 → 481 survivors; 25 → 14; 21 → 1). And the *unordered* working-4-subset count (§[8]) drops to **0** at 560T, vs 4 at 742M and 8 at 11.2T — so at 560T no unordered 4-tuple of boundaries jointly reduces survivors to ≤ 1; the greedy ordered minimum works only because each boundary's effect compounds on the prior. The "ordered minimum 4 → 5 → 4 across 10T → 100T → 560T" and the "unordered-4-set count 8 → 4 → 0 across 11.2T → 742M → 560T" trajectories together establish that **minimum-set size as a function of scale is non-monotone**; the durable structural claim is the greedy-ordered version ("4 boundaries suffice via greedy application"), reaffirmed at the current deepest scale. Boundaries {25, 27} remain in the greedy minimum across every partition tested. §[9] result: `{25, 27}` is one of the most informationally-independent boundary pairs (ratio 0.007) — explaining WHY they keep appearing. §[10] top pairwise MI at 560T: pos 12 ↔ 13 = 1.3417 bits (cascade-region 11–20 dominates the top-10). §[18] boundary 4 alone yields 45.14 bits of conditional-entropy information gain (of 77.81 bits total baseline) — half of total entropy. See [SOLVE-SUMMARY.md](../documentation/SOLVE-SUMMARY.md) §"560T canonical results" + [HISTORY.md](../documentation/HISTORY.md) "June 10-11, 2026" entry for the full §[1]–§[28] dump.
>
