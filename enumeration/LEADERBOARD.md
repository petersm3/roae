...
>
> **Novel finding from 100T `--c3-min` analysis (2026-04-20, pending 560T re-validation):** KW is **not** the C3-minimum under C1+C2+C3. Minimum complement distance = **424** (221 records); KW sits at C3 = 776 (ceiling of the constraint). The simple axiom "minimize C3" does not derive KW — this is a negative result for Open Question #7 Phase A. 560T re-run on the warm copy is queued and will either reaffirm the 424 floor or surface a deeper minimum-complement-distance class; the finding's structural form (KW ≠ C3-min) is robust under the 100T → 560T scale move regardless because the 100T solution set is a subset of the 560T set, but the specific minimum value may shift downward at the deeper scale.
>
> **Second novel finding from 100T `--analyze` (2026-04-20, partial 560T re-validation 2026-06-09):** the **boundary-minimum increases from 4 to 5** at deeper enumeration. At d2 10T and d3 10T, 4-boundary subsets uniquely identify KW. At d3 100T (the prior deepest canonical), **NO 4-subset works** — the deeper enumeration surfaces orderings that are 4-subset-indistinguishable from KW. Greedy-optimal 5-set at 100T: **{1, 4, 21, 25, 27}**. Boundaries {25, 27} remain mandatory across all three partitions (d2 10T, d3 10T, d3 100T) — the partition-stability claim holds. **Partial 560T re-evaluation 2026-06-09**: in-flight `solve --analyze` on the 560T canonical confirms §[8] still has **0 working 4-subsets at 560T** (boundary-min still ≥ 5); whether the minimum is exactly 5 at 560T or has grown to 6+ requires a later analyze section explicitly testing 5-subsets, currently in progress. New §[9] result: boundary-pair correlation analysis quantifies that `{25, 27}` is one of the most informationally-independent boundary pairs (ratio 0.007), explaining WHY they keep appearing in every minimum set across all partitions. See SOLVE.md §Boundary analysis for the updated text and `solve_c/runs/20260419_100T_d3_d128westus3/analyze_output.log.gz` for the historical 100T raw data.
>
