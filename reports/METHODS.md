...
|---|---|---|---|---|
| C1–C5 space size | 1.3287×10³⁸ (95% CI [1.3283, 1.3292]×10³⁸ — half-width **0.03%**; relerr = SE/mean **0.02%**) ⚠ **[LABELLED 2026-09-19 (Q-663) — as in the \|C1–C7\| row below, the bare trailing percentage was the estimator's relative STANDARD ERROR, not the interval's half-width.** Note that 1.96 × 0.02% = 0.04% does **not** reproduce the 0.03% half-width, because the printed relerr is rounded to two decimals — the unrounded relerr is 0.0173%. The 1.96 factor therefore may not be applied to a rounded published relerr to recover an interval; each half-width here is computed from the archived bracket itself ([evidence/knuth_whole_tree_5e10.out](evidence/knuth_whole_tree_5e10.out):5). No figure moved; only the label is new.]** | **estimate** (Knuth) | raw (orientation-explicit; label unified 2026-08-06 — "orientation-resolved" elsewhere in the suite is the same convention) | [TR-4] §3 |
...
| \|C1–C7\| | 5.21×10³¹ (95% CI [5.13, 5.29]×10³¹ — half-width **1.53%**; relerr = SE/mean **0.78%**) ⚠ **[LABELLED 2026-09-19 (Q-663) — the bare trailing percentage in this cell was the estimator's relative STANDARD ERROR, never the interval's half-width, and a bare percentage standing beside a printed "95% CI" invites exactly the reading that went wrong one row below at \|C1–C7\|, C3 dropped.** No figure moved: the bracket is as archived ([evidence/c67_probe.out](evidence/c67_probe.out):14, `est=5.211639e+31 95%CI=[5.1317e+31, 5.2916e+31] relerr=0.78%`) that archived file contains no `se=` field: the probe prints `relerr`, and relerr = SE/mean per the Statistics conventions section below. 0.78% is as published; only the label is new.]** | **estimate** | raw (orientation-explicit) | [TR-4] §4 |
| \|C1–C7\|, C3 dropped | 516,880,238,445,773,965,371,923,491,676,160 ≈ 5.16880×10³² | **exact** (two-instrument — (i) IE pinned-step recount 2026-07-25, `verify.c --ie-pin-c6c7 --ie-no-quotient`, small-n-validated 52/52, 3-prime-CRT self-consistent; (ii) independent direct mask-DP recount 2026-07-26, `verify.c --dp-count --dp-pin-c6c7` — a different algorithm class (explicit exact-cover subset DP with polynomial budget-coefficient extraction, no inclusion–exclusion, sharing only the problem spec), small-n-validated 44/44 incl. three-way vs brute force, matched the same integer exactly; mod-24 N/A under pins; ⚠ **[FIGURES CORRECTED 2026-09-19 (Q-659) — this cell put the exact count "~0.22% below the point estimate" of the prior Knuth estimate and inside that estimate's 0.25% band, which it called a confidence interval. Both figures were wrong in kind, and the substantive claim survives both fixes.** The archived 5×10¹⁰-probe run ([evidence/c67_probe.out](evidence/c67_probe.out):13) gives point estimate **5.184964×10³²**, 95% CI **[5.1596, 5.2103]×10³²**, and **relative standard error 0.25%** — an SE, not an interval (§"Statistics conventions" below: the estimator prints mean ± 1.96·√(v̂ar/N) and reports relerr = SE/mean; the archived file contains no `se=` field, only `relerr`). The exact count is **0.31170% below that archived point estimate** — **outside** a ±0.25% band, but **inside** the genuine 95% interval, whose half-width is **0.4889%** = 1.96 × the 0.25% SE. The retired ~0.22% was measured against the rounded 5.18×10³² rather than the archived 5.184964×10³². **The calibration claim itself stands and is NOT withdrawn**, because the exact count falls inside the genuine 95% interval; this is a precision correction only. The retired wording is paraphrased rather than quoted here deliberately: quoting it verbatim would leave the mislabel itself on the page.]** lands inside the genuine 95% interval of the prior 5.18×10³² estimate, 0.31170% below its archived point estimate — a 3rd independent estimator-calibration anchor) | raw (orientation-explicit) | [TR-4] §4 |
...
| Orbit count \|C1∩C2∩C4∩C5\|/24 | 45,710,469,949,549,241,251,504,669,632,357,466,112 | **exact** (= N/24 of the two-instrument count above; the recomputed N is ≡ 0 mod 24, so the division is exact) | ⚠ **[LABEL CORRECTED 2026-09-19 (Q-642) — this cell read "record-level orbit", which its own arithmetic forbids.** Record-level objects are canonical pair-orderings, of which there are at most 31! = 8.2228×10³³, so a record-level orbit count is bounded by 31!/24 = 3.4262×10³² — and the figure in this row is 4.5710×10³⁷, **133,415× above that ceiling**. It is the same shape as the orientation-dedup figure withdrawn from this table on 2026-08-24 for exceeding its own 31! ceiling by ~4,013×, and it survived that sweep. N counts orientation-explicit **sequences**, on which the acting group is the order-48 lift acting freely, so the divisor at this level is **48** — which this cell's own parenthetical already said, so the cell contradicted itself. The figure is therefore **2× the sequence-orbit count**; the sequence-orbit count itself is **N/48 = 22,855,234,974,774,620,625,752,334,816,178,733,056**. **The integer in the value column is unchanged and still exact** — it is N/24, and N ≡ 0 (mod 48) is verified — only the name of the object it counts changes. See [TR-11] §2's precision note and [CORRECTIONS.md](../documentation/CORRECTIONS.md) CX-53.]** Orbit under the order-48 sequence-level lift, published as N/24 = 2× that orbit count (S₄, order 24, free action at the canonical *pair-ordering* level; at the orientation-explicit **sequence** level the group is the order-48 lift and the divisor is 48 — [TR-5] §3(i)) | [TR-11] §9 |
...
| Verified proof checker | cake_lpr ([Tan, Heule & Myreen 2021](../documentation/CITATIONS.md#cakelpr2021)) — **pinned**, unlike the row above: commit `a36874a8b750b43fe4b385b8ddbf5b033e46a3fa`, `cake_lpr.S` sha256 `2f3af32d…`. Built with the upstream repo's own default target, `gcc -O2 basis_ffi.c cake_lpr.S -o cake_lpr -std=c99`. **The compiled sha is compiler-dependent and is therefore NOT the portable identity** — gcc 13.3.0 yields `1822ca1e…` and gcc 11.4.0 yields `98c1649d…` from byte-identical `.S` input — so a replicator pins the `.S` sha, and a binary sha names only which build produced a given verdict. On 2026-09-19 all 24 archived certificates passed this checker in one pass of `reports/certificates/verify_all.sh` (24/24 `s VERIFIED UNSAT`, 0 FAIL). The full 64-nibble shas, the upstream `basis_ffi.c` provenance discrepancy, and the scope of what the `machine_code_sound` theorem does and does not certify are in [certificates/README.md](certificates/README.md) §Checker coverage. | [github.com/tanyongkiam/cake_lpr](https://github.com/tanyongkiam/cake_lpr) |
