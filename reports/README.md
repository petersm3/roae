...
| [TR-12](TR12_QUERY_PROGRAM.md) | The Query Program | **The one entry in this table that reports no finding.** It is an executable *specification* — the question set to be asked of the compiled catalog, with each query's exact definition, space label, instrument, output shape and reader-side check. The full-31 answers are not computed in it and are not stated in it; where a number appears it is one already published elsewhere and cited, one printed beside its reproduction command, or a labelled cost band. Its executable contract is [QUERY_INVENTORY](../documentation/QUERY_INVENTORY.md) (one row per item, with the command line and verdict token) and its driver is `scripts/tr12_repro.sh`, both public | specification only — no result to verify; the battery that will run it is gated by `scripts/tr12_repro_gate.sh` |
