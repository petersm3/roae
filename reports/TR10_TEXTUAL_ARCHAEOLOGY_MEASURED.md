...
2. **The pre-registration discipline.** Following the F4' protocol, the nine composites were registered
   ...
   finds no standard errors. ("tier-1"/"tier-2" throughout this report names the **scoring-axis
   class** — principled rules stated ahead of the data, versus data-like ones — never a run scale,
   and never the campaign "Tier 1" of
   [LARGE_SCALE_CAMPAIGNS.md](../documentation/LARGE_SCALE_CAMPAIGNS.md) or the "Tier 1"
   determinism-hardening level of
   [CANONICAL_HASHES.md](../documentation/CANONICAL_HASHES.md). These are 2×10⁹-probe estimator
   runs.) The SEs quoted here were computed separately. Regenerate them with
...
3. **The scoreboard (2026-07-04, 2×10⁹ probes).** Population = the C1–C5 constraint-satisfying space
   (≈1.33×10³⁸ orderings; the run's own raw C1–C5 leaf estimate, 1.3275×10³⁸, matches the established
   figure to 0.09% — a consistency check within the same estimator family (the absolute validation is TR-11's exact anchors)). Fractions are of raw C1–C5 leaf mass. ⚠ **[LABEL CORRECTED 2026-09-03 — this passage and three others in this report read “canonical-leaf estimate”, and this sentence read “Fractions are of canonical mass”. **No figure changes**: 1.3275×10³⁸ is correct, and it is the **raw, orientation-explicit** object — which is exactly what its 0.09% agreement with [SEARCH_SPACE_SIZE.md](../documentation/SEARCH_SPACE_SIZE.md)'s raw 1.3287×10³⁸ shows. But in this corpus *canonical* denotes the orientation-DEDUPLICATED object ([SOLUTIONS_FORMAT.md](../documentation/SOLUTIONS_FORMAT.md) §Deduplication), whose ceiling is 31! ≈ 8.2228×10³³, so the old label put the corpus's own canonical word on a magnitude 16,144× that ceiling — the collision that produced the 2026-08-24 withdrawal one level up (documentation/CORRECTIONS.md). The label was inherited from the binary's printf field `leaves_canonical_C1C5`, which the archived evidence files still carry verbatim and which is queued for rename to `leaves_oriented_C1C5` in the code lane; grep the evidence for the old field name.]**
   ...
   **nothing promotes**. The single honest bright spot for Davis is the #43–50 trigram array (row 3):
   ...
   prefers FDR control may read this row as clearing global correction **on the one-sided mass only**;
   on the two-sided mass this report publishes it would need BH rank *i* ≥ 3, and only one smaller
   ledger value (`dav_rotinv`) is exhibited. Nothing else in the suite changes, since FWER clearance
   implies FDR clearance. ⚠ **[SIDEDNESS CARRIED INTO THE INSTRUCTION 2026-09-24 (v1.18; Codex
   V3A-084#2, Q-742) — this sentence told the FDR reader the row clears global correction with no
   sidedness (registered as RP-e088a322), ten lines after the note above established that two-sided 1.36×10⁻³ does not clear the
   rank-2 bar 1.0989×10⁻³.]**
   ...
   **(i)** Both results are nulls that clear no threshold under any convention — two-sided
   ...
   raw C1–C5 leaf estimate, 1.3275×10³⁸, matches the established figure to 0.09%.
   ...
   **A tautology from a different source, reported in the same data-like class (D-B1, landed
   ...
   sampled raw C1–C5 leaf population (population mean 6.87, observed range 1–20 at 2×10⁹ probes) —
...
5. **His anti-mathematical argument, measured.** Davis's position implies three predictions:
   ...
   mid-scale symmetries are largely what the **C1–C5 constraints** already make unremarkable ⚠ **[CORRECTED
   2026-09-19 (Q-662) — this read "the classical pairing constraints", using that phrase as the
   CONDITIONING SET.** The population every mass in this paragraph is measured against is **C1–C5**, and
   the classical pedigree covers part of it, not the set. Per the taxonomy this project settled on
   2026-09-01 ([CORRECTIONS.md](../documentation/CORRECTIONS.md), the `rev_V2-F14` entry item 4): **C1** is
   classical (孔穎達's 二二相耦，非覆即變); **C4**'s *pair* choice is classical but its **orientation is ours by
   definition**; **C2** is external-modern ([McKenna 1975](../documentation/CITATIONS.md)); **C3** and
   **C5** are project-derived. Note this supersedes the 2026-08-29 ledger entry's "only **C1** is
   classical", which that later entry corrected — C4's pair choice is classical too. **This site is also
   the counter-example to that entry's own census.** It asserted it had found *"the only two occurrences
   repo-wide, verified by grep"*; that grep was an **exact-string** search for "the classical constraints"
   and could not see a paraphrase, so it missed this third site and the count was wrong by one. Measured
   2026-09-19 at `origin/main`: `git grep -F 'classical pairing'` returns **20 lines across 13 files**, of
   which this was the **only** conditioning-set use — the rest name C1 itself, or are registry and
   self-test rows. Not idle: §3's own v1.12 null-sensitivity note shows rows 6–7 **reverse** under a
   different null, so which population a mass is conditioned on is load-bearing here. No mass, tail
   probability, classification or verdict changes. See CX-54.]** —
...
- Population masses and histograms (§3 table): `SOLVE_THREADS=32 SOLVE_KNUTH_SCORE_DAV=1
  ...
  self-validation: same run's raw C1–C5 leaf estimate 1.3275×10³⁸ vs [SEARCH_SPACE_SIZE.md](../documentation/SEARCH_SPACE_SIZE.md)'s
  1.3287×10³⁸, 0.09%)
...
- Wave-2 population masses and histograms (§3b table): `SOLVE_THREADS=32
  ...
  tier-1 run 2026-07-11; self-validation: same run's raw C1–C5 leaf estimate 1.3275×10³⁸ vs
...
| v1.14 | 2026-09-02 | **Stack requirement narrowed to what the binary enforces (prose batch P37, Codex V2-F08 #4; wording only).** The `--estimate-knuth` warning published `ulimit -s unlimited` as REQUIRED. It is a **sufficient** setting, not a necessary one, and on a host or container whose hard limit forbids `unlimited` the published requirement was a false blocker. `solve.c`'s preflight tests `rlim_cur != RLIM_INFINITY && rlim_cur < 16UL*1024*1024` and its message names ">= 16 MB"; executed under TR-9 v1.24, `ulimit -s 8192` refuses and exits 1 while `ulimit -s 16384` runs the estimator to completion. The banner now states "at least 16 MB (`ulimit -s 16384` suffices)" with `unlimited` named as one sufficient setting. This is the sibling sweep TR-9 v1.24 reported but did not perform. No figure, count, command, claim or scope changes |
| v1.15 | 2026-09-03 | **"tier-1"/"tier-2" disambiguated at the §3 evidence citation (hardening lane, Q-86; wording only).** The project spells three unrelated senses "Tier 1"/"tier-1" — the campaign-scale 11.2T canonical, the determinism-hardening level, and this report's scoring-axis class (principled vs data-like) — and the `dav_tier1.out` / `dav2_tier1.out` citations gave a reader no way to tell which was meant, inviting the reading that these are canonical-scale runs. They are 2×10⁹-probe estimator runs. The sense is now stated in place. **No mass, standard error, null, threshold, classification or verdict changes** |
| v1.16 | 2026-09-03 | **The raw C1–C5 leaf estimate is no longer labelled *canonical* (hardening lane, Q-321; wording only).** Six sites in this report gave the orientation-EXPLICIT leaf estimate 1.3275×10³⁸, and the population masses derived from it, the corpus's *canonical* word — which [SOLUTIONS_FORMAT.md](../documentation/SOLUTIONS_FORMAT.md) §Deduplication reserves for the orientation-DEDUPLICATED object, ceiling 31! ≈ 8.2228×10³³. The figure is right and its 0.09% agreement is with [SEARCH_SPACE_SIZE.md](../documentation/SEARCH_SPACE_SIZE.md)'s row explicitly headed **RAW (orientation-explicit)**, so the object was never in doubt — only its noun, which sat 16,144× above the ceiling its own label implies. This is the same raw/canonical collision that produced the 2026-08-24 withdrawal one level up, and it was unguarded by construction: `scripts/doc_gates.sh` GATE 26 refuses exactly this arithmetic but excludes the string *canonical-leaf* by design, so as not to adjudicate terminology. Four sites (§3, §3b, and two Verification-Guide self-validation lines) carried the estimate; the row count is six because §3's *"Fractions are of canonical mass"* and §3b's *"sampled canonical population"* name the same orientation-explicit object and were not in the row's census. The binary's printf field `leaves_canonical_C1C5` is the root and is unchanged here — it is a code change, queued to the code lane for rename to `leaves_oriented_C1C5`, and the archived evidence files (`reports/evidence/dav_tier1.out`, `dav2_tier1.out`) still print the old field name, which is what a reader should grep. **No mass, standard error, null, threshold, classification or verdict changes**; no reproduction command changes |
| v1.17 | 2026-09-19 | **A third conditioning-set site, the one an exact-string grep could not see (Q-662; wording only).** §4's Davis-(b) paragraph described the comparison population as *"the classical pairing constraints"*. The conditioning set is **C1–C5**; it now says so, matching the two sites corrected on 2026-08-29. The correction carries the **later** taxonomy — C1 classical (孔穎達), C4's pair choice classical with its orientation ours by definition, C2 external-modern (McKenna 1975), C3 and C5 project-derived — rather than the 2026-08-29 entry's "only C1 is classical", which the 2026-09-01 `rev_V2-F14` entry corrected. **The census that entry published was also wrong.** It claimed *"the only two occurrences repo-wide, verified by grep"*; the grep was an exact-string search for "the classical constraints" and a paraphrase defeated it. Measured at `origin/main` 2026-09-19: `git grep -F 'classical pairing'` returns 20 lines in 13 files, and this paragraph was the only one using the phrase as a conditioning set (the others name C1 itself or sit in registry/self-test rows). A paraphrase-shaped scan over all 108 tracked markdown files returns this site plus one adjudicated false positive (`documentation/DISTRIBUTIONAL_ANALYSIS.md:604`, which says two classical *orderings* share constraint principles — not a conditioning set). Load-bearing rather than cosmetic: v1.12's null-sensitivity note shows §3 rows 6–7 reverse under a different null. No mass, tail probability, null, classification or verdict changes |
| v1.18 *(current)* | 2026-09-24 | **The FDR reading instruction carries the sidedness its own note states (Codex V3A-084#2, Q-742; wording only).** §3's instruction to the FDR reader said to "read this row as clearing global correction" with no sidedness, while the 2026-09-03 SIDEDNESS note directly above it shows that only the one-sided mass clears the BH rank-2 bar (6.8×10⁻⁴ < 1.0989×10⁻³) and the two-sided 1.36×10⁻³ this report publishes does not. The instruction is now scoped to the one-sided mass, with a ⚠ note in place. No measurement, threshold or verdict changed |
