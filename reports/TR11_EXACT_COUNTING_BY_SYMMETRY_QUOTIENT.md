...
*Technical report — **v1.20** (2026-08-21; the Verification Guide's `verify.c` build line gains the `-lm` it needs to link — the same-day VERIFY.md/METHODS.md `-lm` correction missed this third site, caught by the execution lane (`scripts/exec_lane.sh`) — no number changed; previously v1.19, 2026-08-11, two stale references in v1.18's own revision row corrected — see Revision history).*
