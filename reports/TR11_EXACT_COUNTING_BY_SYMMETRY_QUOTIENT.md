...
([1997](../documentation/CITATIONS.md#huangshisheng1997)), reporting an argument he attributes to
Shen Yijia 沈宜甲 and Dong Guangbi 董光璧, gives 1,625,702,400 as the number of arrangements of the
64 hexagrams when the orderings of the upper and of the lower trigrams are left free, with a single
arrangement once both are fixed (「六十四卦如果任意排列，那就将有1625702400种排法。只有当上下卦都
定好次序，才是唯一的64种排法」, p. 21). The figure is 8!×8!, exact for the two-trigram-order
parametrisation his second sentence describes — the 方阵 (matrix-form) count, as Chen Zhuangwei
([2007](../documentation/CITATIONS.md#chenzhuangwei2007)) reads it; his first sentence's 「任意排列」
taken alone would suggest unrestricted arrangement, which is 64!, and he uses the figure as
rhetorical motivation for a claim about King Wen's intent (「从六十四卦十六亿多种排法中，唯独选出」).
He derives nothing and poses no counting question; Chen (2007) restates the figure with a 7!×7!
refinement (乾 first in both trigram orders) and asks why the received order rather than another, but
likewise poses no enumeration question. *(An earlier revision of this sentence said Huang
"mislabels" the figure as the count of arbitrary arrangements; that read his first sentence without
the second, which states the restriction, and is withdrawn — see
[CORRECTIONS.md](../documentation/CORRECTIONS.md).)* Closed-form counts of simply-constrained
hexagram-ordering spaces therefore predate all of this (Huang 1997, reporting Shen/Dong: 8!×8!
matrix-form orderings; Chen 2007: 62! with the first pair fixed, 7!×7! with 乾 first in both trigram
orders) — counts of the same kind as this
...
9. **The full-scale exact count — LANDED (2026-07-16).** The full-31 run completed on 2026-07-16 on the
   ...
   **N/24 = 45,710,469,949,549,241,251,504,669,632,357,466,112** ⚠ **[LABEL CORRECTED 2026-09-19 (Q-642) —
   this read "record-level orbit count N/24", which its own arithmetic forbids.** Record-level objects are
   canonical pair-orderings, at most 31! = 8.2228×10³³ of them, so a record-level orbit count cannot exceed
   31!/24 = 3.4262×10³², and this figure is 4.5710×10³⁷ — **133,415× above that ceiling**. N counts
   orientation-explicit sequences, on which the order-48 lift acts freely, so N/24 is **2× the sequence-orbit
   count** and the sequence-orbit count itself is **N/48 =
   22,855,234,974,774,620,625,752,334,816,178,733,056**. The clause immediately following already said the
   divisor at this level is 48, so the sentence contradicted itself in its own parenthetical. The label was
   added here on 2026-08-06 (v1.16) to make the counting units explicit; it made them explicitly wrong. The
   integer is unchanged and still exact — only its name moves. See [CORRECTIONS.md](../documentation/CORRECTIONS.md)
   CX-53.]** (24 is the divisor of the record-level S₄ action, not of this count; at the
   orientation-explicit sequence level orbits have size 48 and N/24 is 2×
...
10. **Honest limits — what stays estimated, and why.** (i) The flagship **1.3287×10³⁸ (|C1–C5|) remains
   ...
   **remains a statistical estimate**, now for stated economic rather than structural reasons. Two
   ...
   the bridge to `solve.c` is still carried by prose, the runtime gates, and the reduced-instance
   agreement ladder: plain DP against the quotient on the U1–U3 unions (9–13 pairs; `--f1-subset`) and at
   n = 18 (`verify.py --recount-rung 18`, since 2026-08-10), and in-RAM against out-of-core quotient
   runs up to n = 28 ⚠ **[CORRECTED 2026-09-24 (v1.29; Codex V3A-085#3, Q-742) — this read "the n ≤ 28
   plain-vs-quotient agreement". The 24–28 rungs compare the quotient with itself across two storage
   modes, and both modes call the same `f1c5_gather_entries` kernel (item 7 above). That
   concordance tests storage and streaming. It does not test the gather arithmetic against a plain
   recursion, which has been done only up to n = 18.]** — specifically it is not machine-checked that `f1_gather_layer`/`f1c5_gather_entries`
...
- Divisibility gate on the full-31 count, reader-side: reduce
  ...
  = 0; that ÷ 24 = 45,710,469,949,549,241,251,504,669,632,357,466,112 ⚠ **[LABEL CORRECTED 2026-09-19
  (Q-642) — this read "record-level orbit count = that ÷ 24". It is not one.** A record-level orbit count is
  bounded by 31!/24 = 3.4262×10³² and this figure is 4.5710×10³⁷, 133,415× above it. It is **2× the
  sequence-orbit count**, the sequence-orbit count being **N/48 =
  22,855,234,974,774,620,625,752,334,816,178,733,056**. Integer unchanged and still exact; label only. §9
  and [METHODS.md](METHODS.md) carry the same correction — see
  [CORRECTIONS.md](../documentation/CORRECTIONS.md) CX-53.]** — the
  ÷24 is the divisor of the record-level S₄ action; sequence-level orbits have size 48, per §2's precision note).
