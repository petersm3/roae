...
2. **The first-wave scoreboard (2026-07-02).**
   ...
   tendencies rather than exact laws; (d) Cook's final-pair anchor is real but **mostly class structure** —
   the measured 7.84% looks like a ×2.4 enrichment against the naive 1/31 ≈ 3.2258%, but 1/31 is the wrong
   reference: the wrap-parity theorem makes 15 of the 31 non-initial pairs ineligible to close (all 16 even
   pairs are barred from the final slot, and the pure pair, itself even, is already fixed at the opening by
   C4), so the counting baseline is 1/16 = 6.25% — a **reference, not a null**, because reading that
   16-element eligibility *support* as a probability of 1/16 assumes an exchangeability across the 16
   eligible pairs that [TR-7](TR7_CIRCULAR_READING.md) §"The anchors on the circle" does not prove and
   that its own measured wrap-class masses (65.2 / 17.5 / 17.4% for d = 3 / 1 / 5, against the counting
   62.5 / 18.75 / 18.75%) already contradict. Priced against the **measured** 6.52% average of the ten
   d = 3 closers (65.2% / 10), **×2.02 of the apparent enrichment tracks class structure** (6.52 / 3.2258)
   and only **×1.20 is the A₂-specific residual** (7.84 / 6.52); the two compose to 7.84 / 3.2258 = ×2.43,
   the ×2.4 observed. *(Corrected 2026-08-01: this entry priced the anchor against the 1/31 uniform-slot
   null and attributed it to "C5's transition budget favouring a distance-6 closer" — both superseded. Per
   ...
   it is demoted to at most a candidate mechanism for the residual — ×1.25 as then priced, ×1.20 now.)*
   *(Corrected 2026-09-02: from v1.21 through v1.31 this entry split the ×2.4 as "×1.9 of the apparent
   enrichment is parity-forced (it holds for every C4+C5 ordering) and only ×1.25 is the contingent
   residual", off the counting baseline 1/16 as though it were a null — the split TR-7 v2.3 withdrew
   earlier the same day for converting an eligibility support into a probability — and it said "16 of the
   31 non-initial pairs ineligible", whose own arithmetic gives 1/15, the miscount
   LITERATURE_RULES_POPULATION_TESTS.md corrected on 2026-09-01 without reaching this copy. What the
   theorem forces for every C4+C5 ordering is the 16-pair support, not a ×1.9. Nothing was re-measured:
   7.84%, the 65.2% d = 3 wrap mass and the ten-closer class are figures TR-7 already prints.)*
...
3. **The extended scoreboard: 31 further rules (2026-07-03 batch, 2×10¹⁰ probes).** The full candidate
   ...
   McKenna & Mair 1979, Drasny, and Schöter — 31 rules formalized (two-language verified, **27 of the
   31** reproducing its source's stated KW values; the other **four** — MM-T3, MM-T6, C1 and the C2
   histogram — are **KW-measured anchors**, because the registry states only qualitative or percentile
   expectations for those), measured in one run. ⚠ **[CORRECTED 2026-09-02 — this clause claimed the
   provenance for *each* of the 31. `solve.py`'s own comment directly above `REGISTRY_KW_EXPECTED`
   (`solve.py:8078-8082`) says otherwise for four of them, and the 31-entry list below it is what
   `--registry-verify` checks: 31 entries, 4 anchors, so 27. The same wording was corrected in
   [LITERATURE_RULES_POPULATION_TESTS.md](../documentation/LITERATURE_RULES_POPULATION_TESTS.md) on
   2026-08-28; this report was the site that correction did not reach, because here the clause spans a
   line break and is spelled `KW` rather than `King Wen`. No rule, measurement, mass or verdict
   changes — the registry ran and passed on all 31 either way; what changes is what the 31 checks are
   compared *against*. See CORRECTIONS.md]** Three headline findings. **(1) A new strongest
...
4. **SAT-decided exactness: the precursors and the 3-edit events.** Population fractions are statistical; a
   ...
   achieved): an orientation flip at slot 7, an adjacent-pair swap at slots 21/22, and the orientation
   reversal of the pair that swap moves into slot 22 (binary (31,62) → (62,31); 0-based slots). ⚠
   **[RECIPE CORRECTED 2026-09-24 (v1.34; Codex V3A-087#3, Q-742) — this read "an orientation flip at
   slot 7 and an adjacent-pair swap at slots 21/22", which leaves one rhythm break: executed with the
   archived scorers (`reports/evidence/f11/f11_events.py`), that recipe scores (parity, rhythm, gender) =
   (0, 1, 0) whether "slot 7" is read 0- or 1-based, and adding the slot-22 reversal gives (0, 0, 0),
   C1–C5-valid at C3 ≤ 776 — the archived k = 3 hits in `f11_events.json` (swap 21, flips {6, 22} or
   {7, 22}). The edit count is unchanged: the reversal falls on a slot the swap already touched, so
   it is still 3 slot-edits.]** The three rules'
   ...
   need not be unique. Also SAT-verified: the parity-alternation theorem over the full space (a mechanized
   *corroboration* of the counting step, **not** a third independent verification — the prose proof and the
   Lean-checked lemmas are the two independent legs; "≤14" and "≥16" both
   UNSAT under C1+C2+C4+C5) ⚠ **[CORRECTED 2026-09-02, prose batch P37 — this ranked the SAT check as a third
   verification independent of the prose proof and the Lean-checked lemmas (the retired wording is
   registered in RETRACTED_PHRASES.tsv; it is not restated here, so GATE 3 binds this site too). The 2026-08-29 retraction carried at
   [TR-6](TR6_PARITY_SKELETON.md) §Abstract established that the SAT leg is NOT independent of the prose
   leg: `BETWEEN_MULTISET` forces 2(d=1)+13(d=3)=15 odd-distance slots, so both targets are refuted by
   C5's cardinality clauses alone, and the encoding assumes the alternation-to-odd-distance bridge that
   the Lean leg proves. The retraction reached README.md, CLAIMS_DECIDED.md, CLAIM_TO_ARTIFACT.md and
   LITERATURE_RULES_POPULATION_TESTS.md but not this site, which spans a line wrap and was invisible to
   every line-based grep for it. The THEOREM, the DRAT certificates and every figure here are
   UNAFFECTED. See CORRECTIONS.md]**; joint-strict population size (pinned-walk estimate) ≈1.13×10²⁹ canonical orderings (95% CI [1.09, 1.17]×10²⁹; relerr 1.66%) ⚠ **[CORRECTED 2026-08-28 — the ±4.7% was a PREREGISTERED ANCHOR TOLERANCE BAND, not an error bar: `reports/evidence/f11/compute_f11_bf.py:85` names its check "Moore-joint size outside the +/-4.7% anchor band". The published 1.13×10²⁹ comes from `reports/evidence/r11/r11_moore_strict.out` (`est=1.131036e+29`, 95%CI [1.0942e+29, 1.1679e+29], relerr 1.66%), NOT from `f11_runB.out`, which reports 1.16583e29. See CORRECTIONS.md]**. The encoder round-trip validation's first solver model, pleasingly, is King Wen
   itself.
5. **The conflict theorem (2026-07-04, SAT-decided, drat-trim verified): perfection was never available.**
   ...
   data-like rule (highly specific); the theorem is about the **strict, zero-exception forms** of the
   literature's rules as implemented (`sat.py`'s `moore-strict` and `rc4-strict` encodings). ⚠
   **[SCOPED 2026-09-24 (v1.34; Codex V3A-087#7, Q-742) — this sentence said the theorem covers the
   rules in their authors' own formulations (registered as RP-6086b15a). The sources state the gender rule with one
   admitted exception pair at adjacent class positions (the convention-stability note in
   [LITERATURE_RULES_POPULATION_TESTS.md](../documentation/LITERATURE_RULES_POPULATION_TESTS.md)); the certified conflict is of the strict forms, and no claim is made
   here for the source-stated exception form.]** Certificate chain: all four UNSAT proofs (alt-le-14, alt-ge-16, moore-strict-near-2,
...
7. **The orientation layer, measured (2026-07-05; re-scoped + pair-only re-check 2026-07-26).**
   ...
   **C4's definition** *(re-scoped 2026-07-26: the earlier text said "forced (Theorem 6)" — that
   ...
   | 3 | Rising/falling alternation — Moore 1989 rule K-8, **as adapted** in `f5_ground_truth.py` (see note below the table) | 10/17 | 0.6972 | null |
   ...
   ⚠ **[ROW 3 ADAPTATION STATED 2026-09-24 (v1.34; Codex V3A-087#2, Q-742) — two scorers for Moore's
   1989 rhythm rule ship in this repository and they differ. Row 3 is `rf_alt` in
   `reports/evidence/f5/f5_ground_truth.py`: it drops complement and non-directional pairs and counts
   alternations between consecutive *surviving* directional symbols, so an exempt pair is skipped over.
   The scorer behind TR-2's (2, 2, 2) is `rhythm_breaks` in `reports/evidence/f11/f11_events.py`, which
   treats an exempt pair as an interruption and resets the comparison there. Row 3's p = 0.6972 is for
   the first form only. It has not been re-enumerated under the second here, so this row is not a
   measurement of "Moore's rule" as distinct from this adaptation of it.]**
   ...
   *(Scope corrections, 2026-08-01. **(i)** Neither control is a member of that population — SOLVE.md
   ...
   verified by direct `vdb_nucorient` scoring); its instrument and raw output **now ship** as
   `reports/evidence/f5/f5_pair_only_fiber.py` / `.out` and
   `reports/evidence/f5/f5_pair_only_verify30.py` / `.out` (landed 2026-09-04, v1.33 — the
   follow-up commit this sentence promised on 2026-07-26 and which read as an open promise for
   the six weeks between; the published copies rerun byte-identically to the archived outputs,
   ~21 s and ~56 s, and the reproduction commands are in
   [evidence/f5/README.md](evidence/f5/README.md)).
...
*The conflict theorem's trade-off (§5). King Wen (red) misses the three graded rules by 2 each ⚠ **[CORRECTED 2026-09-01 — this caption read "by the minimal measured margins (2 each)"; "minimal" is UNSUPPORTED on the same grounds §5 gave on 2026-08-28: `reports/evidence/f11/f11_runA.out` carries `f11_hist 1 1 0` and `f11_hist 2 1 1`, both componentwise better than KW's `2 2 2` and with nonzero mass, and that histogram is not CC-N4-conditioned, so no extremal check exists. The rendered image above carried the old wording until 2026-09-02, when `viz/report_figures.py`'s drawn string was fixed and the PNG/SVG were regenerated (prose batch P73); image and caption now agree.]** and keeps the S25–28 trigram configuration exactly; the grand unified
...
- Population fractions, both waves: `SOLVE_KNUTH_SCORE=1 SOLVE_KNUTH_SCORE_REG=1 ./solve --estimate-knuth 20000000000`
  ...
  `solve.c` `knuth_score_reg`, default 0), so the command as printed produced none of the registry rows,
  ccn4 included. **Thread count not recorded (Codex V3B-14#13):** a sampled draw reproduces only at the
  same (probes, threads), and no archived output or record in this repository names the run behind these
...
- Joint-strict population size (§4, ≈1.13×10²⁹, 95% CI [1.09, 1.17]×10²⁹) ⚠ **[CORRECTED 2026-08-28 — the ±4.7% was a PREREGISTERED ANCHOR TOLERANCE BAND, not an error bar: `reports/evidence/f11/compute_f11_bf.py:85` names its check "Moore-joint size outside the +/-4.7% anchor band". The published 1.13×10²⁹ comes from `reports/evidence/r11/r11_moore_strict.out` (`est=1.131036e+29`, 95%CI [1.0942e+29, 1.1679e+29], relerr 1.66%), NOT from `f11_runB.out`, which reports 1.16583e29. See CORRECTIONS.md]**: `SOLVE_KNUTH_SCORE=1 SOLVE_KNUTH_R11_HIST=1
  ...
  [METHODS.md](METHODS.md) §"Reproducibility rule for estimator output". The `SCORE`/`R11_HIST` flags are report-only additions carried by the
  archived run.)
...
- Orientation battery, the **two-opening** (pair-only-C4, 2,703,360-vector) re-check:
  `cd reports/evidence/f5 && python3 f5_pair_only_fiber.py` (~21 s; rerun exercised 2026-09-04,
  byte-identical to the archived reports/evidence/f5/f5_pair_only_fiber.out) and
  `python3 f5_pair_only_verify30.py` (~56 s; direct `vdb_nucorient` re-scoring of the two X=30
  vectors, byte-identical to reports/evidence/f5/f5_pair_only_verify30.out)
- Orientation battery, population run: `SOLVE_KNUTH_SCORE_F5=1 SOLVE_KNUTH_F5_HIST=1 SOLVE_THREADS=64 ./solve
  --estimate-knuth 2000000000` (archived instance: reports/evidence/f5/f5_tier1.out; flags documented in
  SOLVE_C_CLI.md §ENVIRONMENT) ⚠ **[THREAD PIN ADDED 2026-09-25 (v1.35; Codex V3B-04, F1 thread-pin
  class, Q-758) — the command carried no `SOLVE_THREADS`. The archived output names the count: the first
  line of reports/evidence/f5/f5_tier1.out reads `KNUTH-ESTIMATE probes=2000000000 threads=64`. Knuth
  seeds are per-thread, so only (2×10⁹, 64) reproduces that file's digits.]**
...
| v1.28 | 2026-09-01 | **C4's orientation narrowed from attested to definitional at four §7 sites (the 2026-08-30 correction propagated).** [METHODS.md](METHODS.md) §"Constraint set" established on 2026-08-30 that the *Xugua* attests only that the {Heaven, Earth} pair opens, and not the order of Heaven over Earth within it — 天地 is a compound, not an ordering — so what the classical record carries is C1's pairing rule (孔穎達《周易正義·序卦傳疏》, 二二相耦，非覆即變), verified from the primary text, and C4's *pair choice*, which is unaffected. Four sites in this report still gave C4's orientation the wider classical epithet: §7's re-scope parenthetical, the Van den Berghe fiber-ceiling sentence, and the two clauses of the 2026-07-26 pair-only re-check that credited a *classical* rather than a *received* opening with forcing his declared exception. All four now say **definitional — this project's convention**, and where the point is about the received text they say **received**. **No number, count, fiber size, p-value, verdict or certificate changed**: the 12-of-1,720,320 C4-oriented result, the 2-of-2,703,360 pair-only result, both P(X ≥ 29) figures and every frozen 2026-07-05 value stand exactly as published — the orientation is fixed by C4 either way, so the fibers are the same fibers and only the epithet on their defining bit moved. The v1.16 row above retains the superseded epithet as an append-only historical record, per the v1.24 precedent |
| v1.29 | 2026-09-02 | **The 2026-08-29 SAT-independence retraction reaches its last dependent, and the stack requirement is narrowed (prose batch P37, Codex V2-F08 #3–#4).** (1) §4 called the SAT decision of the parity-alternation theorem the theorem's third verification independent of the prose proof and the Lean-checked lemmas. That independence was retracted at [TR-6](TR6_PARITY_SKELETON.md) on 2026-08-29 — `sat.py`'s `BETWEEN_MULTISET` fixes 2 between-pair slots at d=1 and 13 at d=3 and the encoding defines `odd[s]` as `T[s,1] ∨ T[s,3]`, so \|odd\| = 15 identically and both `alt-le-14` and `alt-ge-16` are refuted by C5's cardinality clauses before any ordering variable is consulted, while the alternation-to-odd-distance bridge that carries the content is ASSUMED by the encoding and PROVEN by the Lean leg. The retraction had reached README.md, CLAIMS_DECIDED.md, CLAIM_TO_ARTIFACT.md and LITERATURE_RULES_POPULATION_TESTS.md; **this site was missed because the phrase spans a hard line wrap** and no line-based grep could see it. Found by flattening the corpus before matching, the check GATE 3 performs. Three needles are now registered, where none existed before. **The theorem, the DRAT certificates and every figure in this report are unaffected.** (2) **Stack requirement narrowed to what the binary enforces (prose batch P37, Codex V2-F08 #4; wording only).** The `--estimate-knuth` warning published `ulimit -s unlimited` as REQUIRED. It is a **sufficient** setting, not a necessary one, and on a host or container whose hard limit forbids `unlimited` the published requirement was a false blocker. `solve.c`'s preflight tests `rlim_cur != RLIM_INFINITY && rlim_cur < 16UL*1024*1024` and its message names ">= 16 MB"; executed under TR-9 v1.24, `ulimit -s 8192` refuses and exits 1 while `ulimit -s 16384` runs the estimator to completion. The banner now states "at least 16 MB (`ulimit -s 16384` suffices)" with `unlimited` named as one sufficient setting. This is the sibling sweep TR-9 v1.24 reported but did not perform. No figure, count, command, claim or scope changes |
| v1.30 | 2026-09-02 | **The §5 figure's own pixels stopped carrying the retracted margin superlative (prose review, batch P73; Codex V2-F04 #8, adjudication batch 6).** v1.27 corrected this caption and recorded, in the marker itself, that the rendered image still carried the withdrawn wording because its generator was outside that pass's scope. That is now closed: `viz/report_figures.py`'s drawn string was corrected and the PNG/SVG regenerated, after verifying the generator reproduces the committed PNG byte-for-byte so the only change in the image is the sentence. The evidence was re-derived rather than taken from the marker: `reports/evidence/f11/f11_runA.out:112` is `f11_hist 1 1 0` at 4.1291082539e-09 and `:320` is `f11_hist 2 1 1` at 2.9255247935e-08, both nonzero and componentwise no worse than King Wen's `2 2 2`, and that histogram is not CC-N4-conditioned. The caption's marker is updated to say the image and caption now agree. This report is edited only for that marker; the batch's other charges are TR-2's. No theorem, certificate, count or verdict changed |
| v1.31 | 2026-09-02 | **A source-provenance claim corrected at the one site the 2026-08-28 correction did not reach (prose review, batch P65; Codex V2-F63 #6, adjudication row 26).** §“The extended scoreboard” said the 31 formalized literature rules were “two-language verified, each reproducing its source's stated KW values”. That is true of **27**. `solve.py`'s own comment directly above `REGISTRY_KW_EXPECTED` (`solve.py:7343-7347`) names four exceptions — MM-T3, MM-T6, C1 and the C2 histogram are **KW-measured anchors**, because the registry states only qualitative or percentile expectations for those — and the list it introduces holds 31 entries, so 31 entries minus 4 anchors = 27. `documentation/LITERATURE_RULES_POPULATION_TESTS.md` was corrected to 27 on 2026-08-28 and `documentation/CORRECTIONS.md` records that correction; this report kept the unqualified wording for the five days between (2026-08-28 to 2026-09-02). **Why it survived the sweep, and this is the transferable part:** here the clause spans a hard line wrap *and* is spelled `KW` where the corrected site spells it `King Wen`, so neither a line-oriented grep nor a long-spelling grep could see it — found only by flattening every tracked file before matching. The abbreviated spelling is now registered in `RETRACTED_PHRASES.tsv` (`RP-502d8730`), which GATE 3 matches on the flattened text. `tests.py` checks only the registry’s numeric PASS banner and exit code, so provenance was invisible to it and still is. **No rule, count, mass, p-value, theorem, certificate or verdict changes**: `--registry-verify` ran and passed on all 31 before and after — what moves is the stated origin of four of the expected values it checks against |
