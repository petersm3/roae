...
- **What is demonstrated.** The 560T artifact's sha256 was derived twice from scratch, weeks apart, on
  ...
  byte-identical both times. Independent re-derivation across machines, regions, thread counts and merge
  paths is a *result*, not an aspiration. **Shard-partition invariance is demonstrated at 5.6T and 100T
  and inherited at 560T:** both 560T derivations used the same depth-3 partition
  ([PARTITION_INVARIANCE.md](../documentation/PARTITION_INVARIANCE.md) §5a).
  ⚠ **[SCOPED 2026-09-24 (v1.15; Codex V3A-089#2, Q-742) — this bullet listed "shard partitions" among
  what the 560T re-derivation demonstrated. PARTITION_INVARIANCE.md records that a partition-strategy
  re-run at 560T "has not been performed", and that the 2026-06-30 re-run "re-ran the same depth-3
  partition", so partition invariance there is inherited from the 5.6T and 100T direct witnesses.]**
...
suppressed an interactive refusal that a scripted run never receives in the first place — `mkfs.ext4`
enables that check only when stdin and stdout are both terminals — so the flag was not what stood
between the command and the disk, and omitting it would not have saved the disk. Three rules followed
(the first of which is a discipline, not a safeguard): `mkfs -F` is banned
...
./solve --verify           # C1-C5 + sorted + no duplicates; King Wen REPORTED, not enforced
                           # (add --expect-kw to make its absence a FAIL); gz-aware: reads raw or gz
```

⚠ **[CORRECTED 2026-09-07 — the `--verify` comment above promised a check the binary does not
...
so gating on absence would be a false reject, and `tests.py`'s `TestSolveVerifyKingWenScope` pins the
reported-not-enforced contract with a mutation test that goes red on exactly that change. An operator
who does want the assertion on a complete canonical passes the opt-in `--expect-kw` (added 2026-09-04,
mirroring `verify.py --expect-kw`), which folds absence into the verdict and prints `KW_REQUIRED=YES`.
The promise, not the behaviour, was the defect — the same disposition `solve.c` records for
`--validate`'s runtime banner, which carried the identical wording and was corrected 2026-09-04
([SOLVE_C_CLI.md](../documentation/SOLVE_C_CLI.md#--validate);
[CANONICAL_HASHES.md](../documentation/CANONICAL_HASHES.md) §"How to verify a `solutions.bin`").
No sha, record count, or canonical anchor changed.]**
