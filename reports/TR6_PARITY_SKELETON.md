...
[evidence/f4p_tier1.out](evidence/f4p_tier1.out), regenerable via `SOLVE_KNUTH_SCORE_F4P=1 SOLVE_THREADS=32 ./solve
--estimate-knuth 2000000000` — flag documented in [SOLVE_C_CLI.md](../documentation/SOLVE_C_CLI.md); ⚠ **[THREAD PIN ADDED
2026-09-25 (v1.9; Q-758, Codex V3B-09#14) — the command carried no `SOLVE_THREADS`. The archived output's first line reads
`KNUTH-ESTIMATE probes=2000000000 threads=32`, and only (2×10⁹, 32) reproduces its digits. `par_switch` is constant, so
the discovery itself does not depend on the draw.]**) before being proved — the measurement
