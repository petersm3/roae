...
count, reporting the median rarity with a CI.)* **Look-elsewhere context (F-32):** the suite's
...
suite applies throughout — and by **≥~10× under Benjamini–Hochberg FDR at q = 0.05** on the same
...
Under either family the dof-matched comparison above shows much of even the smaller (Bonferroni) margin is
attributable to specification, so nothing in this report's conclusion turns on which family a reader prefers. The multiple-comparisons accounting for the battery as a whole is in
