...
| v1.8 *(current)* | 2026-07-31 | **Trust-base label refreshed (kernel migration).** The §Commands note on `within_pair_even_nonzero` still carried v1.4's `native_decide` label; since the 2026-07-27 kernel migration the lemma is kernel `decide` (see lean/README.md's trust-base note). Label updated; no result changed |
