...
sequence remain unexplained (exact figure depends on which layers are granted explanatory standing: 105.4 = log₂|C1–C7| keeps every cut, even the data-like pins; 139.1 = log₂|C1∩C2∩C4| is the residual against the claimed-explanatory layers alone)** — the honest measure of how much of the sequence's information is still unaccounted for. It is not evidence that structure remains to be found: at the C5 layer the same pipeline leaves a residual of the same size for a random C1∩C2 ordering under its own extracted multiset (decoy control, N = 1,000, King Wen at the 65th percentile — §2 population context, §4, and the Verification Guide), and whether the residual is structure or noise is the open question of §5(g).
...
4. **The residual — the honest thesis.** Knowing everything structural in this table, the sequence retains
   **log₂|C1–C7| = 105.4 bits** of unexplained information — the most conservative reading: unexplained
   ...
   those layers standing (the 129.7 reading is measured to be typical of a random C1∩C2 target under its own extracted multiset — 65th percentile, N = 1,000, §2 population context; the 3.0-bit C3 cut that separates it from 126.6 has no decoy measurement yet; and the 139.1 endpoint is by construction the same for every such target); note the direction of the v1.22 correction that produced this range: each
...
- Decoy control at the C5 layer (2026-09-04; §2 population context, §4). Sampler and estimator are both shipped, and the run is their composition on a binary built from this repository's `solve.c` (stack limit as for every `--estimate-knuth` command — `ulimit -s 16384` suffices). **The thread count is load-bearing:** Knuth seeds are per-thread, so `SOLVE_THREADS=2` reproduces the archived estimates digit-for-digit (checked on rows 1–3, 5, 204 and 205 of the archived file, including the King Wen-multiset row; `SOLVE_THREADS=1` gives different digits), while another thread count reproduces the percentile only to within the estimator's ≈4 % per-decoy error.
  ...
  Archived output: [evidence/decoy/decoy_cardinality_n1000_2026_09_04.tsv](evidence/decoy/decoy_cardinality_n1000_2026_09_04.tsv) — 1,000 `estimate<TAB>multiset` rows plus one stray `0.000000e+00` line (line 206, no multiset) that the 2026-09-04 harness wrote and that is kept as written; drop it with `NF==2`. Provenance and the percentile arithmetic: [evidence/decoy/README.md](evidence/decoy/README.md). `awk -F'\t' 'NF==2 && $1+0 < 1.097051e39' … | wc -l` → **651 of 1,000**. One decoy returned an estimate of 0 — no probe reached a leaf at 10⁵ probes, which bounds its count loosely from above and is not a proof of emptiness. Per-decoy relative error at 10⁵ probes is ≈4 %, i.e. ≈0.06 bit against the 2.4-bit decoy interquartile range. What the control does not measure: any decoy's own C3 cut (the 126.6 row), and any per-boundary rate ([TR-4](TR4_SIZE_OF_THE_SPACE.md) §Update v1.7).
- C2 start-free rarity **4.29341%** (1 in 23.29), the exact start-unpinned |C1 ∩ C2|. ⚠ **[REPRODUCTION
  ...
  today (`grep -n 'E1 F1U exact' solve.c`). Three runs at distinct primes below 2⁶² (62 bits each), then offline CRT: ⚠ **[CORRECTED 2026-09-25 (v1.30; Q-700, Codex V3A-054#5 swept to its sibling) — this read "distinct 63-bit primes, then offline CRT".** The three moduli below are 2⁶² − 57, 2⁶² − 87 and 2⁶² − 117, and `--f1-mod` refuses any P ≥ 2⁶² (exit 71, executed 2026-09-25); [SOLVE_C_CLI.md](../documentation/SOLVE_C_CLI.md) §`--f1-exact-c1c2` carried the same wording and is corrected the same day.]**
  ...
  9.81×10⁵⁵) ⚠ **[CORRECTED 2026-09-25 (v1.30) — the product was printed as 9.81×10⁵⁶, one decade high.** Computed exactly from the three moduli: p₀p₁p₂ = 9.808×10⁵⁵. The rigour argument is unchanged, because 1.13×10⁴⁵ is below either value.]**
...
output [evidence/f4p_tier1.out](evidence/f4p_tier1.out) — **"tier-1" naming the *scoring-axis
class* (principled rules stated ahead of the data), not a run scale: this is the 2×10⁹-probe
