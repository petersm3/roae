...
The point of this directory is that **you do not have to trust our solver**: each result ships
as a DRAT certificate — a step-by-step logical derivation that any third-party
...
re-verified end-to-end on separate hardware before publication. ⚠ **[SCOPED 2026-09-24 — this sentence also released the reader from trusting our code and
ourselves (Codex V3A-096#1 / V3A-149#1; Q-742; registered as RP-8643c201). The
certificate removes the solver from the trust chain and nothing else: the formula the checker replays
against is regenerated by *our* encoder (`sat.py --emit-cnf`), so a DRAT replay certifies that the CNF is
unsatisfiable, never that the CNF means what the prose says it means. The identical sentence in
[TR-2](../TR2_THE_RULES_CONFLICT.md) (§"What a certificate is") was scoped the same way on 2026-09-03 and
this sibling was missed. `lean/SatEncodingFidelity.lean` machine-checks a *model* of the encoder, and
its own header states that the bridge from that model to the shipping Python encoder is not
machine-checked; the two-way encoding validation TR-2 describes is what addresses that gap, not the
certificate. The "In short" sentence below inherits the same scope: the fact is machine-checkable
*given* the regenerated CNF.]**
...
| artifact | contents |
|---|---|
| `c3_positional_witnesses.txt` | 42 verified C1∩C2∩C4∩C5 orderings: one at **every** integer rung G = 12..51 (G = 12 is the structural floor — 12 complement couples in pairwise-distinct slots give G ≥ 12 by counting, and it is **achieved**, so the constraints impose no floor above the trivial minimum; KW sits 83 above it); one at G = 95 whose pair-slot layout differs from KW's (`--not-kw` — the G = 95 tie class is not KW-unique, engine-independently); and one at G = 97 > 95 (the region above the C3 ≤ 776 cap is populated). The 560T-population minimum G = 51 is truncation-biased: SAT reaches G = 12. Regeneration commands are in the file header; `verify_all.sh` §3b rechecks every line through `verify.py`'s independent functions and, since 2026-09-24, asserts that the G values are exactly {12..51, 95, 97}. ⚠ **[CORRECTED 2026-09-24 (Q-742) — from 2026-08-01 (a15c6ddc) until 2026-09-24 §3b did NOT do what this cell said: a comment inside its Python heredoc contained a double quote, which ended the shell word early, so only the permutation test ran and the C1/C2/C3/G/C4/C5 and count checks never executed while the section printed PASS (it passed an empty witness file). The count gate also never checked coverage (Codex V3A-097#1). Re-run with both fixes: all 42 archived witnesses pass every check, so the data were sound and the checker was not.]** |
