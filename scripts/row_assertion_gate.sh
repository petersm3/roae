...
a0_build	TR12_BUILD	DRIVER-BUILT	the driver-computed SELFTEST_EXPECTED_EQ_ACTUAL=YES|NO line (:727-731) is printed and asserted by nothing: if either sed finds no sha the row prints NO and still exits 0. The enumeration sha itself IS carried, by solve.c's rc=40 on --selftest mismatch, so the row is not blind -- this one derived line is. G-31 2026-09-11 group A0 records the same.
a1_q6_oracle	TR12_Q6_ORACLE	DRIVER-BUILT	the driver counts the regenerated walks and prints '# walks regenerated by --kc-enum-desc: N' (:1123) without comparing N to anything; rc is verify.py's. An empty --kc-enum-desc would publish 0 walks and an oracle verdict over nothing. The row is SKIP:universe-too-large above n=9, so it cannot execute at n=31 and is the lowest-severity entry here.
a1_q2b	TR12_Q2B	DRIVER-BUILT	the three REL walks at r = 0, floor(N/2), N-1 are printed by a driver loop whose only failure flag is set by the solver's exit status (:1328-1336) -- membership, rank(unrank(r)) == r and the walks themselves are checked by nothing. G-31 2026-09-11 group A1: 'rc only'. Full-31 answers for all three are banked in F_ONLY_CATALOG_SURVEY_2026_08_04.md section C and are not read.
a2_v4	TR12_V4_TSV	DRIVER-BUILT	a bare awk re-projection of q3_profile_exact.tsv into a k/g/g_parent/f table (:1756-1757) with no row count, no arithmetic identity and no comparison to the Q3 reader next door. An absent or truncated profile TSV yields a header-only table and rc 0 -- the exact D11 shape. G-31 2026-09-11 section 3 item 5.
ALLOW_EOF
}
