...
|---|---|---|
| `n9/` | n = 9, N = 26,112 | the reduced-universe battery. Self-contained: `scripts/tr12_repro.sh --n9` builds its own f/g/t ladders in a temp dir in about a second, needs no campaign data, no disk and no network, and finishes in ~90 s on two cores, and it costs nothing. 🔴 **It is NOT "the whole correctness argument", which is what this cell used to say (Codex MQ1 §1).** The blocks are of two kinds and the difference matters: a few are graded against an INDEPENDENTLY DERIVED value (an exact count, an identity recomputed by another route), and those do test correctness; most are STABILITY PINS captured from a first run, and a stability pin that matches proves only that the engine still emits what it emitted before. That is a regression check, not a proof of correctness -- if the first capture was wrong, every later run agrees with it forever. `--regen` both generates and grades, which is exactly why the diff must be READ (CX-39). |
