...
# ================================================================================================
# KC CLASS-SWAP DETECTOR — folded in from a standalone scripts/ file on 2026-09-11.
#
# WHY IT MOVED. It shipped as a separate script that day because `solve.py` is inside the
# reproduction stamp's SURFACE and a separate `scripts/*.py` is not, so keeping it out avoided
# re-opening the currency ratchet. That was a COST argument, not a constraint, and the operator
# asked the right question: why not fold it before the run rather than after. The answer is that
# there was no reason — the battery never executes this code and the run never calls it, so it
# cannot affect the run; the only price was one gate cycle, which is cheaper than carrying a
# deferred commitment. This project's record with "fold it in later" is poor (Q-481 sat two days
# in neither-fixed-nor-deferred), so it was folded now.
#
# ⚠ AN EARLIER JUSTIFICATION FOR THE SEPARATE WAS WRONG and is recorded so it is not repeated:
# "folding it into solve.py would put the detector inside the program that produces what it
# checks" is false. `by_class` AND the sample draw are both produced by **solve.c**; solve.py is a
# consumer. The detector's independence comes from the f-unranking sample path, which is unchanged
# by which file holds the comparison. The `c2c3_joint_null.py` precedent does not transfer.
#
# WHAT IT IS, AND WHAT ITS MEASURED POWER IS — do not read a CLEAN verdict as "no swap":
...
# ================================================================================================
_KC_SWAP_DOC = """Statistical detector for whole-row class-mass mis-attribution in the KC atlas.
...
class _TokenArgParser(argparse.ArgumentParser):
    ...
    def error(self, message):
        print("%s=ERROR" % TOKEN)
        print("kc-class-swap-detect: bad invocation: %s" % message)
        self.print_usage(sys.stderr)
        sys.exit(2)


def kc_class_swap_detect_cli(argv=None):
    p = _TokenArgParser(
        prog="solve.py --kc-class-swap-detect",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=_KC_SWAP_DOC)
    ...
    try:
        ...
        print("%s=ERROR" % TOKEN)
        print("kc-class-swap-detect: %s" % exc)
        print("kc-class-swap-detect: " + LIMITS
              % (args.alpha, PROD_DRAWS, PROD_CELLS))
        print("kc-class-swap-detect: " + POWER)
        return 2

    verdict = "SUSPECT" if res["suspect"] else "CLEAN"
    print("%s=%s" % (TOKEN, verdict))
    if res["mode"] == "exact":
        print("kc-class-swap-detect: mode=exact n=%d walks=%d "
              "N_total=%d cells=%d differing_cells=%d" %
              (n, draws, total, res["cells"], int(res["stat"])))
    else:
        print("kc-class-swap-detect: mode=sample n=%d draws=%d "
    ...
    if res["worst"] is not None:
        print("kc-class-swap-detect: worst_cell k=%d d=%d"
              % res["worst"])
    for row in res["bad"][:max(0, args.show)]:
        if res["mode"] == "exact":
            print("kc-class-swap-detect:   k=%d d=%d observed=%d "
                  "atlas=%d" % row)
        else:
            print("kc-class-swap-detect:   k=%d d=%d empirical=%.6f "
                  "atlas=%.6f diff=%.6f" % row)
    if len(res["bad"]) > max(0, args.show):
        print("kc-class-swap-detect:   ... %d more"
              % (len(res["bad"]) - max(0, args.show)))
    if records:
        print("kc-class-swap-detect: skipped %d --kc-record "
              "representative line(s); they are not draws" % records)
    print("kc-class-swap-detect: atlas=%s sample=%s"
    ...
    if args.json:
        try:
            ...
            print("%s=ERROR" % TOKEN)
            print("kc-class-swap-detect: cannot write --json %s: %s"
                  % (args.json, exc))
            return 2
...
def main():
    ...
    # this module already defines --atlas-queries/-out/-select/-q3-trace/-verdicts/-selftest/
    # -walks/-keep/-fault. argparse ABBREVIATION MATCHING makes a bare `--atlas` ambiguous against
    # all nine, and `nargs=REMAINDER` does not prevent it -- measured during the fold: the first
    # wiring produced "error: ambiguous option: --atlas could match --atlas-queries, ...", i.e. a
    # detector unreachable through its own documented interface. Splitting argv here hands the
    # detector its own argument space and leaves the query parser untouched.
    if "--kc-class-swap-detect" in sys.argv[1:]:
        i = sys.argv.index("--kc-class-swap-detect")
        sys.exit(kc_class_swap_detect_cli(sys.argv[i + 1:]))
